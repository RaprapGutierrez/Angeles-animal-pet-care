...
const BranchToastStack = ({ toasts, onClose }) => {
  ...
  return (
    <div style={{
      ...
      {visible.map(t => (
        <BranchToastItem key={t.id} show={t.show} message={t.message} type={t.type} onClose={() => onClose(t.id)} />
      ))}
    </div>
  );
};
...
const emptyModules = () => ({ admin: [], manager: [], employee: [], customer: [] });

// ── Branch form validation ──────────────────────────────────────────────────
const validateBranchForm = (form, isEdit) => {
  if (!form.name?.trim()) return { valid: false, message: 'Branch name is required' };
  if (!form.address?.trim()) return { valid: false, message: 'Address is required' };
  if (!form.phone?.trim()) return { valid: false, message: 'Phone number is required' };
  if (!isEdit && !form.email?.trim()) return { valid: false, message: 'Branch email is required to create a manager account' };
  if (form.email?.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim())) {
    return { valid: false, message: 'Please enter a valid email address' };
  }
  if (!form.lat?.toString().trim() || !form.lng?.toString().trim()) {
    return { valid: false, message: 'Latitude and longitude are required so customers can find this branch on the map' };
  }
  const latNum = parseFloat(form.lat);
  const lngNum = parseFloat(form.lng);
  if (isNaN(latNum) || isNaN(lngNum)) return { valid: false, message: 'Latitude and longitude must be valid numbers' };
  ...
  if (Math.abs(latNum) < 0.0001 && Math.abs(lngNum) < 0.0001) {
    return { valid: false, message: 'Coordinates look like a placeholder (0, 0) — please enter the exact pinned location' };
  }
  ...
  if (latDecimals < 4 || lngDecimals < 4) {
    return { valid: false, message: 'Please provide precise coordinates (at least 4 decimal places) so the branch is easy to find on the map' };
  }
...
const ModuleSelector = ({ modules, onChange }) => {
  ...
  return (
    <div style={{ border: '1.5px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
      {/* Role tabs */}
      <div className="module-role-tabs" style={{ display: 'flex', borderBottom: '1.5px solid var(--border)', background: '#f8fafc' }}>
        {Object.keys(ALL_MODULES).map(role => {
          ...
          return (
            <button key={role} onClick={() => setActiveRole(role)}
              style={{
                flex: 1, padding: '8px 4px', fontSize: 12, fontWeight: 700, border: 'none', cursor: 'pointer', transition: 'all 0.15s',
          ...
          {roleModules.map(mod => {
            ...
            return (
              <div key={mod.key} onClick={() => toggleModule(mod.key)}
                style={{
                  display: 'flex', alignItems: 'center', gap: 5, padding: '5px 11px', borderRadius: 99, fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s', userSelect: 'none',
...
const ConfirmDialog = ({ show, title, message, accent = '#dc2626', confirmText = 'Confirm', onConfirm, onCancel }) => {
  if (!show) return null;
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 99999, padding: 16 }}>
      <div style={{ background: 'var(--card)', borderRadius: 14, width: '100%', maxWidth: 400, boxShadow: '0 20px 60px rgba(0,0,0,0.28)', overflow: 'hidden' }}>
        <div style={{ background: accent === '#dc2626' ? 'linear-gradient(135deg,#fef2f2,#fee2e2)' : 'linear-gradient(135deg,#fffbeb,#fef3c7)', padding: '18px 22px 14px', borderBottom: `1px solid ${accent}44`, display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 38, height: 38, borderRadius: '50%', background: accent, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            {accent === '#dc2626'
              ? <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round"><polyline points="3 6 5 6 21 6" /><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6" /><path d="M10 11v6M14 11v6" /><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2" /></svg>
              : <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>
            }
          </div>
          <div>
            <h3 style={{ margin: 0, fontSize: 15, fontWeight: 800, color: accent === '#dc2626' ? '#7f1d1d' : '#78350f' }}>{title}</h3>
            <p style={{ margin: '2px 0 0', fontSize: 12, color: accent }}>{accent === '#dc2626' ? 'This action cannot be undone' : 'You have unsaved changes'}</p>
          </div>
        </div>
        ...
        <div style={{ padding: '12px 22px 18px', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          <button onClick={onCancel} style={{ padding: '8px 18px', borderRadius: 8, border: '1.5px solid var(--border)', background: 'var(--card)', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', color: 'var(--text)' }}>
            Cancel
          </button>
          <button onClick={onConfirm} style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: accent, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
            {confirmText}
          </button>
        </div>
      </div>
...
const Branches = () => {
  ...
  const [confirm, setConfirm] = useState({ show: false, title: '', message: '', accent: '#dc2626', confirmText: 'Confirm', onConfirm: null });
  ...
  const openAdd = () => {
    setForm({ name: '', address: '', phone: '', email: '', manager: '', status: 'Active', services: [], lat: '', lng: '', modules: emptyModules() });
    setEditBranch(null); setFormDirty(false); setShowModal(true);
  };
  const openEdit = (b) => {
    setForm({ ...b, services: b.services || [], lat: b.lat || '', lng: b.lng || '', modules: b.modules || defaultModules() });
    setEditBranch(b); setFormDirty(false); setShowModal(true);
  };
  ...
  const handleModalClose = () => {
    if (formDirty) {
      setConfirm({
        ...
        onConfirm: () => { setConfirm(c => ({ ...c, show: false })); setShowModal(false); setFormDirty(false); },
      });
    } else {
      setShowModal(false);
    }
  };
  const toggleService = (svc) => { setFormDirty(true); setForm(prev => ({ ...prev, services: prev.services.includes(svc) ? prev.services.filter(s => s !== svc) : [...prev.services, svc] })); };
  ...
  const saveBranch = async () => {
    ...
    if (editBranch) {
      ...
      setCreating(false);
      if (error) { showToast('Failed to update branch: ' + error.message, 'error'); return; }
      fetchBranches(); setShowModal(false); setFormDirty(false);
      showToast('Branch updated successfully', 'success');
    } else {
      const { error: insertError } = await supabase.from('branches').insert([payload]);
      if (insertError) { setCreating(false); showToast('Error saving branch: ' + insertError.message, 'error'); return; }
      ...
      try {
        ...
        showToast(`Branch saved! But account creation failed: ${err.message}`, 'warning');
      }
  ...
  const doDelete = async () => {
    ...
    if (error) { showToast('Error deleting branch: ' + error.message, 'error'); return; }
  ...
  const handleDeleteClick = (id) => {
    setConfirm({
      show: true,
      title: 'Delete Branch',
      message: 'Are you sure you want to delete this branch? All associated data will be permanently removed and cannot be recovered.',
      accent: '#dc2626',
      confirmText: 'Yes, Delete Branch',
      onConfirm: () => { setConfirm(c => ({ ...c, show: false })); setDeleteId(id); doDelete(); },
    });
  };
  ...
  return (
    <Layout>
      ...
      <div className="branches-page">
        <div className="branches-topbar-pos branches-topbar">
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <h1 style={{ fontSize: 20, fontWeight: 800, color: 'var(--text)', margin: 0, whiteSpace: 'nowrap' }}>Branch Management</h1>
              <p style={{ fontSize: 13, color: 'var(--muted)', margin: 0, whiteSpace: 'nowrap' }}>Manage all hospital branches</p>
            </div>
          </div>
          <div style={{ position: "fixed", bottom: 28, right: 28, zIndex: 999, display: "flex", alignItems: "center", gap: 10 }}
            ...
            <span className="fab-tooltip" style={{
              ...
              <span style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.2 }}>
                <span style={{ fontSize: 12, fontWeight: 800, color: '#fff' }}>Add Branch</span>
                <span style={{ fontSize: 10, fontWeight: 500, color: 'rgba(255,255,255,0.55)' }}>Register a new location</span>
              </span>
              <span style={{
        ...
        <div className="branches-content">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(195px,1fr))', gap: 14, marginBottom: 24 }}>
            {loading ? (
              ...
              [
                { label: 'Total Branches', value: branches.length, icon: '/icon/branches.png', color: 'blue', sub: 'All registered branches' },
                { label: 'Active', value: branches.filter(b => b.status === 'Active').length, icon: '/icon/available.png', color: 'green', sub: 'Currently operating' },
              ].map((sc, i) => (
          ...
          {loading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: 12, alignItems: 'start' }}>
              {branches.map(b => {
                ...
                return (
                  <div key={b.id} className="fade-in" style={{ background: 'var(--card)', border: '1.5px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', transition: 'all 0.2s', boxShadow: isExpanded ? '0 4px 24px rgba(30,58,138,0.13)' : 'var(--shadow)' }}>
                    <div style={{ height: 88, background: 'var(--light-blue)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', overflow: 'hidden', position: 'relative', borderBottom: '1.5px solid #c7d2fe' }}>
                    ...
                    <div style={{ padding: 16 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8, gap: 8 }}>
                        <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', margin: 0, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.name}</h3>
                        <span className={`badge ${b.status === 'Active' ? 'badge-green' : 'badge-red'}`} style={{ flexShrink: 0 }}>{b.status}</span>
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexwrap: 'wrap' }}>
                        <button title="Edit" style={{ height: 28, padding: '0 10px', gap: 5, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: '#f8fafc', border: '1.5px solid #e2e8f0', color: '#475569', borderRadius: 20, fontSize: 11, fontWeight: 600, cursor: 'pointer' }} onClick={() => openEdit(b)}>
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                          Edit
                        </button>
                        <button title="Delete" style={{ height: 28, padding: '0 10px', gap: 5, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: '#fef2f2', border: '1.5px solid #fca5a5', color: '#dc2626', borderRadius: 20, fontSize: 11, fontWeight: 600, cursor: 'pointer' }} onClick={() => handleDeleteClick(b.id)}>
                        ...
                        <button title="View" style={{ height: 28, padding: '0 10px', gap: 5, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: '#eff6ff', border: '1.5px solid #bfdbfe', color: '#1d4ed8', borderRadius: 20, fontSize: 11, fontWeight: 600, cursor: 'pointer', marginLeft: 'auto' }} onClick={() => setViewBranch(b)}>
          ...
          <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>
            ...
            <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              {(() => {
                ...
                return img ? (
                  <div style={{ borderRadius: 10, overflow: 'hidden', border: '1.5px solid var(--border)', background: '#f8fafc' }}>
                    <img
                      src={`/image/${img}`}
                      alt={viewBranch.name}
              ...
              {/* Basic Info */}
              <div style={{ background: '#f8fafc', borderRadius: 10, border: '1.5px solid var(--border)', overflow: 'hidden' }}>
                <div style={{ padding: '10px 14px', borderBottom: '1.5px solid var(--border)', background: '#f1f5f9' }}>
                  <p style={{ fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em', margin: 0 }}>Branch Info</p>
                </div>
                {[
                  { icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>, label: 'Address', val: viewBranch.address },
                  { icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.15 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.06 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.09 8.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 21 16z" /></svg>, label: 'Phone', val: viewBranch.phone },
                  { icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /><polyline points="22,6 12,13 2,6" /></svg>, label: 'Email', val: viewBranch.email },
                  { icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="8" r="4" /><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7" /></svg>, label: 'Manager', val: viewBranch.manager },
                  { icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="10" /><line x1="2" y1="12" x2="22" y2="12" /><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" /></svg>, label: 'Coordinates', val: viewBranch.lat && viewBranch.lng ? `${viewBranch.lat}, ${viewBranch.lng}` : '—' },
                  ...
                  <div key={label} style={{
                    ...
                    <span style={{ color: '#94a3b8', flexShrink: 0, marginTop: 2, display: 'flex' }}>{icon}</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 2px' }}>{label}</p>
                      <p style={{ fontSize: 13, color: 'var(--text)', fontWeight: 600, margin: 0, wordBreak: 'break-word' }}>{val || '—'}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* ── ADD/EDIT MODAL ── */}
      {showModal && (
        <div className="branch-modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 16 }}>
          <div className="branch-modal-box" style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>
            <div>
              {!editBranch && (
                <div className="branch-modal-banner" style={{ height: 110, background: '#eef2ff', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', overflow: 'hidden', position: 'relative', borderBottom: '1.5px solid #c7d2fe' }}>
                  ...
                  <button onClick={handleModalClose} style={{ position: 'absolute', top: 10, right: 14, background: 'rgba(255,255,255,0.8)', border: '1px solid #c7d2fe', borderRadius: 6, width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', zIndex: 2 }}>
            ...
            <div className="modal-body">
              ...
              <div className="form-grid">
                <div className="form-group form-full"><label>Branch Name *</label><input type="text" value={form.name} onChange={e => { setFormDirty(true); setForm({ ...form, name: e.target.value }); }} placeholder="e.g. Tarlac City Branch" /></div>
                <div className="form-group form-full"><label>Address <span style={{ color: '#dc2626' }}>*</span></label><input type="text" value={form.address} onChange={e => { setFormDirty(true); setForm({ ...form, address: e.target.value }); }} placeholder="Full address" /></div>
                <div className="form-group"><label>Phone <span style={{ color: '#dc2626' }}>*</span></label><input type="text" value={form.phone} onChange={e => { setFormDirty(true); setForm({ ...form, phone: e.target.value }); }} /></div>
                <div className="form-group"><label>Branch Email {!editBranch && <span style={{ color: '#dc2626' }}>*</span>}</label><input type="email" value={form.email} onChange={e => { setFormDirty(true); setForm({ ...form, email: e.target.value }); }} placeholder="manager@ach.com" /></div>
                <div className="form-group"><label>Manager Name</label><input type="text" value={form.manager} onChange={e => { setFormDirty(true); setForm({ ...form, manager: e.target.value }); }} /></div>
                <div className="form-group">
                  <label>Status</label>
                  <CustomSelect
                    value={form.status}
                    onChange={val => { setFormDirty(true); setForm({ ...form, status: val }); }}
                    placeholder="— Select Status —"
                    options={['Active', 'Inactive']}
                  />
                </div>
                <div className="form-group">
                  <label>Latitude <span style={{ color: '#dc2626' }}>*</span></label>
                  <input type="text" value={form.lat} onChange={e => { setFormDirty(true); setForm({ ...form, lat: e.target.value }); }} placeholder="e.g. 15.205205" />
                </div>
                <div className="form-group">
                  <label>Longitude <span style={{ color: '#dc2626' }}>*</span></label>
                  <input type="text" value={form.lng} onChange={e => { setFormDirty(true); setForm({ ...form, lng: e.target.value }); }} placeholder="e.g. 120.580370" />
                  {(form.lat || form.lng) && !validateBranchForm(form, !!editBranch).valid && validateBranchForm(form, !!editBranch).message.toLowerCase().includes('coordinate') && (
                    <p style={{ fontSize: 11, color: '#dc2626', margin: '4px 0 0' }}>{validateBranchForm(form, !!editBranch).message}</p>
                  )}
                </div>
                ...
                <div className="form-group form-full">
                  ...
                  <ModuleSelector
                    modules={form.modules}
                    onChange={mods => { setFormDirty(true); setForm(prev => ({ ...prev, modules: mods })); }}
                  />
                </div>
            ...
            <div className="modal-footer">
              <button className="btn btn-ghost branches-btn-auto" onClick={handleModalClose}>Cancel</button>
              <button
                className="btn btn-primary branches-btn-auto"
                onClick={saveBranch}
                disabled={creating || !validateBranchForm(form, !!editBranch).valid}
                style={{ opacity: (creating || !validateBranchForm(form, !!editBranch).valid) ? 0.5 : 1, cursor: (creating || !validateBranchForm(form, !!editBranch).valid) ? 'not-allowed' : 'pointer' }}
              >
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 16 }}>
          <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 400, boxShadow: '0 20px 60px rgba(0,0,0,0.25)', overflow: 'hidden' }}>
              ...
              {[['Branch', createdAccount.branchName], ['Email', createdAccount.email], ['Password', createdAccount.password]].map(([label, value]) => (
                <div key={label} style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#f8fafc', border: '1.5px solid var(--border)', borderRadius: 8, padding: '9px 14px' }}>
                    ...
                    <button className="btn btn-ghost btn-sm branches-btn-auto" onClick={() => navigator.clipboard.writeText(value)}>Copy</button>
                  </div>
                </div>
              ))}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, padding: '14px 24px', borderTop: '1px solid var(--border)' }}>
              <button className="btn btn-ghost branches-btn-auto" onClick={() => { navigator.clipboard.writeText(`Branch: ${createdAccount.branchName}\nEmail: ${createdAccount.email}\nPassword: ${createdAccount.password}`); }}>Copy All</button>
              <button className="btn btn-primary branches-btn-auto" onClick={() => setCreatedAccount(null)}>Done</button>
            </div>
          </div>
        </div>
      )}
