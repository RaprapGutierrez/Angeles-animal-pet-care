...
const InsightCard = ({ icon, title, body, color, action }) => (
  <div style={{
    background: "#fff", borderRadius: 14, border: `1px solid ${color}30`,
    ...
    <div style={{ flex: 1 }}>
      <p style={{ margin: "0 0 4px", fontSize: 13, fontWeight: 700, color: "#1e293b" }}>{title}</p>
      <p style={{ margin: "0 0 8px", fontSize: 12, color: "#64748b", lineHeight: 1.5 }}>{body}</p>
      {action && (
        <span style={{
...
const PredictiveAnalytics = () => {
  ...
  useEffect(() => {
    ...
    const HOUR_MAP = { "08:00 AM": 0,"09:00 AM": 1,"10:00 AM": 2,"11:00 AM": 3,
  ...
  return (
    <Layout>
      ...
      <style>{`
        ...
        .pa-card { animation: fadeUp 0.4s ease both; }
      `}</style>
      ...
      <div className="content">
          ...
          {loading ? (
            ...
            <div key={kpi.label} className="pa-card" style={{ ...card, animationDelay: `${i * 0.07}s` }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-end", gap: 8, marginBottom: 8 }}>
                <h3 style={{ margin: 0, fontSize: 28, fontWeight: 900, color: "#1e293b", lineHeight: 1 }}>{kpi.value}</h3>
                {kpi.delta && (
        ...
        {!loading && analytics?.insights?.length > 0 && (
          <div className="pa-card" style={{ ...card, marginBottom: 24, animationDelay: "0.28s" }}>
            <SectionHeader icon="🧠" title="AI-Generated Insights" subtitle="Based on historical patterns from the last 90 days" />
        ...
        {tab === "visits" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            ...
            <div className="pa-card" style={{ ...card, animationDelay: "0.1s" }}>
              <SectionHeader icon="📊" title="Busiest Days of the Week" subtitle="Aggregate visits over the past 90 days" />
              {loading ? <Skel h={140} /> : (
                <BarChart
                  labels={DAYS}
            ...
            <div className="pa-card" style={{ ...card, animationDelay: "0.15s" }}>
              <SectionHeader icon="⏱️" title="Visits by Time Slot" subtitle="Which hours see the most traffic" />
              {loading ? <Skel h={140} /> : (
                <BarChart
            ...
            <div className="pa-card" style={{ ...card, animationDelay: "0.2s" }}>
              <SectionHeader icon="🗓️" title="Visit Activity Heatmap" subtitle="Last 20 days + upcoming week" />
            ...
            <div className="pa-card" style={{ ...card, animationDelay: "0.25s" }}>
              <SectionHeader icon="🔮" title="Predicted Busy Days" subtitle="Next 14 days — based on day-of-week patterns" />
              {loading ? <Skel h={160} /> : (
                <div>
            ...
            <div className="pa-card" style={{ ...card, gridColumn: "1 / -1", animationDelay: "0.3s" }}>
              <SectionHeader icon="🩺" title="Top Services Requested" subtitle="Appointment purposes ranked by volume" />
              {loading ? <Skel h={120} /> : (
        ...
        {tab === "inventory" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            ...
            {/* Critical stock */}
            <div className="pa-card" style={{ ...card, animationDelay: "0.1s" }}>
              <SectionHeader icon="⚠️" title="Low & Critical Stock" subtitle="Items at or near reorder level" />
              {loading ? <Skel h={200} /> : (
                <>
            ...
            {/* Inventory turnover */}
            <div className="pa-card" style={{ ...card, animationDelay: "0.15s" }}>
              <SectionHeader icon="🔄" title="Inventory Usage Rate" subtitle="Items with highest consumption relative to stock level" />
              {loading ? <Skel h={200} /> : (
                <>
            ...
            <div className="pa-card" style={{ ...card, gridColumn: "1 / -1", animationDelay: "0.2s" }}>
              <SectionHeader icon="📋" title="Restock Recommendations" subtitle="Predictive suggestions based on stock vs. usage trends" />
              {loading ? (
        ...
        {tab === "patients" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            ...
            <div className="pa-card" style={{ ...card, animationDelay: "0.1s" }}>
              <SectionHeader icon="📈" title="New Patient Registrations" subtitle="Month-over-month for the past 3 months" />
              {loading ? <Skel h={140} /> : (
                <BarChart
            ...
            {/* Appointment trend */}
            <div className="pa-card" style={{ ...card, animationDelay: "0.15s" }}>
              <SectionHeader icon="📅" title="Appointment Volume Trend" subtitle="Monthly appointment counts" />
              {loading ? <Skel h={140} /> : (
                <BarChart
                  labels={analytics?.monthLabels || []}
            ...
            {/* Growth projection */}
            <div className="pa-card" style={{ ...card, gridColumn: "1 / -1", animationDelay: "0.2s" }}>
              <SectionHeader icon="🔭" title="3-Month Projection" subtitle="Extrapolated from current growth trend (illustrative)" />
              {loading ? <Skel h={100} /> : (() => {
              ...
              <p style={{ margin: "12px 0 0", fontSize: 11, color: "#94a3b8" }}>
                ℹ️ Projection assumes a conservative 8% month-over-month growth based on the last 90-day trend. Actual results may vary.
              </p>
            </div>

            {/* Purpose breakdown for patients */}
            <div className="pa-card" style={{ ...card, gridColumn: "1 / -1", animationDelay: "0.25s" }}>
              <SectionHeader icon="🩺" title="Visit Reasons — Patient Perspective" subtitle="What patients most commonly come in for" />
              {loading ? <Skel h={100} /> : (
