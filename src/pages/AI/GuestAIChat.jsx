...
const StepLine = ({ done }) => (
  <div className={`g-step-line${done ? " done" : ""}`} />
);
...
const TypingDots = ({ light = false }) => (
  <div className="g-typing">
    {[0, 1, 2].map((i) => (
      <span
        key={i}
        className={`g-typing-dot${light ? " light" : ""}`}
    ...
    <div className="g-fc-container">
      ...
      <div className="g-fc-header">
        ...
        <div>
          ...
          <div className="g-fc-header-sub">
            Ask anything about {petName}'s assessment
          </div>
        </div>
      ...
      <div className="g-fc-messages">
        {messages.map((msg, i) => (
          <div
            key={i}
            className={`g-fc-msg-row${msg.role === "user" ? " user" : ""}`}
          >
            {msg.role === "assistant" && (
      ...
      {limitReached ? (
        <div className="g-fc-locked">
          <p className="g-fc-locked-text">
            You've reached the {MAX_GUEST_QUESTIONS}-question limit for guests.
            Log in or create a free account to keep chatting with the AI.
          </p>
          <div className="g-fc-locked-actions">
            ...
            <button
              ...
              className="g-fc-locked-btn secondary"
            >
              Create Account
            </button>
          </div>
...
const GuestAIChat = () => {
  ...
  const buildPreAssessmentPayload = () => ({
    petName: form.petName,
    petType: form.petType,
  ...
  // Router state is lost on a refresh or a real page redirect (e.g. Supabase
  // signup/email-confirm), so persist a fallback copy the destination page
  // can recover from even if location.state comes back empty.
  const stashPreAssessment = () => {
    try {
      sessionStorage.setItem(
        "pendingPreAssessment",
        JSON.stringify(buildPreAssessmentPayload()),
      );
    } catch (e) {
      console.warn("Could not stash pre-assessment:", e);
    }
  };
  ...
  const handleBooking = () => {
    ...
    navigate("/login", {
      state: {
        preAssessment: buildPreAssessmentPayload(),
        ...
        <div className={`g-topbar${isMobile ? " mobile" : ""}`}>
            ...
            <div className="g-topbar-name-wrap">
              ...
              {!isMobile && (
                <div className="g-topbar-sub">
                  AI Symptoms Pre-Assessment · Guest Access
                </div>
              )}
            </div>
          </div>
          <div className="g-topbar-actions">
            {step === 4 && (
              <button
                onClick={reset}
                className={`g-topbar-btn g-topbar-btn-ghost${isMobile ? " mobile" : ""}`}
              >
                <Icons.RotateCcw /> New
              </button>
            )}
            ...
            <button
              ...
              className={`g-topbar-btn g-topbar-btn-primary${isMobile ? " mobile" : ""}`}
            >
          ...
          <div className="g-content-inner">
            ...
            {step === 1 && (
              <>
                <div className={`g-intro-card${isMobile ? " mobile" : ""}`}>
                  <div className="g-intro-icon">
                    <PetsIcon size={20} color="#fff" />
                  </div>
                  <div>
                    <h2 className={`g-intro-title${isMobile ? " mobile" : ""}`}>
                      Tell us about your pet
                    </h2>
                    <p className="g-intro-sub">
                      No account needed · Your info stays private
                    </p>
                  </div>
                </div>
                ...
                <div>
                  <label className="g-field-label">
                    Pet's Name <span className="g-field-required">*</span>
                  </label>
                  <input
                    className="g-input"
                    value={form.petName}
                ...
                <div>
                  <label className="g-field-label">
                    Type of Pet <span className="g-field-required">*</span>
                  </label>
                  <div className="g-pettype-row">
                    {PET_TYPES.map((t) => {
                      ...
                      return (
                        <button
                          key={t}
                          className={`g-pettype-btn${active ? " active" : ""}`}
                ...
                <div>
                  <label className="g-field-label">
                    Approximate Age <span className="g-field-required">*</span>
                  </label>
                  <div className="g-age-list">
                    {AGE_OPTIONS.map((a) => {
                      ...
                      return (
                        <button
                          key={a}
                          className={`g-age-btn${active ? " active" : ""}`}
                          onClick={() => set("petAge", a)}
                        >
                          <span
                            className={`g-age-radio${active ? " active" : ""}`}
                          />
                          {a}
                        </button>
                      );
            ...
            {step === 2 && (
              <>
                <div className={`g-intro-card${isMobile ? " mobile" : ""}`}>
                  <div className="g-intro-icon">
                    <StethoscopeIcon size={20} color="#fff" />
                  </div>
                  <div>
                    <h2 className={`g-intro-title${isMobile ? " mobile" : ""}`}>
                ...
                <div>
                  <label className="g-field-label">
                    Describe in detail{" "}
                    <span className="g-field-required">*</span>
                  </label>
                  <textarea
                    className="g-input"
                    value={form.symptoms}
                    onChange={(e) => set("symptoms", e.target.value)}
                    placeholder={`e.g. ${form.petName} has been vomiting since this morning…`}
                    rows={5}
                ...
                <div>
                  <label className="g-field-label">
                    Additional notes{" "}
                    <span className="g-field-optional">(optional)</span>
                  </label>
                  <textarea
                    className="g-input note"
                    value={form.additionalNotes}
                    onChange={(e) => set("additionalNotes", e.target.value)}
                    placeholder="Recent diet changes, medications, when symptoms started…"
                    rows={3}
                    style={{ resize: "vertical" }}
                  />
                </div>
                ...
                <div className="g-disclaimer-box">
                  <span className="g-disclaimer-icon">
                    <Icons.AlertTriangle />
                  </span>
                  <p className="g-disclaimer-text">
                    <strong>Disclaimer:</strong> This AI assessment is for
                    informational purposes only and does not replace
                ...
                <div className="g-step2-actions">
                  ...
                  <button
                    ...
                    className={`g-submit-btn${canSubmit && !loading ? " enabled g-btn" : ""}`}
                  >
                    <StethoscopeIcon
                      size={16}
                      color={
            ...
            {step === 4 && assessment && urgency && (
              ...
              >
                ...
                >
                  ...
                  <div>
                    <div
                      className="g-urgency-title"
                      style={{ color: urgency.color }}
                    >
                      {urgency.label}
                    </div>
                    <div
                      className="g-urgency-reason"
                      style={{ color: urgency.color }}
                    >
                ...
                <div className="g-pet-row">
                  <PetsIcon size={20} color="#fff" />
                  <div>
                    <div className="g-pet-row-title">
                      {form.petName} · {form.petType} · {form.petAge}
                    </div>
                    <div className="g-pet-row-sub">
                      Assessment complete · Guest access
                    </div>
                  </div>
                </div>
                    ...
                    <div className="g-homecare-card">
                      ...
                      {assessment.homeCareTips.map((t, i) => (
                        <div key={i} className="g-homecare-item">
                          <span className="g-homecare-icon">
                            <Icons.CheckCircle />
                          </span>
                ...
                {!showChat && (
                  <div className="g-chat-toggle-card">
                    <div className="g-chat-toggle-icon">
                      <Icons.MessageCircle />
                    </div>
                    <div className="g-chat-toggle-body">
                      <p className="g-chat-toggle-title">
                        Still have questions?
                      </p>
                      <p className="g-chat-toggle-sub">
                        Ask the AI anything about {form.petName}'s condition
                        before booking.
                      </p>
                    </div>
                    <button
                ...
                <p className="g-disclaimer-footer">
                  This AI assessment does not replace professional veterinary
