...
const CustomSelect = ({ value, onChange, options, placeholder = '—', accent = '#05328A' }) => {
  ...
  const portal = open && typeof document !== 'undefined'
    ? ReactDOM.createPortal(
      <div ref={ref} style={{ position: 'absolute', top: dropPos.top, left: dropPos.left, width: dropPos.width, background: '#fff', borderRadius: 12, zIndex: 99999, boxShadow: '0 16px 40px rgba(0,0,0,0.13), 0 4px 12px rgba(0,0,0,0.07), 0 0 0 1px rgba(0,0,0,0.06)', border: '1.5px solid #e8edf4', maxHeight: 260, overflowY: 'auto', padding: '5px' }}>
        {options.map((opt, i) => {
          ...
          return (
            <div key={i} onClick={() => { onChange(optVal); setOpen(false); }}
              style={{ padding: '8px 10px', fontSize: 13, fontWeight: isSelected ? 700 : 500, color: isSelected ? accent : '#0f172a', cursor: 'pointer', transition: 'background 0.12s, color 0.12s', background: isSelected ? `${accent}12` : 'transparent', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 1 }}
              onMouseEnter={e => { if (!isSelected) e.currentTarget.style.background = '#f4f6fa'; }}
              onMouseLeave={e => { if (!isSelected) e.currentTarget.style.background = 'transparent'; }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                <div style={{ width: 6, height: 6, borderRadius: '50%', flexShrink: 0, background: isSelected ? accent : 'transparent', border: `1.5px solid ${isSelected ? accent : '#cbd5e1'}` }} />
  ...
  return (
    <div style={{ position: 'relative', width: '100%' }}>
      <div ref={triggerRef} onClick={handleOpen}
        style={{ width: '100%', padding: '12px 34px 12px 12px', border: '1.5px solid', borderRadius: 10, background: open ? 'linear-gradient(135deg, #ffffff 0%, #f5f3ff 100%)' : '#fff', fontSize: 13, fontWeight: 600, color: value ? '#0f172a' : '#94a3b8', cursor: 'pointer', userSelect: 'none', boxSizing: 'border-box', boxShadow: open ? `0 0 0 3px ${accent}22` : 'none', borderColor: open ? accent : '#e2e8f0', transition: 'border-color 0.18s, box-shadow 0.18s', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, minHeight: 42 }}>
        <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>{label}</span>
        <div style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', width: 20, height: 20, borderRadius: 6, background: open ? accent : '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke={open ? '#fff' : '#94a3b8'} strokeWidth="3" strokeLinecap="round" style={{ transition: 'transform 0.2s', transform: open ? 'rotate(180deg)' : 'rotate(0deg)' }}>
...
const Register = () => {
  const [form, setForm] = useState({
    firstName: "", lastName: "", phoneNumber: "", sex: "Male", branchId: "",
  ...
  const handleNextStep = () => {
    if (step === 1) {
      ...
      if (!firstName || !lastName || !phoneNumber) {
        showModal("warning", "Missing Fields", "Please fill in your first name, last name, and contact number.");
        return;
      }
      if (!/^[0-9]{11}$/.test(phoneNumber)) {
        showModal("warning", "Invalid Contact Number", "Please Enter an 11-digit contact number.");
        return;
      }
      setStep(2);
    } else if (step === 2) {
      if (!form.branchId) {
        showModal("warning", "Missing Branch", "Please select your preferred branch.");
        return;
      }
      setStep(3);
    } else if (step === 3) {
      if (!form.email) {
        showModal("warning", "Missing Email", "Please enter your email address.");
        return;
      }
  ...
  const passwordStrength = () => {
    ...
    const score = [p.length >= 8, hasUpper, hasLower, hasNumber, hasSymbol].filter(Boolean).length;
  ...
  const handleRegister = async () => {
    const { firstName, lastName, phoneNumber, sex, branchId, email, password, confirmPassword } = form;
    ...
    if (!branchId) {
      showModal("warning", "Missing Branch", "Please go back and select your preferred branch.");
      return;
    }
    if (password.length < 8) {
      showModal("warning", "Weak Password", "Password must be at least 8 characters long.");
      return;
    }
    if (!/[A-Z]/.test(password)) {
      showModal("warning", "Weak Password", "Password must contain at least one uppercase letter (A–Z).");
      return;
    }
    if (!/[0-9]/.test(password)) {
      showModal("warning", "Weak Password", "Password must contain at least one number (0–9).");
      return;
    }
    if (!/[^A-Za-z0-9]/.test(password)) {
      showModal("warning", "Weak Password", "Password must contain at least one symbol (e.g. @, #, !, $).");
      return;
    }
    ...
    try {
      ...
      if (signUpError) {
        showModal("danger", "Registration Failed", signUpError.message || "Could not create your account. Please try again.");
        return;
      }
      ...
      if (session) {
        showModal("success", "Welcome!", "Your account has been created and you're now signed in.");
  ...
  return (
    <>
      ...
      <div className="auth-screen register" style={{ position: "fixed", inset: 0, width: "100vw", height: "100vh" }}>
        ...
        <div className="auth-brand auth-brand-pos-right">
          ...
          <p className="auth-brand-subtitle">Create your account and get started</p>
        </div>

        {/* ── Form left ── */}
        <div className="auth-form-wrapper" style={{ position: "fixed", inset: 0, display: "flex", alignItems: "center", justifyContent: "flex-start", padding: "24px 48px", pointerEvents: "none", overflowY: "auto" }}>
          <div className="auth-form" style={{ maxWidth: 400, width: "100%", borderRadius: 20, boxShadow: "0 8px 48px rgba(0,0,0,0.18)", padding: "24px 28px 20px", background: "#fff", pointerEvents: "all", maxHeight: "100vh", overflowY: "auto", scrollbarWidth: "none", msOverflowStyle: "none" }}>
            ...
            <p className="subtitle" style={{ fontFamily: "'Poetsen One', sans-serif" }}>Fill in the details below to register</p>

            {/* Step indicator */}
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 18 }}>
              {[1, 2, 3, 4].map(s => (
                <React.Fragment key={s}>
                  <div style={{ width: 26, height: 26, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, background: step >= s ? "#05328A" : "#e2e8f0", color: step >= s ? "#fff" : "#94a3b8", fontFamily: "'Poetsen One', sans-serif", flexShrink: 0 }}>
                    {step > s ? "✓" : s}
                  </div>
                  {s < 4 && <div style={{ flex: 1, height: 3, borderRadius: 99, background: step > s ? "#05328A" : "#e2e8f0" }} />}
                </React.Fragment>
              ))}
            </div>
              ...
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 10, marginBottom: 14 }}>
                  <div className="float-group">
                    <input type="text" placeholder=" " value={form.firstName} onChange={set("firstName")} onKeyDown={e => e.key === "Enter" && handleNextStep()} required />
                    <label>First name</label>
                  </div>
                  <div className="float-group">
                    <input type="text" placeholder=" " value={form.lastName} onChange={set("lastName")} onKeyDown={e => e.key === "Enter" && handleNextStep()} required />
                    <label>Last name</label>
                  </div>
                </div>

                <div className="float-group">
                  <input type="tel" placeholder=" " value={form.phoneNumber} onChange={set("phoneNumber")} onKeyDown={e => e.key === "Enter" && handleNextStep()} maxLength={11} inputMode="numeric" required />
                  <label>Contact number</label>
                </div>

                <div className="input-group" style={{ marginTop: 4 }}>
                  <button type="button" onClick={handleNextStep} className="auth-btn-text-stroke">
                    Next
                  </button>
                </div>
              </>
            ...
            {step === 2 && (
              <>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
                  <div>
                    <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#94a3b8", marginBottom: 5 }}>Sex</label>
                    <CustomSelect
                      value={form.sex}
                  ...
                  <div>
                    <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#94a3b8", marginBottom: 5 }}>Preferred branch</label>
                    <CustomSelect
                      value={form.branchId}
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                  <button type="button" onClick={handleBack} style={{ flex: 1, fontFamily: "'Poetsen One', sans-serif", background: "#e2e8f0", color: "#334155", border: "none", borderRadius: 10, padding: "12px 0", fontWeight: 700, cursor: "pointer" }}>
                    Back
                  </button>
                  <button type="button" onClick={handleNextStep} className="auth-btn-text-stroke" style={{ flex: 2 }}>
                    Next
                  </button>
                </div>
              </>
            ...
            {step === 3 && (
              <>
                <div className="float-group">
                  <input type="text" placeholder=" " value={form.email} onChange={set("email")} onKeyDown={e => e.key === "Enter" && handleNextStep()} required />
            ...
            {step === 4 && (
              <>
                <div className="float-group">
                  <input type="password" placeholder=" " value={form.password} onChange={set("password")} onKeyDown={e => e.key === "Enter" && handleRegister()} required />
                ...
                {strength && (
                  <div style={{ marginBottom: 10, marginTop: -6 }}>
                    <div style={{ display: "flex", gap: 4, marginBottom: 5 }}>
                      {[1, 2, 3, 4, 5].map(i => (
                        <div key={i} style={{ flex: 1, height: 4, borderRadius: 99, background: i <= strength.score ? strength.color : "#e2e8f0", transition: "background 0.3s" }} />
                      ))}
                    </div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 4 }}>
                      {[
                        ...
                        <span key={r.label} style={{ fontSize: 10, fontWeight: 600, padding: "2px 7px", borderRadius: 99, background: r.ok ? "#dcfce7" : "#f1f5f9", color: r.ok ? "#16a34a" : "#94a3b8" }}>
                          {r.ok ? "✓" : "·"} {r.label}
                        </span>
                      ))}
                    </div>
                ...
                <div className="float-group">
                  <input type="password" placeholder=" " value={form.confirmPassword} onChange={set("confirmPassword")} onKeyDown={e => e.key === "Enter" && handleRegister()} required />
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                  ...
                  <button type="button" onClick={handleRegister} disabled={loading} className="auth-btn-text-stroke" style={{ flex: 2 }}>
            ...
            <Link to="/ai-assessment" className="ai-assessment-link" style={{ marginTop: 8 }}>
