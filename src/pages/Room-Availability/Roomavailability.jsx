        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2, fontWeight: 600 }}>
              {room.type || 'General'}{room.infected ? <> · <svg style={{ display: 'inline', verticalAlign: 'middle' }} width="10" height="10" viewBox="0 0 24 24" fill="#dc2626"><circle cx="12" cy="12" r="10" /></svg> Isolation</> : ''}
            </div>
          </div>
        ...
        {isOccupied && room.patient ? (
          <div style={{ background: 'var(--bg)', borderRadius: 8, padding: '8px 10px', marginBottom: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 4 }}>
              <div style={{ width: 26, height: 26, borderRadius: '50%', background: STATUS_COLOR[room.status] + '22', border: `1.5px solid ${STATUS_COLOR[room.status]}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 11, fontWeight: 800, color: STATUS_COLOR[room.status] }}>
          ...
          <div style={{ background: '#fef2f2', borderRadius: 8, padding: '8px 10px', marginBottom: 8 }}>
...
const ViewModal = ({ room, onClose, onEdit, onDelete }) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.52)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 16 }}>
      <div style={{ background: 'var(--card)', borderRadius: 16, width: '100%', maxWidth: 460, boxShadow: '0 24px 64px rgba(0,0,0,0.28)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1 }}>
          {[
            ...
            { label: 'Discharge Date', value: room.discharge_date ? new Date(room.discharge_date).toLocaleDateString() : '— Not set —' },
          ].map(({ label, value }) => (
...
const CustomSelect = ({ value, onChange, options, placeholder = "—", accent = "#6366f1" }) => {
  ...
  const handleOpen = () => {
    if (!open && triggerRef.current) {
      ...
      setDropPos({ top: showAbove ? rect.top + window.scrollY - dropHeight - 6 : rect.bottom + window.scrollY + 6, left: rect.left + window.scrollX, width: rect.width });
    }
    setOpen(o => !o);
  };
        ...
        {[{ value: "", label: placeholder }, ...options].map((opt, i) => {
          ...
          return (
            <div key={i} onClick={() => { onChange(optVal); setOpen(false); }}
              style={{ padding: "8px 10px", fontSize: 13, fontWeight: isSelected ? 700 : 500, color: isEmpty ? "#b0bac9" : isSelected ? accent : "var(--text)", cursor: isEmpty ? "default" : "pointer", transition: "background 0.12s, color 0.12s", background: isSelected ? `${accent}12` : "transparent", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 1 }}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                {!isEmpty && <div style={{ width: 6, height: 6, borderRadius: "50%", flexShrink: 0, background: isSelected ? accent : "transparent", border: `1.5px solid ${isSelected ? accent : "#cbd5e1"}`, transition: "background 0.15s, border-color 0.15s" }} />}
  ...
  return (
    <div style={{ position: "relative", width: "100%" }}>
      <div ref={triggerRef} onClick={handleOpen}
        style={{ width: "100%", padding: "8px 34px 8px 12px", border: "1.5px solid", borderRadius: 9, background: open ? "linear-gradient(135deg,#ffffff,#f5f3ff)" : "linear-gradient(to bottom,#ffffff,#f8fafc)", fontSize: 13, fontWeight: 600, color: value ? "var(--text)" : "#b0bac9", cursor: "pointer", userSelect: "none", boxSizing: "border-box", boxShadow: open ? `0 0 0 3px ${accent}22, 0 2px 8px rgba(0,0,0,0.08)` : "0 1px 3px rgba(0,0,0,0.06), inset 0 1px 0 rgba(255,255,255,0.9)", borderColor: open ? accent : "#dde3ec", transition: "border-color 0.18s, box-shadow 0.18s, background 0.18s", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, position: "relative", minHeight: 36 }}
...
const RoomFormModal = ({ editRoom, form, setForm, onSave, onClose, saving, existingNumbers = [], validateRoom }) => {
  ...
  const DatePicker = ({ value, onChange, placeholder = "Pick a date" }) => {
    ...
    React.useEffect(() => { if (value) setViewDate(new Date(value + "T00:00:00")); }, [value]);
    React.useEffect(() => {
      const handler = (e) => { if (popRef.current && !popRef.current.contains(e.target) && triggerRef.current && !triggerRef.current.contains(e.target)) setOpen(false); };
    ...
    const handleOpen = () => {
      if (!open && triggerRef.current) {
        const rect = triggerRef.current.getBoundingClientRect();
        const spaceBelow = window.innerHeight - rect.bottom;
        setPopPos({ top: spaceBelow > 320 ? rect.bottom + window.scrollY + 6 : rect.top + window.scrollY - 316, left: rect.left + window.scrollX, width: Math.max(rect.width, 280) });
      }
      setOpen(o => !o);
    };
    ...
    const portal = open && typeof document !== "undefined" ? ReactDOM.createPortal(
      <div ref={popRef} style={{ position: "absolute", top: popPos.top, left: popPos.left, width: popPos.width, zIndex: 99999, background: "var(--card)", border: "1.5px solid #e8edf4", borderRadius: 14, boxShadow: "0 16px 48px rgba(0,0,0,0.16),0 4px 12px rgba(0,0,0,0.08)", overflow: "hidden" }}>
        <div style={{ background: "linear-gradient(135deg,#0f172a,#1e3a8a)", padding: "14px 16px 12px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <button onClick={() => setViewDate(new Date(year, month - 1, 1))} style={{ width: 28, height: 28, borderRadius: 8, border: "1px solid rgba(255,255,255,0.2)", background: "rgba(255,255,255,0.1)", cursor: "pointer", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="15 18 9 12 15 6" /></svg>
          </button>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 14, fontWeight: 800, color: "#fff" }}>{MONTHS[month]}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", padding: "10px 12px 4px", gap: 2 }}>
          {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((d, i) => (
            <div key={d} style={{ textAlign: "center", fontSize: 10, fontWeight: 800, color: i === 0 ? "#ef4444" : i === 6 ? "#3b82f6" : "#94a3b8", padding: "3px 0", textTransform: "uppercase" }}>{d}</div>
          ))}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", padding: "2px 12px 12px", gap: 2 }}>
          {cells.map((day, i) => {
            ...
            const isSelected = dateStr === value; const isToday = dateStr === todayStr;
            const isSun = i % 7 === 0; const isSat = i % 7 === 6;
            return (
              <div key={i} onClick={() => selectDay(day)}
                style={{ width: 32, height: 32, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: isSelected ? 800 : isToday ? 700 : 500, cursor: "pointer", background: isSelected ? "linear-gradient(135deg,#1e3a8a,#3b82f6)" : isToday ? "#eff6ff" : "transparent", color: isSelected ? "#fff" : isToday ? "#1e40af" : isSun ? "#ef4444" : isSat ? "#3b82f6" : "var(--text)", border: isToday && !isSelected ? "1.5px solid #bfdbfe" : "none", boxShadow: isSelected ? "0 2px 8px rgba(30,58,138,0.35)" : "none", margin: "auto", transition: "background 0.12s" }}
        ...
        <div style={{ padding: "8px 12px 10px", borderTop: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between" }}>
          <button onClick={() => { onChange(""); setOpen(false); }} style={{ fontSize: 11, fontWeight: 700, color: "#64748b", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", padding: "4px 8px", borderRadius: 6 }}>Clear</button>
          <button onClick={() => { selectDay(new Date().getDate()); setViewDate(new Date()); }} style={{ fontSize: 11, fontWeight: 700, color: "#1e40af", background: "#eff6ff", border: "1px solid #bfdbfe", cursor: "pointer", fontFamily: "inherit", padding: "4px 10px", borderRadius: 6 }}>Today</button>
        </div>
      </div>, document.body
    ) : null;
    return (
      <div style={{ position: "relative" }}>
        <div ref={triggerRef} onClick={handleOpen}
          style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", border: "1.5px solid", borderColor: open ? "#6366f1" : "#dde3ec", borderRadius: 9, background: open ? "linear-gradient(135deg,#ffffff,#f5f3ff)" : "linear-gradient(to bottom,#ffffff,#f8fafc)", cursor: "pointer", userSelect: "none", boxShadow: open ? "0 0 0 3px rgba(99,102,241,0.12),0 2px 8px rgba(0,0,0,0.08)" : "0 1px 3px rgba(0,0,0,0.06),inset 0 1px 0 rgba(255,255,255,0.9)", transition: "all 0.18s" }}>
          ...
          {value && <div onClick={e => { e.stopPropagation(); onChange(""); }} style={{ width: 16, height: 16, borderRadius: "50%", background: "#e2e8f0", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}><svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#64748b" strokeWidth="3" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg></div>}
        </div>
        {portal}
      </div>
    );
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.52)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 16, overflowY: 'auto' }}>
      <div style={{ background: 'var(--card)', borderRadius: 14, width: '100%', maxWidth: 580, maxHeight: 'calc(100vh - 32px)', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.28)', overflow: 'hidden', margin: 'auto' }}>
        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
          ...
          <div style={{ borderBottom: '1.5px solid #e2e8f0' }}>
            <div style={{ background: 'var(--bg)', borderBottom: '1px solid var(--border)', padding: '6px 16px' }}>
              <span style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px', color: 'var(--muted)' }}>Room Identity</span>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isEdit ? '1fr' : '1fr 1fr', borderBottom: '1px solid #e2e8f0' }}>
              ...
              <div style={{ padding: '10px 16px' }}>
                ...
                <CustomSelect value={form.type} onChange={val => setForm({ ...form, type: val })} placeholder="Select type" options={["General", "Isolation", "ICU", "Recovery"]} />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
              <div style={{ padding: '10px 16px', borderRight: '1px solid #e2e8f0' }}>
                ...
                <CustomSelect value={form.status} onChange={val => setForm({ ...form, status: val })} placeholder="Select status" options={["Available", "Occupied", "Quarantine", "Cleaning"]} />
              </div>
              <div style={{ padding: '10px 16px' }}>
                ...
                <CustomSelect value={form.infected ? 'yes' : 'no'} onChange={val => setForm({ ...form, infected: val === 'yes' })} placeholder="Select" options={[{ value: 'no', label: 'No' }, { value: 'yes', label: 'Yes — Isolation' }]} />
              </div>
            </div>
          </div>
          ...
          <div style={{ borderBottom: '1.5px solid #e2e8f0' }}>
            ...
            <div style={{ padding: '10px 16px', borderBottom: '1px solid #e2e8f0' }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6 }}>
                Patient Name {form.status === 'Occupied' && <span style={{ color: '#ef4444' }}>*</span>}
              </div>
               <input
            ...
            <div style={{ padding: '10px 16px' }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6 }}>Discharge / Available Date</div>
              <DatePicker
                value={form.discharge_date ? form.discharge_date.slice(0, 10) : ''}
                onChange={val => setForm({ ...form, discharge_date: val })}
                placeholder="Select discharge date"
              />
              <div style={{ fontSize: 10, color: '#94a3b8', marginTop: 6 }}>Room auto-switches to Available once this date is reached</div>
            </div>
          </div>
        ...
        {/* ── Footer ── */}
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, padding: '14px 24px', borderTop: '2px solid #e2e8f0', background: 'var(--bg)', flexShrink: 0 }}>
          <button className="btn btn-ghost" style={{ width: 'auto' }} onClick={onClose}>Cancel</button>
          <button
            className="btn btn-primary"
            style={{ width: 'auto', background: '#0f172a', borderColor: '#0f172a', display: 'inline-flex', alignItems: 'center', gap: 6, opacity: (!isFormValid() || saving) ? 0.5 : 1, cursor: (!isFormValid() || saving) ? 'not-allowed' : 'pointer' }}
            onClick={onSave} disabled={saving || !isFormValid()}
          >
            {saving ? 'Saving...' : (
              <>
...
const validateRoom = (form, isEdit, existingNumbers = []) => {
  if (!isEdit) {
    ...
    if (!/^[a-zA-Z0-9-]+$/.test(num)) return { valid: false, message: 'Room number can only contain letters, numbers, and dashes' };
    if (existingNumbers.some(n => n?.toString().trim().toLowerCase() === num.toLowerCase())) return { valid: false, message: 'A room with this number already exists' };
  }
  if (form.status === 'Occupied' && !form.patient?.toString().trim()) return { valid: false, message: 'Please assign a patient name for an occupied room' };
  if (form.patient && !/^[a-zA-Z\s'-]+$/.test(form.patient.trim())) return { valid: false, message: 'Patient name can only contain letters' };
  if (form.status === 'Occupied' && !form.diagnosis?.toString().trim()) return { valid: false, message: 'Please enter a diagnosis for an occupied room' };
  if (form.status === 'Occupied' && form.discharge_date && form.discharge_date < new Date().toISOString().split('T')[0]) return { valid: false, message: 'Discharge date cannot be in the past' };
...
const RoomAvailability = () => {
 ...
 const [formRoom, setFormRoom] = useState(null);       // add/edit modal (null = closed, false = new, obj = edit)
  ...
  const [form, setForm] = useState({ number: '', type: 'General', status: 'Available', patient: '', diagnosis: '', infected: false, discharge_date: '' });
  ...
  const fetchRooms = async () => {
    ...
    setLoading(true);
    let q = supabase.from('rooms').select('*').is('deleted_at', null).order('number');
  ...
  const fetchDeletedRooms = async () => {
    if (userLoading || !user) return;
    let q = supabase.from('rooms').select('*').not('deleted_at', 'is', null).order('deleted_at', { ascending: false });
    if (!seeAllBranches && user?.branchId) q = q.eq('branch_id', user.branchId);
    if (seeAllBranches && branchFilter) q = q.eq('branch_id', branchFilter);
    const { data, error } = await q;
    if (error) return;
    const now = Date.now();
    const expired = (data || []).filter(r => now - new Date(r.deleted_at).getTime() > THIRTY_DAYS_MS);
    if (expired.length > 0) await supabase.from('rooms').delete().in('id', expired.map(r => r.id));
    setDeletedRooms((data || []).filter(r => now - new Date(r.deleted_at).getTime() <= THIRTY_DAYS_MS));
  };
  ...
  const autoDischargeExpiredRooms = async (list) => {
    ...
    const expired = (list || []).filter(r =>
      r.status === 'Occupied' && (
        ...
        !r.patient || !r.patient.toString().trim()
      )
    );
    if (expired.length === 0) return;
    for (const room of expired) {
      const reason = (!room.patient || !room.patient.toString().trim())
        ? 'no patient assigned'
      ...
      await supabase.from('rooms').update({
        status: 'Available', patient: '', diagnosis: '', discharge_date: null,
      }).eq('id', room.id);
      logActivity(user, 'Auto-released room', `Room ${room.number} automatically set to Available (${reason})`);
    }
  };

  useEffect(() => {
    if (user) logActivity(user, 'Viewed room availability', 'Opened room availability');
  }, []);
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel('rooms-availability-rt')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'rooms' }, () => { fetchRooms(); fetchDeletedRooms(); })
      .subscribe();
    // Re-check discharge dates every minute even if no other change happens
    const dischargeTimer = setInterval(() => fetchRooms(), 60000);
    return () => { supabase.removeChannel(ch); clearInterval(dischargeTimer); };
  }, [user, userLoading, seeAllBranches, branchFilter]);

   const openCreate = () => {
    setForm({ number: '', type: 'General', status: 'Available', patient: '', diagnosis: '', infected: false, discharge_date: '' });
  ...
  const hasUnsavedRoomEdits = () => {
    ...
    const norm = (f) => JSON.stringify({
      number: f.number || '', type: f.type || 'General', status: f.status || 'Available',
      patient: f.patient || '', diagnosis: f.diagnosis || '', infected: !!f.infected,
      discharge_date: f.discharge_date ? f.discharge_date.toString().slice(0, 10) : '',
    });
  ...
  const attemptCloseFormRoom = () => {
    if (hasUnsavedRoomEdits()) {
      setConfirm({
        ...
        message: "You have unsaved changes to this room's record. Do you want to discard them?",
        confirmText: 'Discard',
        confirmColor: '#dc2626',
        onConfirm: () => { setConfirm({ show: false }); setFormRoom(null); setFormOriginal(null); },
        onCancel: () => setConfirm({ show: false }),
      });
    } else {
      setFormRoom(null); setFormOriginal(null);
    }
  };

   const saveRoom = async () => {
    if (saving) return;
    const check = validateRoom(form, !!formRoom, rooms.map(r => r.number));
    ...
    const payload = {
      ...
      discharge_date: form.status === 'Available' ? null : (form.discharge_date || null),
    };
    if (formRoom) {
      // Edit
      const { error } = await supabase.from('rooms').update(payload).eq('id', formRoom.id);
      setSaving(false);
      ...
      const { error } = await supabase.from('rooms').insert([{
        ...payload, number: form.number, branch_id: user?.branchId ?? null,
      }]);
      setSaving(false);
      if (error) { showToast('Error: ' + error.message, 'error'); return; }
 ...
 const deleteRoom = (room) => {
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        const { error } = await supabase.from('rooms').update({ deleted_at: new Date().toISOString() }).eq('id', room.id);
        if (error) { showToast('Error: ' + error.message, 'error'); return; }
        logActivity(user, 'Deleted room', `Moved to Recently Deleted: Room ${room.number}`);
        setViewRoom(null);
        showToast(`Room ${room.number} moved to Recently Deleted`, 'info');
        fetchRooms(); fetchDeletedRooms();
      },
      onCancel: () => setConfirm({ show: false }),
    });
  };
  ...
  const restoreRoom = (room) => {
    setConfirm({
      ...
      onConfirm: async () => {
        setConfirm({ show: false });
        const { error } = await supabase.from('rooms').update({ deleted_at: null }).eq('id', room.id);
        if (error) { showToast('Error: ' + error.message, 'error'); return; }
  ...
  const permanentlyDeleteRoom = (room) => {
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        if (error) { showToast('Error: ' + error.message, 'error'); return; }
        logActivity(user, 'Permanently deleted room', `Removed: Room ${room.number}`);
    ...
    <Layout>
      ...
      <div className="topbar room-topbar" style={{ position: "fixed", top: 68, left: "var(--current-sidebar-w, 62px)", right: 0, zIndex: 40, background: "#fff" }}>
       ...
       <div className="topbar-actions">
          {seeAllBranches && (
            <div style={{ width: 180 }}>
              <CustomSelect value={branchFilter} onChange={val => setBranchFilter(val)} placeholder="All Branches" accent="#7c3aed" options={branches.map(b => ({ value: b.id, label: b.name }))} />
            </div>
          )}
          <button
          ...
          >
            ...
            <span className="fab-tooltip" style={{ opacity: 0, transform: 'translateX(8px)', transition: 'opacity 0.2s ease, transform 0.2s ease', background: 'linear-gradient(135deg,#0f172a,#1e3a8a)', color: '#fff', fontSize: 12, fontWeight: 700, padding: '8px 14px', borderRadius: 10, whiteSpace: 'nowrap', pointerEvents: 'none', boxShadow: '0 8px 24px rgba(30,58,138,0.35)', border: '1px solid rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', gap: 7, position: 'relative' }}>
              <span style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.2 }}>
                <span style={{ fontSize: 12, fontWeight: 800, color: '#fff' }}>Add Room</span>
                <span style={{ fontSize: 10, fontWeight: 500, color: 'rgba(255,255,255,0.55)' }}>Register new room</span>
              </span>
            ...
            <button onClick={openCreate} className="fab-btn" style={{ width: 56, height: 56, borderRadius: '50%', background: 'linear-gradient(135deg,#1e3a8a,#3b82f6)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 20px rgba(30,58,138,0.4)', transition: 'transform 0.2s, box-shadow 0.2s', flexShrink: 0 }}>
      ...
      <div className="content">
          ...
          {loading
            ...
            : [
              ...
              <div key={i} className={`stat-card-v2 ${sc.color} fade-in`}
                onClick={() => setFilter(f => f === sc.f ? 'All' : sc.f)} style={{ animationDelay: `${i * 0.1}s` }}>
        ...
        {/* Filter tabs */}
        <div className="fade-in" style={{ display: 'flex', gap: 6, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
          {FILTER_TABS.map(f => (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.52)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 16 }}>
          <div style={{ background: 'var(--card)', borderRadius: 14, width: '100%', maxWidth: 560, maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.28)' }}>
            <div style={{ padding: '18px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: 'var(--text)' }}>Recently Deleted Rooms</h3>
                <p style={{ margin: 0, fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>Rooms are permanently removed 30 days after deletion.</p>
              </div>
            ...
            <div style={{ padding: 20, overflowY: 'auto', flex: 1 }}>
              {deletedRooms.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--muted)' }}>
                  <p style={{ fontSize: 13, margin: 0 }}>No recently deleted rooms.</p>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {deletedRooms.map(room => {
                    const daysLeft = Math.max(0, 30 - Math.floor((Date.now() - new Date(room.deleted_at).getTime()) / (24 * 60 * 60 * 1000)));
                    return (
                      <div key={room.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px', flexWrap: 'wrap' }}>
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--text)' }}>Room {room.number}</div>
                          <div style={{ fontSize: 11, color: 'var(--muted)' }}>{room.type || 'General'}{room.patient ? ` · Was housing: ${room.patient}` : ''}</div>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button onClick={() => restoreRoom(room)} style={{ background: '#f0fdf4', border: '1.5px solid #86efac', color: '#166534', borderRadius: 8, padding: '7px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Restore</button>
                          <button onClick={() => permanentlyDeleteRoom(room)} style={{ background: '#fef2f2', border: '1.5px solid #fca5a5', color: '#dc2626', borderRadius: 8, padding: '7px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Delete Now</button>
                        </div>
                      </div>
                    );
                  })}
