...
const CustomSelect = ({ value, onChange, options, placeholder = "—", accent = "#dc2626", disabled = false }) => {
  ...
  const portal = open && typeof document !== "undefined"
    ? ReactDOM.createPortal(
      <div ref={ref} style={{ position: "absolute", top: dropPos.top, left: dropPos.left, width: dropPos.width, background: "var(--card)", borderRadius: 12, zIndex: 99999, boxShadow: "0 16px 40px rgba(0,0,0,0.13), 0 4px 12px rgba(0,0,0,0.07), 0 0 0 1px rgba(0,0,0,0.06)", border: "1.5px solid #fecaca", maxHeight: 260, overflowY: "auto", padding: "5px" }}>
        {options.map((opt, i) => {
          ...
          return (
            <div key={i} onClick={() => { if (!isDisabled) { onChange(optVal); setOpen(false); } }}
              style={{ padding: "8px 10px", fontSize: 13, fontWeight: isSelected ? 700 : 500, color: isDisabled ? "#cbd5e1" : isSelected ? accent : "var(--text)", cursor: isDisabled ? "not-allowed" : "pointer", transition: "background 0.12s, color 0.12s", background: isSelected ? `${accent}12` : "transparent", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, opacity: isDisabled ? 0.5 : 1, marginBottom: 1 }}
              onMouseEnter={e => { if (!isSelected && !isDisabled) e.currentTarget.style.background = "#fef2f2"; }}
              onMouseLeave={e => { if (!isSelected) e.currentTarget.style.background = "transparent"; }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                <div style={{ width: 6, height: 6, borderRadius: "50%", flexShrink: 0, background: isSelected ? accent : "transparent", border: `1.5px solid ${isSelected ? accent : isDisabled ? "#e2e8f0" : "#fca5a5"}`, transition: "background 0.15s, border-color 0.15s" }} />
  ...
  return (
    <div style={{ position: "relative", width: "100%" }}>
      <div ref={triggerRef} onClick={handleOpen}
        style={{ width: "100%", padding: "9px 34px 9px 12px", border: "1.5px solid", borderRadius: 9, background: disabled ? "#f8fafc" : open ? "linear-gradient(135deg, #ffffff 0%, #fef2f2 100%)" : "linear-gradient(to bottom, #ffffff 0%, #fefefe 100%)", fontSize: 13, fontWeight: 600, color: disabled ? "#cbd5e1" : value ? "var(--text)" : "#b0bac9", cursor: disabled ? "not-allowed" : "pointer", userSelect: "none", boxSizing: "border-box", boxShadow: open ? `0 0 0 3px ${accent}22, 0 2px 8px rgba(0,0,0,0.08)` : "0 1px 3px rgba(0,0,0,0.06), inset 0 1px 0 rgba(255,255,255,0.9)", borderColor: open ? accent : "#fecaca", transition: "border-color 0.18s, box-shadow 0.18s, background 0.18s", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, position: "relative", minHeight: 38 }}
        onMouseEnter={e => { if (!open && !disabled) { e.currentTarget.style.borderColor = "#fca5a5"; e.currentTarget.style.boxShadow = "0 2px 8px rgba(220,38,38,0.10), inset 0 1px 0 rgba(255,255,255,0.9)"; } }}
        onMouseLeave={e => { if (!open && !disabled) { e.currentTarget.style.borderColor = "#fecaca"; e.currentTarget.style.boxShadow = "0 1px 3px rgba(0,0,0,0.06), inset 0 1px 0 rgba(255,255,255,0.9)"; } }}>
        <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{label}</span>
        <div style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", width: 20, height: 20, borderRadius: 6, background: open ? accent : "#fef2f2", display: "flex", alignItems: "center", justifyContent: "center", transition: "background 0.18s", flexShrink: 0 }}>
          <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke={open ? "#fff" : "#dc2626"} strokeWidth="3" strokeLinecap="round" style={{ transition: "transform 0.2s, stroke 0.18s", transform: open ? "rotate(180deg)" : "rotate(0deg)" }}>
...
const CITIES_BY_PROVINCE = {
  ...
  "Ilocos Sur": ["Vigan City", "Candon City", "Narvacan", "Santa Maria", "Cabugao"],
  "La Union": ["San Fernando City", "Bauang", "Agoo", "Naguilian", "Bacnotan"],
  "Pangasinan": ["Dagupan City", "San Carlos City", "Urdaneta City", "Alaminos City", "Lingayen", "Binmaley"],
  ...
  "Bulacan": ["Malolos City", "Meycauayan City", "San Jose del Monte City", "Baliuag", "Marilao", "Bocaue"],
  "Nueva Ecija": ["Cabanatuan City", "Gapan City", "Palayan City", "San Jose City", "Muñoz City"],
  "Pampanga": ["Angeles City", "San Fernando City", "Mabalacat City", "Magalang", "Apalit", "Guagua"],
  "Tarlac": ["Tarlac City", "Capas", "Concepcion", "Paniqui", "Camiling"],
  "Zambales": ["Olongapo City", "Iba", "Subic", "Botolan", "Castillejos"],
  // ── Region IV-A – CALABARZON ──
  "Batangas": ["Batangas City", "Lipa City", "Tanauan City", "Santo Tomas", "Bauan", "Nasugbu"],
  "Cavite": ["Bacoor City", "Dasmariñas City", "Imus City", "Cavite City", "Tagaytay City", "General Trias City"],
  "Laguna": ["Calamba City", "Santa Rosa City", "San Pablo City", "Biñan City", "Cabuyao City", "Los Baños"],
  "Quezon": ["Lucena City", "Tayabas City", "Candelaria", "Sariaya", "Lucban"],
  "Rizal": ["Antipolo City", "Cainta", "Taytay", "Angono", "Binangonan", "Rodriguez"],
  ...
  "Palawan": ["Puerto Princesa City", "Coron", "El Nido", "Narra", "Brooke's Point"],
  ...
  "Negros Occidental": ["Bacolod City", "Bago City", "Silay City", "Talisay City", "Kabankalan City"],
  // ── Region VII – Central Visayas ──
  "Bohol": ["Tagbilaran City", "Panglao", "Tubigon", "Ubay"],
  "Cebu": ["Cebu City", "Mandaue City", "Lapu-Lapu City", "Talisay City", "Toledo City", "Danao City"],
...
const BARANGAYS_BY_CITY = {
  "Angeles City": [
    ...
    "Malabañas", "Margot", "Mining", "Ninoy Aquino (Marisol)", "Pampang", "Pandan",
    "Pulungbulu", "Pulung Cacutud", "Pulung Maragul", "Salapungan", "San Jose", "San Nicolas",
    "Santa Teresita", "Santa Trinidad", "Santo Cristo", "Santo Domingo", "Santo Rosario",
    "Sapalibutad", "Sapangbato", "Tabun", "Virgen Delos Remedios",
  ],
  "San Fernando City": [
    "Alasas", "Baliti", "Bulaon", "Calulut", "Del Carmen", "Del Pilar", "Del Rosario",
    "Dela Paz Norte", "Dela Paz Sur", "Dolores", "Juliana", "Lara", "Lourdes", "Magliman",
    "Maimpis", "Malino", "Malpitic", "Pandaras", "Panipuan", "Pulung Bulu", "Quebiauan",
    "Saguin", "San Agustin", "San Felipe", "San Isidro", "San Jose", "San Juan",
    "San Nicolas", "San Pedro", "Santa Lucia", "Santa Teresita", "Santo Niño",
    "Santo Rosario (Poblacion)", "Sindalan", "Telabastagan",
  ],
  "Mabalacat City": [
    ...
    "San Joaquin", "Santa Ines", "Santa Maria", "Santo Rosario", "Sapang Balen",
    "Sapang Biabas", "Tabun",
  ],
  "Tarlac City": [
    "Aguso", "Alvindia", "Amucao", "Armenia", "Asturias", "Atioc", "Balanti", "Balete",
    "Balibago I", "Balibago II", "Balingcanaway", "Banaba", "Bantog", "Baras-baras",
    "Batang-batang", "Binauganan", "Bora", "Buenavista", "Buhilit", "Burot", "Calingcuan",
    "Capehan", "Carangian", "Care", "Central", "Culipat", "Cut-cut I", "Cut-cut II",
    "Dalayap", "Dela Paz", "Dolores", "Laoang", "Ligtasan", "Lourdes", "Mabini", "Maligaya",
    "Maliwalo", "Mapalacsiao", "Mapalad", "Matatalaib", "Paraiso", "Poblacion", "Salapungan",
    "San Carlos", "San Francisco", "San Isidro", "San Jose", "San Jose de Urquico",
    "San Juan Bautista", "San Juan de Mata", "San Luis", "San Manuel", "San Miguel",
    "San Nicolas", "San Pablo", "San Pascual", "San Rafael", "San Roque", "San Sebastian",
    "San Vicente", "Santa Cruz", "Santa Maria", "Santo Cristo", "Santo Domingo", "Santo Niño",
    "Sapang Maragul", "Sapang Tagalog", "Sepung Calzada", "Sinait", "Suizo", "Tariji",
    "Tibag", "Tibagan", "Trinidad", "Ungot", "Villa Bacolor",
  ],
};
...
const BARANGAY_CITY_OPTIONS = Object.entries(BARANGAYS_BY_CITY).flatMap(([city, brgys]) =>
...
const BRANCH_COORDS = {
  ...
  "Tarlac": { lat: 15.4755, lng: 120.5960 },
...
const CITY_COORDS = {
  ...
  "Tarlac City": { lat: 15.4755, lng: 120.5960 },
...
const PROVINCE_COORDS = {
  "Metro Manila": { lat: 14.5995, lng: 120.9842 }, "Ilocos Norte": { lat: 18.1647, lng: 120.7116 },
  "Ilocos Sur": { lat: 17.5747, lng: 120.3869 }, "La Union": { lat: 16.6159, lng: 120.3210 },
  "Pangasinan": { lat: 15.8949, lng: 120.2863 }, "Batanes": { lat: 20.4487, lng: 121.9702 },
  "Cagayan": { lat: 17.6132, lng: 121.7270 }, "Isabela": { lat: 16.9754, lng: 121.8107 },
  "Nueva Vizcaya": { lat: 16.3301, lng: 121.1710 }, "Quirino": { lat: 16.2333, lng: 121.5667 },
  "Aurora": { lat: 15.7333, lng: 121.5667 }, "Bataan": { lat: 14.6417, lng: 120.4818 },
  "Bulacan": { lat: 14.7943, lng: 120.8794 }, "Nueva Ecija": { lat: 15.5784, lng: 120.9700 },
  "Pampanga": { lat: 15.0794, lng: 120.6200 }, "Tarlac": { lat: 15.4755, lng: 120.5960 },
  "Zambales": { lat: 15.5082, lng: 120.0691 }, "Batangas": { lat: 13.7565, lng: 121.0583 },
  "Cavite": { lat: 14.2456, lng: 120.8786 }, "Laguna": { lat: 14.1699, lng: 121.2439 },
  "Quezon": { lat: 13.9314, lng: 121.9310 }, "Rizal": { lat: 14.6255, lng: 121.3086 },
  "Marinduque": { lat: 13.4771, lng: 121.9032 }, "Occidental Mindoro": { lat: 13.1024, lng: 120.7651 },
  "Oriental Mindoro": { lat: 13.0565, lng: 121.4069 }, "Palawan": { lat: 9.8349, lng: 118.7384 },
  "Romblon": { lat: 12.5778, lng: 122.2695 }, "Albay": { lat: 13.1391, lng: 123.7437 },
  "Camarines Norte": { lat: 14.1389, lng: 122.7632 }, "Camarines Sur": { lat: 13.6252, lng: 123.1829 },
  "Catanduanes": { lat: 13.7089, lng: 124.2422 }, "Masbate": { lat: 12.3686, lng: 123.6151 },
  "Sorsogon": { lat: 12.9743, lng: 124.0067 }, "Aklan": { lat: 11.8166, lng: 122.0942 },
  "Antique": { lat: 10.9995, lng: 122.0995 }, "Capiz": { lat: 11.3889, lng: 122.6277 },
  "Guimaras": { lat: 10.5928, lng: 122.6325 }, "Iloilo": { lat: 10.7202, lng: 122.5621 },
  "Negros Occidental": { lat: 10.6713, lng: 122.9511 }, "Bohol": { lat: 9.8500, lng: 124.1435 },
  "Cebu": { lat: 10.3157, lng: 123.8854 }, "Negros Oriental": { lat: 9.3103, lng: 123.3040 },
  "Siquijor": { lat: 9.2168, lng: 123.5155 }, "Biliran": { lat: 11.5836, lng: 124.4649 },
  "Eastern Samar": { lat: 11.6112, lng: 125.4966 }, "Leyte": { lat: 11.2543, lng: 124.9530 },
  "Northern Samar": { lat: 12.4644, lng: 124.6262 }, "Samar": { lat: 11.7761, lng: 124.9611 },
  "Southern Leyte": { lat: 10.3830, lng: 125.0810 }, "Zamboanga del Norte": { lat: 8.1527, lng: 123.2577 },
  "Zamboanga del Sur": { lat: 7.8383, lng: 123.4362 }, "Zamboanga Sibugay": { lat: 7.5222, lng: 122.8340 },
  "Bukidnon": { lat: 8.0515, lng: 125.0985 }, "Camiguin": { lat: 9.1735, lng: 124.7300 },
  "Lanao del Norte": { lat: 8.1156, lng: 123.9377 }, "Misamis Occidental": { lat: 8.5083, lng: 123.7833 },
  "Misamis Oriental": { lat: 8.5046, lng: 124.6220 }, "Davao de Oro": { lat: 7.5000, lng: 126.0500 },
  "Davao del Norte": { lat: 7.4479, lng: 125.8258 }, "Davao del Sur": { lat: 6.7656, lng: 125.3284 },
  "Davao Occidental": { lat: 6.2000, lng: 125.6167 }, "Davao Oriental": { lat: 7.3172, lng: 126.5420 },
  "Davao City": { lat: 7.1907, lng: 125.4553 }, "Cotabato": { lat: 7.2231, lng: 124.2452 },
  "Sarangani": { lat: 5.9500, lng: 125.2000 }, "South Cotabato": { lat: 6.3350, lng: 124.8514 },
  "Sultan Kudarat": { lat: 6.5069, lng: 124.4238 }, "Agusan del Norte": { lat: 8.9456, lng: 125.5319 },
  "Agusan del Sur": { lat: 8.3667, lng: 125.9333 }, "Dinagat Islands": { lat: 10.1281, lng: 125.6086 },
  "Surigao del Norte": { lat: 9.7897, lng: 125.4966 }, "Surigao del Sur": { lat: 8.7500, lng: 126.1500 },
  "Abra": { lat: 17.5951, lng: 120.7983 }, "Apayao": { lat: 18.0177, lng: 121.1710 },
  "Benguet": { lat: 16.4023, lng: 120.5960 }, "Ifugao": { lat: 16.8300, lng: 121.1710 },
  "Kalinga": { lat: 17.4766, lng: 121.3557 }, "Mountain Province": { lat: 17.0417, lng: 121.1084 },
  "Basilan": { lat: 6.4297, lng: 121.9689 }, "Lanao del Sur": { lat: 7.9750, lng: 124.2367 },
  "Maguindanao del Norte": { lat: 7.0000, lng: 124.4167 }, "Maguindanao del Sur": { lat: 6.9000, lng: 124.4500 },
  "Sulu": { lat: 6.0500, lng: 121.0000 }, "Tawi-Tawi": { lat: 5.1339, lng: 119.9556 },
};

const MAX_BRANCH_DISTANCE_KM = 30;

const getLocationCoords = (province, city) => CITY_COORDS[city] || PROVINCE_COORDS[province] || null;
...
const STATUS_COLORS = {
  ...
  responding: { bg: "rgba(147,197,253,0.15)", border: "#93c5fd", text: "#60a5fa" },
  resolved: { bg: "rgba(134,239,172,0.15)", border: "#86efac", text: "#4ade80" },
};
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 11, padding: '14px 14px 12px' }}>
        <div style={{ width: 32, height: 32, borderRadius: 8, flexShrink: 0, background: '#f0fdf4', color: '#16a34a', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 1 }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12" /></svg>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ marginBottom: 5 }}>
            <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#166534', background: '#dcfce7', borderRadius: 4, padding: '2px 7px' }}>
              {variant === 'responding' ? 'Update' : 'Success'}
            </span>
          </div>
          <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: 'var(--text)', lineHeight: 1.4 }}>
            {variant === 'responding' ? '🚨 Help is on the way!' : (guestMode ? 'Emergency report submitted!' : 'Emergency alert sent!')}
          </p>
          <p style={{ margin: '3px 0 0', fontSize: 12, color: 'var(--muted)', lineHeight: 1.4 }}>
            {variant === 'responding'
              ? 'Our team is now responding to your emergency. Please stay calm and keep your phone line open.'
              : (guestMode ? 'Our team has been notified and will respond shortly.' : 'Staff and branches have been notified.')}
          </p>
        </div>
        <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'var(--muted)', fontSize: 16, cursor: 'pointer', lineHeight: 1, padding: '2px 4px', flexShrink: 0, pointerEvents: 'all' }}>✕</button>
      </div>
      <div style={{ height: 2, background: '#22c55e22' }}>
        <div style={{ height: '100%', background: '#22c55e', opacity: 0.6, width: show ? '0%' : '100%', transition: show ? 'width 3s linear' : 'none' }} />
      </div>
    </div>
  );
};
...
const ToastStack = ({ toasts, onClose }) => {
  ...
  return (
    <div style={{
      ...
      {visible.map(t => (
        <ToastItem key={t.id} show={t.show} guestMode={t.guestMode} variant={t.variant} onClose={() => onClose(t.id)} />
      ))}
    </div>
  );
};
    ...
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0, marginLeft: 8 }}>
         ...
         <span style={{ fontSize: 11, color: "var(--muted)", whiteSpace: "nowrap" }}>
            {new Date(a.created_at).toLocaleString('en', { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
          </span>
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 8, flexWrap: "wrap" }}>
        <span style={{ fontSize: 10, color: "#94a3b8", display: "inline-flex", alignItems: "center", gap: 3 }}>
          ...
          Reported: {new Date(a.created_at).toLocaleString('en', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
        </span>
        {a.updated_at && a.status !== "pending" && (
          <span style={{ fontSize: 10, color: "#94a3b8" }}>
            · Last updated: {new Date(a.updated_at).toLocaleString('en', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
          </span>
        )}
      </div>

     {a.description && (
        <p style={{ fontSize: 12, color: "var(--muted)", margin: "0 0 8px", lineHeight: 1.5 }}>{a.description}</p>
      )}

      {(a.guest_contact || a.guest_address) && !a.guest_full_name && (
        <p style={{ fontSize: 11, color: "var(--muted)", margin: "0 0 8px" }}>
          {a.guest_contact ? <span>Contact: <span style={{ fontWeight: 600, color: "var(--text)" }}>{a.guest_contact}</span></span> : ""}
          {a.guest_contact && a.guest_address ? " · " : ""}
          {a.guest_address ? <span>Location: <span style={{ fontWeight: 600, color: "var(--text)" }}>{a.guest_address}</span></span> : ""}
        </p>
      )}
      ...
      <p style={{ fontSize: 11, color: "var(--muted)", margin: 0, borderTop: `1px solid var(--border)`, paddingTop: 8, marginTop: 6 }}>
...
// ─── Emergency Form ───────────────────────────────────────────────────────────
const EmergencyForm = memo(({ guestMode, sending, onSend, onExit, userBranch }) => {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          const map = {};
          data.forEach(r => { map[r.branch] = r.available; });
  ...
  const set = (key, val) => {
    ...
    else if (branchAvailability[form.branch] === false) errs.branch = "This branch is currently unavailable. Please select another branch.";
    if (!form.type) errs.type = "Please select the emergency type.";
    else if (form.type === OTHER_TYPE && !form.customType.trim()) errs.type = "Please describe the emergency.";
    if (guestMode) {
      ...
      if (!form.guest_barangay) errs.guest_address = "Please select a barangay.";
      else if (!form.guest_province) errs.guest_address = "Please select a province.";
      else if (!form.guest_city) errs.guest_address = "Please select a city.";
      else if (form.branch && isLocationTooFar(form.branch, form.guest_province, form.guest_city)) {        const d = Math.round(distanceToBranchKm(form.branch, form.guest_province, form.guest_city));
      ...
      if (!form.location_barangay) errs.location = "Please select a barangay.";
      else if (!form.location_province) errs.location = "Please select a province.";
      else if (!form.location_city) errs.location = "Please select a city.";
      else if (form.branch && isLocationTooFar(form.branch, form.location_province, form.location_city)) {        const d = Math.round(distanceToBranchKm(form.branch, form.location_province, form.location_city));
  ...
  const distanceOk = guestMode
    ...
    : !isLocationTooFar(form.branch, form.location_province, form.location_city);

  const canSend = !!(
    form.branch &&
    form.type && (form.type !== OTHER_TYPE || form.customType.trim()) &&
    (guestMode
      ? form.guest_full_name.trim() && form.guest_contact.trim() && form.guest_barangay && form.guest_province && form.guest_city && form.patient_name.trim()
      : form.contact_number.trim() && form.location_barangay && form.location_province && form.location_city) &&
    distanceOk
  );
  ...
  const handleSend = useCallback(async () => {
    ...
    const finalForm = {
      ...
      location: [form.location_street.trim(), form.location_barangay, form.location_city, form.location_province].filter(Boolean).join(", "),
      guest_address: [form.guest_street.trim(), form.guest_barangay, form.guest_city, form.guest_province].filter(Boolean).join(", "),
    };
    const result = await onSend(finalForm);
    if (result?.success) {
     setForm({ type: "", customType: "", contact_number: "", location_province: "", location_city: "", location_barangay: "", location_street: "", branch: userBranch || "", guest_full_name: "", guest_contact: "", guest_province: "", guest_city: "", guest_barangay: "", guest_street: "", patient_name: "" });      setErrors({});
    }
  }, [form, onSend]);
 ...
 return (
    ...
    <div style={{
      ...
      <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 4, color: "#dc2626", display: "flex", alignItems: "center", gap: 7 }}>
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#dc2626" strokeWidth="2.5" strokeLinecap="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
            <div>
              ...
              <input type="text" placeholder="e.g. Juan dela Cruz" value={form.guest_full_name} onChange={e => set("guest_full_name", sanitizeName(e.target.value))} style={inp(errors.guest_full_name)} />
            ...
            <div>
              ...
              <input type="tel" inputMode="numeric" maxLength={11} placeholder="e.g. 09XXXXXXXXX" value={form.guest_contact} onChange={e => set("guest_contact", sanitizeContact(e.target.value))} style={inp(errors.guest_contact)} />
              {form.guest_contact && form.guest_contact.length !== 11 && <p style={errStyle}>Contact number must be 11 digits.</p>}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)", marginBottom: 4, display: "block" }}>Province</label>
                  <CustomSelect
                    value={form.guest_province}
                    onChange={val => {
                      set("guest_province", val);
                      if (CITY_TO_PROVINCE[form.guest_city] !== val) { set("guest_city", ""); set("guest_barangay", ""); }
                    }}
                ...
                <div>
                  <label style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)", marginBottom: 4, display: "block" }}>City</label>
                  <CustomSelect
                    value={form.guest_city}
                    onChange={val => {
                      set("guest_city", val); set("guest_province", CITY_TO_PROVINCE[val] || "");
                      if (!BARANGAYS_BY_CITY[val]?.includes(form.guest_barangay)) set("guest_barangay", "");
                    }}
                    options={(form.guest_province ? [...(CITIES_BY_PROVINCE[form.guest_province] || [])].sort() : ALL_CITIES).map(c => ({ value: c, label: c }))}
                    placeholder="— City —"
                    accent="#dc2626"
              ...
              <div style={{ marginBottom: 8 }}>
                <label style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)", marginBottom: 4, display: "block" }}>Barangay</label>
                <CustomSelect
                  value={form.guest_city ? form.guest_barangay : (form.guest_barangay ? `${form.guest_barangay}||${form.guest_city}` : "")}
                  onChange={val => {
                    if (form.guest_city) {
                      set("guest_barangay", val);
                    } else {
                      const [brgy, city] = val.split("||");
                      set("guest_barangay", brgy); set("guest_city", city); set("guest_province", CITY_TO_PROVINCE[city] || "");
                    }
                  }}
                  options={form.guest_city
                    ? (BARANGAYS_BY_CITY[form.guest_city] || []).map(b => ({ value: b, label: b }))
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)", marginBottom: 4, display: "block" }}>Street</label>
                <input
                  type="text"
            ...
            <div>
              ...
              <input type="text" placeholder="e.g. Brownie" value={form.patient_name} onChange={e => set("patient_name", sanitizeName(e.target.value))} style={inp(errors.patient_name)} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
         <div>
          ...
          <CustomSelect
            ...
            accent="#dc2626"
            options={EMERGENCY_TYPES.map(t => ({ value: t, label: t === OTHER_TYPE ? "Other (describe emergency)" : t }))}
          />          
        ...
        <div>
          ...
          <CustomSelect
            ...
            options={BRANCHES.map(b => {
              ...
              return { value: b, label: isAvailable ? b : `${b} — Unavailable`, disabled: !isAvailable };
            })}
          />
          {branchAvailability[form.branch] === false && (
            <p style={{ fontSize: 11, color: '#dc2626', marginTop: 4, display: 'flex', alignItems: 'center', gap: 5 }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#dc2626" strokeWidth="2.5" strokeLinecap="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>
      ...
      {!guestMode && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
          <div>
            <label style={{ ...labelStyle, whiteSpace: "nowrap" }}>Contact Number <span style={{ color: "#dc2626" }}>*</span></label>
            <input type="tel" inputMode="numeric" maxLength={11} placeholder="e.g. 09XXXXXXXXX" value={form.contact_number} onChange={e => set("contact_number", sanitizeContact(e.target.value))} style={inp(errors.contact_number)} />
            {form.contact_number && form.contact_number.length !== 11 && <p style={errStyle}>Contact number must be 11 digits.</p>}
            {errors.contact_number && <p style={errStyle}>{errors.contact_number}</p>}
          </div>
           <div>
             <label style={{ ...labelStyle, whiteSpace: "nowrap" }}>Location of Emergency <span style={{ color: "#dc2626" }}>*</span></label>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)", marginBottom: 4, display: "block" }}>Province</label>
                <CustomSelect
                  ...
                  onChange={val => {
                    ...
                    if (CITY_TO_PROVINCE[form.location_city] !== val) { set("location_city", ""); set("location_barangay", ""); }
                  }}
                  options={PROVINCES.map(p => ({ value: p, label: p }))}
                  placeholder="— Province —"
                  accent="#dc2626"
              ...
              <div>
                ...
                <CustomSelect
                  ...
                  onChange={val => {
                    set("location_city", val); set("location_province", CITY_TO_PROVINCE[val] || "");
                    if (!BARANGAYS_BY_CITY[val]?.includes(form.location_barangay)) set("location_barangay", "");
                  }}
                  options={(form.location_province ? [...(CITIES_BY_PROVINCE[form.location_province] || [])].sort() : ALL_CITIES).map(c => ({ value: c, label: c }))}
                  placeholder="— City —"
                  accent="#dc2626"
            ...
            <div style={{ marginBottom: 8 }}>
              <label style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)", marginBottom: 4, display: "block" }}>Barangay</label>
              <CustomSelect
                value={form.location_city ? form.location_barangay : (form.location_barangay ? `${form.location_barangay}||${form.location_city}` : "")}
                onChange={val => {
                  if (form.location_city) {
                    set("location_barangay", val);
                  } else {
                    const [brgy, city] = val.split("||");
                    set("location_barangay", brgy); set("location_city", city); set("location_province", CITY_TO_PROVINCE[city] || "");
                  }
                }}
                options={form.location_city
                  ? (BARANGAYS_BY_CITY[form.location_city] || []).map(b => ({ value: b, label: b }))
                  : BARANGAY_CITY_OPTIONS}
                placeholder="— Barangay —"
                accent="#dc2626"
    ...
    <div style={{ maxWidth: 320, margin: "16px auto 0" }}>
      ...
      {!distanceOk && form.branch && (
        <div style={{
          ...
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#991b1b" strokeWidth="2.5" strokeLinecap="round" style={{ flexShrink: 0, marginTop: 1 }}><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>
          <span>
            {guestMode
...
// ─── Admin View ───────────────────────────────────────────────────────────────
const AdminView = ({ alerts, loading, onRefresh, onUpdateStatus, userBranch, branchAvailable, onToggleAvailability }) => {
  const visibleAlerts = userBranch ? alerts.filter(a => normalizeBranchName(a.branch) === normalizeBranchName(userBranch)) : alerts;
    ...
    <div className="emg-page">
      <div className="emergency-topbar emg-topbar-pos emg-topbar-card">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          >
            <span style={{ width: 8, height: 8, borderRadius: "50%", background: branchAvailable ? "#16a34a" : "#dc2626", display: "inline-block" }} />
            {branchAvailable ? "Branch Available" : "Branch Unavailable"}
          </button>
          <button onClick={onRefresh} style={{ fontSize: 12, fontWeight: 700, padding: "7px 14px", borderRadius: 8, border: "1.5px solid var(--border)", background: "var(--card)", cursor: "pointer", fontFamily: "inherit", color: "var(--text)", display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <div className="emg-content">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16, marginBottom: 24 }}>
          ...
          {/* ── Pending panel ── */}
          <div style={{ background: "var(--card)", borderRadius: "var(--radius-lg)", border: "1px solid var(--border)", padding: 24, boxShadow: "var(--shadow)" }}>
            ...
            {loading ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12, maxHeight: 420, overflowY: "auto" }}>
                {pending.map(a => <AlertCard key={a.id + a.status} a={a} showActions={true} onUpdateStatus={onUpdateStatus} />)}
              </div>
            )}
          </div>
...
// ─── Staff View ───────────────────────────────────────────────────────────────
const StaffView = ({ alerts, loading, sending, onSend, onExit, onUpdateStatus, userBranch, branchAvailable, onToggleAvailability }) => {
      ...
      <div className="emergency-topbar emg-topbar-pos emg-topbar-white">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(80px, 1fr))", gap: 10, marginBottom: 24 }}>
          {[
            ...
            { label: "Responding", value: branchAlerts.filter(a => a.status === "responding").length, color: "#1d4ed8", bg: "#dbeafe", border: "#93c5fd" },
            { label: "Resolved", value: branchAlerts.filter(a => a.status === "resolved").length, color: "#16a34a", bg: "#f0fdf4", border: "#86efac" },
          ].map((s, i) => (
            <div key={s.label} className="fade-in" style={{ background: s.bg, border: `1px solid ${s.border}`, borderRadius: 12, padding: "18px 20px", position: "relative", overflow: "hidden", animationDelay: `${i * 0.1}s` }}>
              <div style={{ position: "absolute", top: 0, left: 0, width: 3, height: "100%", background: s.color, borderRadius: "12px 0 0 12px" }} />
              <div style={{ fontSize: 30, fontWeight: 800, color: s.color, lineHeight: 1, marginBottom: 4 }}>{s.value}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16, marginBottom: 24 }}>
          <EmergencyForm guestMode={false} sending={sending} onSend={onSend} onExit={onExit} userBranch={userBranch} />          <div style={{ background: "var(--card)", borderRadius: "var(--radius-lg)", border: "1px solid var(--border)", padding: 24, boxShadow: "var(--shadow)" }}>
...
const GuestView = ({ sending, onSend, onExit }) => (
  <div style={{ width: "100%", minHeight: "100vh", display: "block", background: "#fff" }}>
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 50 }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid var(--border)", padding: "14px 28px", display: "flex", alignItems: "center", gap: 10, boxSizing: "border-box" }}>
        <img src="/icon/emergency_2.png" alt="" style={{ width: 22, height: 22, filter: "brightness(0) saturate(100%) invert(17%) sepia(82%) saturate(1200%) hue-rotate(210deg)" }} />
        <div>
    ...
    <div style={{ padding: "24px 28px", paddingTop: 120, width: "100%", boxSizing: "border-box" }}>
      <div style={{ borderRadius: 14, marginBottom: 24, overflow: "hidden", width: "100%" }}>
        <img src="/image/emergency_alert_system.png" alt="Emergency Alert System" style={{ width: "100%", height: "auto", display: "block", borderRadius: 14 }} />
      </div>
...
const Emergency = ({ guestMode = false }) => {
  const { user, isAdmin, isCustomer, isManager, isEmployee, loading: userLoading } = useCurrentUser();
  ...
  const showRespondingToast = useCallback(() => {
    ...
    setToasts(t => [...t, { id, guestMode: true, variant: 'responding', show: false }]);
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel('guest-alert-' + trackedAlertId)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'emergency_alerts', filter: `guest_ref=eq.${trackedAlertId}` }, (payload) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (data) {
        ...
        await supabase
          .from('branch_availability')
          .insert({ branch: userBranch, available: true, updated_at: new Date().toISOString() });
        setBranchAvailable(true);
      }
    };
    load();
    const ch = supabase.channel('branch-avail-staff-' + userBranch)
      .on('postgres_changes',
  ...
  const fetchAlerts = useCallback(async () => {
    ...
    if (!error) {
      const rows = data || [];
      const filtered = (!isAdmin && userBranch)
        ? rows.filter(a => normalizeBranchName(a.branch) === normalizeBranchName(userBranch))
        : rows;
      setAlerts(filtered);
    }
    setLoading(false);
  ...
  const sendAlert = useCallback(async (formData) => {
    ...
    const BRANCH_NAME_TO_ID = { Main: 1, "Mabalacat 2": 2, Tarlac: 3, "San Fernando": 4, Angeles: 5 };
    const guestRef = guestMode ? `g_${Date.now()}_${Math.random().toString(36).slice(2, 8)}` : undefined;
    const payload = guestMode
      ? { ...base, guest_ref: guestRef, branch_id: BRANCH_NAME_TO_ID[formData.branch] ?? null }
    ...
    logActivity(user || { id: 'guest', fullName: formData.guest_full_name || 'Guest', role: 'guest' }, 'Sent emergency alert', `Type: ${formData.type} · Branch: ${formData.branch}`);
  ...
  const requestStatusChange = useCallback((id, status) => {
    if (status === "resolved") {
      const alert = alerts.find(a => a.id === id);
      setConfirmStatusChange({ id, status, alertType: alert?.type || "this alert" });
      return;
    }
  ...
  if (isAdmin) {
    return (
     <Layout>
        <ToastStack toasts={toasts} onClose={closeToast} />
        <AdminView alerts={alerts} loading={loading} onRefresh={fetchAlerts} onUpdateStatus={requestStatusChange} userBranch={userBranch} branchAvailable={branchAvailable} onToggleAvailability={toggleAvailability} />
        <Modal
          ...
          cancelText="Cancel"
          confirmColor="#16a34a"
          onConfirm={() => { updateStatus(confirmStatusChange.id, confirmStatusChange.status); setConfirmStatusChange(null); }}
 ...
 // Employee: can send alerts but cannot mark responding/resolved
  ...
  return (
    <Layout>
      <ToastStack toasts={toasts} onClose={closeToast} />
       <StaffView alerts={alerts} loading={loading} sending={sending} onSend={sendAlert} onExit={handleGuestExit} onUpdateStatus={requestStatusChange} userBranch={userBranch} />
      <Modal
        show={!!confirmStatusChange}
        title="Mark Alert as Resolved?"
        message={`Are you sure you want to mark "${confirmStatusChange?.alertType}" as resolved? This confirms the emergency has been fully addressed.`}
