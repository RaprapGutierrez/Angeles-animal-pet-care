...
const EMPTY_FORM = {
  patient: '', species: 'Dog', sex: 'Unknown', room: '', owner: '', ownerFirstName: '', ownerLastName: '', ownerSex: '', owner_id: null,
  contact: '', purpose: 'Checkup', vet: '', notes: '', status: 'Waiting',
...
const Walkin = () => {
  ...
  const showConfirm = (message, onConfirm, title = 'Confirm') => setDialog({ show: true, message, type: 'confirm', onConfirm, title });
  ...
  const EMPTY_PET = { mode: 'new', existingId: null, patient: '', species: 'Dog', room: "", purpose: 'Checkup', vet: '', notes: '' };
  ...
  const fetchClients = useCallback(async () => {
    if (!user) return;
    let q = supabase.from('profiles').select('id, first_name, last_name, email, phone, role').eq('status', 'Active').in('role', ['customer', 'Customer']).order('first_name');
  ...
  useEffect(() => {
    if (user) logActivity(user, 'Viewed walk-ins', 'Opened walk-in registration');
  }, []);
  ...
  const arrivalDisplay = formatArrival(editItem ? editItem.arrived_at : new Date().toISOString());
  ...
  // ── Modal open/close ──────────────────────────────────────────────────────
  const snapshotWalkinState = (f, ot) => JSON.stringify({ form: f, ownerType: ot });
  ...
  const attemptCloseModal = () => {
    if (hasUnsavedWalkinEdits()) {
      showConfirm("You have unsaved changes to this record. Do you want to discard them?", closeModal, 'Discard Changes?');
    } else {
      closeModal();
    }
  };
  ...
  const upsertPatient = async () => {
    ...
    if (ownerType === 'walkin' && form.owner.trim()) {
      const firstName = form.ownerFirstName.trim() || form.owner.trim().split(' ')[0];
      const lastName = form.ownerLastName.trim() || (form.owner.trim().split(' ').slice(1).join(' ') || '');
      ...
      if (!signUpErr && signUpData?.user?.id) {
        ...
        newCredentials = { fullName: form.owner.trim(), email: generatedEmail, password: generatedPassword };
      }
    }
    ...
    await supabase.from('patients').insert([{
      ...
      condition: form.purpose !== 'Grooming' ? `Walk-in: ${form.purpose}` : 'Grooming',
      status: 'Outpatient',
      health: 'Good',
      branch_id: user?.branchId ?? null,
    }]);
  ...
  const getGroomingUsedForExtra = (excludeIdx) => {
    const usedFromExisting = walkins.filter(w => w.purpose === 'Grooming' && w.status === 'Waiting').length;
    const usedFromMain = form.purpose === 'Grooming' ? 1 : 0;
    const usedFromBatch = extraPets.filter((p, i) => i !== excludeIdx && p.purpose === 'Grooming').length;
  ...
  const isFormValid = () => isMainPetValid() && isOwnerValid() && isExtraPetsValid() && !conflictType;

  const saveWalkin = async () => {
    if (!form.patient.trim()) { showAlert('Please enter the patient (pet) name.', 'Missing Field'); return; }
    if (!form.owner.trim()) { showAlert('Please enter the owner name.', 'Missing Field'); return; }
    if (conflictType) { showAlert('Cannot save: grooming is fully booked.', 'Fully Booked'); return; }

    // Validate extra pets (only relevant for new walk-ins, not edit mode)
    if (!editItem) {
      for (const p of extraPets) {
        if (p.mode === 'existing' && !p.existingId) { showAlert('Please select an existing pet for each entry, or switch to "New Pet".', 'Missing Field'); return; }
        if (p.mode === 'new' && !p.patient.trim()) { showAlert('Please enter a pet name for each new pet.', 'Missing Field'); return; }
      }
      const groomingTotal = getGroomingUsedForExtra(-1);
      if (groomingTotal > MAX_GROOMERS) { showAlert('One or more pets cannot be added: grooming is fully booked.', 'Fully Booked'); return; }
    }
    ...
    if (editItem) {
      ...
      setSaving(false);
      if (error) { showAlert('Error updating: ' + error.message, 'Error'); return; }
      closeModal();
      logActivity(user, 'Updated walk-in', `Edited walk-in for: ${form.patient}`);
    ...
    if (error) { setSaving(false); showAlert('Error saving: ' + error.message, 'Error'); return; }
    await upsertPatient();
    ...
    for (const p of extraPets) {
      ...
      const { error: extraErr } = await supabase.from('walkins').insert([extraPayload]).select();
      if (extraErr) { console.warn('Extra pet walk-in failed:', extraErr.message); continue; }
      ...
      if (p.mode === 'new' && petName) {
        ...
        if (!existing) {
          await supabase.from('patients').insert([{
            ...
            owner: form.owner.trim(),
            contact: form.contact.trim() || null,
            owner_user_id: ownerType === 'registered' ? (form.owner_id || null) : null,
            condition: p.purpose !== 'Grooming' ? `Walk-in: ${p.purpose}` : 'Grooming',
            status: 'Outpatient',
            health: 'Good',
            branch_id: user?.branchId ?? null,
          }]);
    ...
    setSaving(false);
    closeModal();
    logActivity(user, 'Registered walk-in', `Walk-in patient: ${form.patient} · Owner: ${form.owner}${extraPets.length ? ` (+${extraPets.length} more pet${extraPets.length > 1 ? 's' : ''})` : ''}`);
    showToast(extraPets.length ? `✓ ${1 + extraPets.length} walk-ins registered for ${form.owner.trim()}!` : '✓ Walk-in registered & patient record created!');
  };
  ...
  const deleteWalkin = async (id) => {
    showConfirm('Permanently delete this walk-in? This cannot be undone.', async () => {
      const { error } = await supabase.from('walkins').delete().eq('id', id);
      if (error) showAlert('Error deleting walk-in: ' + error.message, 'Error');
    }, 'Delete Walk-In');
  };

  const S = {
    page: { width: '100%', minHeight: '100vh', display: 'block' },
    topbar: { background: 'var(--card)', borderBottom: '1px solid var(--border)', padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'fixed', top: 68, left: 'var(--current-sidebar-w, 62px)', right: 0, zIndex: 40, boxSizing: 'border-box', gap: 8, flexWrap: 'wrap' },
        ...
        {selectedClient ? (
          ...
          <>
            <div
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 12px', border: `1.5px solid ${showOwnerDrop ? 'var(--royal)' : 'var(--border)'}`, borderRadius: 8, background: 'var(--card)', cursor: 'text', boxSizing: 'border-box', transition: 'border-color 0.15s' }}>
  ...
  return (
    <Layout>
      <div style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 999999, display: 'flex', flexDirection: 'column', gap: 10, pointerEvents: 'none' }}>
        {toasts.slice().reverse().map(t => <Toast key={t.id} message={t.message} show={t.show} type={t.type} />)}
      </div>
          ...
          <div style={{ background: 'var(--card)', borderRadius: 14, boxShadow: '0 24px 64px rgba(0,0,0,0.30)', width: '100%', maxWidth: 440, overflow: 'hidden' }}>
            <div style={{ background: 'linear-gradient(135deg,#f0fdf4,#dcfce7)', padding: '20px 24px 16px', borderBottom: '1px solid #bbf7d0' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: '#14532d' }}>Walk-In Account Created!</h3>
                  <p style={{ margin: 0, fontSize: 12, color: '#16a34a', marginTop: 2 }}>Patient record & owner account are ready</p>
                </div>
              </div>
            </div>
            <div style={{ padding: '20px 24px' }}>
              {[['Name', walkinCredentials.fullName], ['Email', walkinCredentials.email], ['Password', walkinCredentials.password]].map(([label, value]) => (
              ...
              <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#92400e', marginTop: 4 }}>
                Share these credentials with the owner. The password won't be shown again.
              </div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, padding: '14px 24px', borderTop: '1px solid var(--border)' }}>
              <button className="btn btn-ghost" style={{ width: 'auto' }} onClick={() => { navigator.clipboard?.writeText(`Name: ${walkinCredentials.fullName}\nEmail: ${walkinCredentials.email}\nPassword: ${walkinCredentials.password}`); }}>Copy All</button>
              <button className="btn btn-primary" style={{ width: 'auto' }} onClick={() => setWalkinCredentials(null)}>Done</button>
            </div>
          </div>
        </div>
      )}
      <div style={S.page}>
        ...
        <div style={S.topbar} className="walkin-topbar">
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <img src="/icon/walkin.png" alt="" style={{ width: 22, height: 22, flexShrink: 0, filter: 'brightness(0) saturate(100%) invert(17%) sepia(82%) saturate(1200%) hue-rotate(210deg)' }} />
            <div>
              <h1 style={{ fontSize: 16, fontWeight: 800, color: 'var(--text)', margin: 0 }}>Walk-In Registration</h1>
            ...
            <div style={{ position: "fixed", bottom: 28, right: 28, zIndex: 999, display: "flex", alignItems: "center", gap: 10 }}
              ...
              <span className="fab-tooltip" style={{ opacity: 0, transform: 'translateX(8px)', transition: 'opacity 0.2s ease, transform 0.2s ease', background: 'linear-gradient(135deg,#0f172a,#1e3a8a)', color: '#fff', fontSize: 12, fontWeight: 700, padding: '8px 14px', borderRadius: 10, whiteSpace: 'nowrap', pointerEvents: 'none', boxShadow: '0 8px 24px rgba(30,58,138,0.35)', border: '1px solid rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', gap: 7, position: 'relative' }}>
                <span style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.2 }}>
                  <span style={{ fontSize: 12, fontWeight: 800, color: '#fff' }}>Register Walk-In</span>
                  <span style={{ fontSize: 10, fontWeight: 500, color: 'rgba(255,255,255,0.55)' }}>Record a new visit</span>
                </span>
        ...
        <div className="content" style={{ paddingTop: 122 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(195px,1fr))', gap: 14, marginBottom: 24 }}>
            {loading ? (
              [1, 2, 3].map(i => (
                <div key={i} style={{ background: 'var(--card)', border: '1.5px solid var(--border)', borderRadius: 16, padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 640 }}>
                  ...
                  <tbody>
                    {filteredWalkins.length === 0 ? (
                      ...
                      return (
                        <tr key={w.id} className="wk-row-hover fade-in" style={{ background: 'var(--card)', transition: 'background 0.15s', animationDelay: `${idx * 0.06}s` }}
                          onClick={() => openView(w)}>
                          <td style={S.td}>
                            <span style={{ fontSize: 11, color: 'var(--muted)', fontWeight: 600 }}>{(safePage - 1) * ROWS_PER_PAGE + idx + 1}</span>
                          </td>
                          <td style={S.td}>
                          ...
                          <td style={S.td}>
                            {w.purpose === 'Grooming'
                              ...
                              : w.vet
                                ? <span style={{ fontSize: 13, color: 'var(--text)', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                          ...
                          <td style={S.td} onClick={e => e.stopPropagation()}>
                            <div style={{ display: 'flex', gap: 5, alignItems: 'center', flexWrap: 'nowrap' }}>
                              {w.status === 'Waiting' && (
                                <button
                                  onClick={() => updateStatus(w.id, 'Attended')}
                                  style={{ background: 'none', border: '1px solid #bbf7d0', borderRadius: 20, height: 28, padding: '0 10px', display: 'flex', alignItems: 'center', gap: 5, cursor: 'pointer', color: '#16a34a', fontSize: 11, fontWeight: 600, fontFamily: 'inherit' }}>
                              ...
                              {w.status === 'Waiting' && (
                                <button
                                  onClick={() => showConfirm('Are you sure you want to cancel this walk-in?', () => updateStatus(w.id, 'Cancelled'), 'Cancel Walk-In')}
                              ...
                              <button
                                onClick={() => { setViewItem(null); openEdit(w); }}
      ...
      {viewItem && (() => {
        const w = viewItem;
        const statusColor = { Waiting: '#d97706', Attended: '#16a34a', Cancelled: '#dc2626' }[w.status] || '#64748b';
        const purposeStyle = {
          Grooming: { bg: '#f3e8ff', color: '#7c3aed' },
          Emergency: { bg: '#fee2e2', color: '#dc2626' },
          Checkup: { bg: '#eff6ff', color: '#1e40af' },
        ...
        return (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 16 }}>
            <div style={{ background: 'var(--card)', borderRadius: 16, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.28)' }}>

              {/* Hero banner */}
              <div style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e3a8a 60%, #2563eb 100%)', borderRadius: '16px 16px 0 0', padding: '20px 22px', position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
                ...
                <div style={{ position: 'relative', zIndex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                      <div style={{
                        ...
                        background: w.species === 'Cat' ? '#f0fdf4' : '#eff6ff',
                      ...
                      <div>
                        <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: '#fff', lineHeight: 1.2 }}>{w.patient}</h2>
                        <p style={{ margin: '4px 0 0', fontSize: 13, color: 'rgba(255,255,255,0.65)' }}>{w.species}{w.owner ? ` · ${w.owner}` : ''}</p>
                      </div>
                    </div>
                    <button onClick={() => setViewItem(null)} style={{ background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.25)', borderRadius: 8, width: 30, height: 30, cursor: 'pointer', color: '#fff', fontSize: 15, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>✕</button>
                  </div>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <span style={{ background: `${statusColor}33`, border: `1.5px solid ${statusColor}66`, color: '#fff', borderRadius: 20, padding: '4px 12px', fontSize: 12, fontWeight: 700, display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                      <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#fff', display: 'inline-block' }} />
                      {w.status}
                    </span>
                    <span style={{ background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 20, padding: '4px 12px', fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.9)' }}>
                      {w.purpose}
                    </span>
                    {!w.owner_id ? (
                      <span style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 20, padding: '4px 12px', fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.8)', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="5" r="2" /><path d="M12 22V12m0 0l-3 3m3-3l3 3" /><path d="M9 9H5m14 0h-4" /></svg>
                        Walk-in Guest
                      </span>
                    ) : (
                      <span style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 20, padding: '4px 12px', fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.8)' }}>
                        Registered Client
                      </span>
                    )}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 14 }}>
                    {[
                      { icon: <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" /><polyline points="9 22 9 12 15 12 15 22" /></svg>, label: w.room ? `Room ${w.room}` : 'No Room Assigned' },
                      { icon: <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>, label: `${arrival.date} · ${arrival.time}` },
                      { icon: <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13.5 19.79 19.79 0 0 1 1.6 4.9 2 2 0 0 1 3.58 2.72h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 10.1a16 16 0 0 0 6 6l.92-.92a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 21.5 17.5z" /></svg>, label: w.contact || 'No Contact' },
                    ].map(({ icon, label }) => (
              ...
              <div style={{ padding: '20px 22px', flex: 1 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  {[
                    ...
                    { label: 'Vet / Service', value: w.purpose === 'Grooming' ? 'Grooming Team' : (w.vet || 'Unassigned') },
                    { label: 'Assigned Room', value: w.room ? `Room ${w.room}` : 'N/A' },
                  ].map(({ label, value }) => (
                    <div key={label} style={{ background: '#f8fafc', border: '1.5px solid var(--border)', borderRadius: 10, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 3 }}>
                  ...
                  {w.notes && (
                    <div style={{ gridColumn: '1 / -1' }}>
                      <div style={{ background: '#fffbeb', border: '1.5px solid #fde68a', borderRadius: 10, padding: '14px 16px' }}>
                        ...
                        <p style={{ margin: 0, fontSize: 14, color: '#78350f', lineHeight: 1.6, fontWeight: 500 }}>{w.notes}</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              {/* Footer */}
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, padding: '14px 22px', borderTop: '1px solid var(--border)', background: 'var(--bg)', flexShrink: 0 }}>
                <button className="btn btn-ghost" style={{ width: 'auto' }} onClick={() => setViewItem(null)}>Close</button>
                <button className="btn btn-ghost" style={{ width: 'auto' }} onClick={() => { setViewItem(null); openEdit(w); }}>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 16 }}>
          <div style={{ background: 'var(--card)', borderRadius: 14, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>

            {/* Clipboard bar */}
            <div className="wk-clipboard-bar" style={{ flexShrink: 0, justifyContent: "flex-end" }}>
              <button onClick={attemptCloseModal} style={{ background: "none", border: "none", fontSize: 18, cursor: "pointer", color: "rgba(255,255,255,0.75)", lineHeight: 1, padding: "2px 6px" }}>✕</button>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              ...
              <div style={{ borderBottom: '1.5px solid #e2e8f0' }}>
                <div className="wk-section-label" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span>Patient Information {extraPets.length > 0 ? `(Pet 1 of ${1 + extraPets.length})` : ''}</span>
                  {!editItem && (
                    <button style={{ padding: '3px 9px', borderRadius: 6, border: '1.5px dashed #c7d2fe', background: '#f5f3ff', color: '#6366f1', fontWeight: 600, fontSize: 11, cursor: 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 5 }}
                      onClick={addExtraPet}>
                ...
                <div style={{ padding: '10px 16px', borderBottom: '1px solid #e2e8f0' }}>
                  <div style={{ display: 'flex', border: '1.5px solid var(--border)', borderRadius: 8, overflow: 'hidden', width: 'fit-content', marginBottom: form.mode === 'existing' ? 10 : 0 }}>
                    {[{ key: 'new', label: 'New Pet' }, { key: 'existing', label: 'Existing Pet' }].map(({ key, label }) => (
                      <button key={key} type="button"
                        onClick={() => setForm(prev => key === 'existing' ? { ...prev, mode: 'existing', existingId: null } : { ...prev, mode: 'new', existingId: null })}
                  ...
                  {form.mode === 'existing' ? (
                    loadingExistingPatients ? (
                      ...
                      <select value={form.existingId || ''} onChange={e => {
                        const sel = existingPatients.find(ep => ep.id === e.target.value);
                        setForm(prev => ({ ...prev, existingId: sel?.id || null, patient: sel?.name || '', species: sel?.species || 'Dog' }));
                      }}
                        style={{ width: '100%', border: '1.5px solid var(--border)', borderRadius: 8, background: '#fff', fontSize: 13, fontWeight: 600, color: 'var(--text)', outline: 'none', padding: '8px 10px', fontFamily: 'inherit', boxSizing: 'border-box' }}>
                        <option value="">— Select a pet —</option>
                        {existingPatients.map(ep => <option key={ep.id} value={ep.id}>{ep.name}{ep.species ? ` (${ep.species})` : ''}</option>)}
                      </select>
                    )
                  ) : (
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 10 }}>
                      <div>
                        ...
                        <input type="text" value={form.patient} onChange={e => setForm({ ...form, patient: sanitizeName(e.target.value) })} placeholder="Pet name"
                      ...
                      <div>
                        <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6 }}>Sex</div>
                        <CustomSelect
                          value={form.sex}
              ...
              {!editItem && extraPets.map((p, idx) => (
                <div key={idx} style={{ borderBottom: '1.5px solid #e2e8f0' }}>
                  <div className="wk-section-label" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span>Pet {idx + 2} of {1 + extraPets.length}</span>
                  ...
                  <div style={{ padding: '10px 16px', borderBottom: '1px solid #e2e8f0' }}>
                    <div style={{ display: 'flex', border: '1.5px solid var(--border)', borderRadius: 8, overflow: 'hidden', width: 'fit-content', marginBottom: p.mode === 'existing' ? 10 : 0 }}>
                      {[{ key: 'new', label: 'New Pet' }, { key: 'existing', label: 'Existing Pet' }].map(({ key, label }) => (
                        <button key={key} type="button"
                          onClick={() => updateExtraPet(idx, key === 'existing' ? { mode: 'existing', patient: '', species: 'Dog' } : { mode: 'new', existingId: null })}
                          style={{ padding: '6px 16px', border: 'none', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s', background: p.mode === key ? 'var(--royal)' : '#fff', color: p.mode === key ? '#fff' : 'var(--muted)' }}>
                          {label}
                        </button>
                      ))}
                    </div>
                    ...
                    {p.mode === 'existing' ? (
                      loadingExistingPatients ? (
                        ...
                        <select value={p.existingId || ''} onChange={e => {
                          const sel = existingPatients.find(ep => ep.id === e.target.value);
                          updateExtraPet(idx, { existingId: sel?.id || null, species: sel?.species || 'Dog' });
                        }}
                          style={{ width: '100%', border: '1.5px solid var(--border)', borderRadius: 8, background: '#fff', fontSize: 13, fontWeight: 600, color: 'var(--text)', outline: 'none', padding: '8px 10px', fontFamily: 'inherit', boxSizing: 'border-box' }}>
                          <option value="">— Select a pet —</option>
                          {existingPatients.map(ep => <option key={ep.id} value={ep.id}>{ep.name}{ep.species ? ` (${ep.species})` : ''}</option>)}
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                        <div>
                          <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6 }}>Patient Name <span style={{ color: '#ef4444' }}>*</span></div>
                         <input type="text" value={p.patient} onChange={e => updateExtraPet(idx, { patient: sanitizeName(e.target.value) })} placeholder="Pet name"
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', borderBottom: '1px solid #e2e8f0' }}>
                    <div style={{ padding: '10px 16px', borderRight: '1px solid #e2e8f0' }}>
                      ...
                      <CustomSelect
                        value={p.purpose}
                        onChange={val => updateExtraPet(idx, { purpose: val, vet: val === 'Grooming' ? '' : p.vet })}
                    ...
                    <div style={{ padding: '10px 16px' }}>
                      {p.purpose !== 'Grooming' ? (
                        <>
                          ...
                          <CustomSelect value={p.vet} onChange={val => updateExtraPet(idx, { vet: val })} options={VETS} placeholder="Unassigned" />
                        </>
                      ) : (
                        <div style={{ fontSize: 11, color: '#9333ea', display: 'flex', alignItems: 'center', gap: 6, paddingTop: 4 }}>
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#7c3aed" strokeWidth="2" strokeLinecap="round"><circle cx="6" cy="6" r="3" /><circle cx="6" cy="18" r="3" /><line x1="20" y1="4" x2="8.12" y2="15.88" /><line x1="14.47" y1="14.48" x2="20" y2="20" /><line x1="8.12" y1="8.12" x2="12" y2="12" /></svg>
              ...
              <div style={{ borderBottom: '1.5px solid #e2e8f0' }}>
                <div className="wk-section-label" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ fontSize: 9, fontWeight: 700, color: '#16a34a', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
                    <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"><rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>
                    Auto-recorded
                  </span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', padding: '12px 16px', gap: 14 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 34, height: 34, borderRadius: 9, background: '#eff6ff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#1d4ed8" strokeWidth="2" strokeLinecap="round"><rect x="3" y="4" width="18" height="18" rx="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>
                    </div>
                    <div>
                      <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px' }}>Date</div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)' }}>{arrivalDisplay.date}</div>
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 34, height: 34, borderRadius: 9, background: '#f0fdf4', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>
                    </div>
                    <div>
                      <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px' }}>Time</div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)' }}>{arrivalDisplay.time}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ borderBottom: '1.5px solid #e2e8f0', position: 'relative', zIndex: showOwnerDrop ? 200 : 'auto' }}>
                ...
                <div style={{ padding: '12px 16px' }}>
                  ...
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 6, padding: '4px 10px', marginBottom: 10, fontSize: 11, fontWeight: 700, color: '#1e40af' }}>
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M3 21h18M5 21V7l7-4 7 4v14M9 21v-6h6v6" /></svg>
                    Branch: {branches.find(b => b.id === user?.branchId)?.name || 'My Branch'}
                  </div>
                  ...
                  {ownerType === 'walkin' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                        <div>
                          <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6 }}>First Name <span style={{ color: '#ef4444' }}>*</span></div>
                         <input type="text" placeholder="e.g. Juan" value={form.ownerFirstName}
                            onChange={e => {
                              const v = sanitizeName(e.target.value);
                              setForm(prev => ({ ...prev, ownerFirstName: v, owner: `${v} ${prev.ownerLastName}`.trim(), owner_id: null }));
                            }}
                            onBlur={() => fetchExistingPatientsFor(form.owner.trim(), null)}
                            style={{ width: '100%', border: 'none', borderBottom: '1.5px solid #cbd5e1', background: 'transparent', fontSize: 13, fontWeight: 600, color: 'var(--text, #1e293b)', outline: 'none', padding: '2px 0', fontFamily: 'inherit', boxSizing: 'border-box' }} />
                        </div>
                        <div>
                          <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6 }}>Last Name <span style={{ color: '#ef4444' }}>*</span></div>
                         <input type="text" placeholder="e.g. dela Cruz" value={form.ownerLastName}
                            onChange={e => {
                              const v = sanitizeName(e.target.value);
                              setForm(prev => ({ ...prev, ownerLastName: v, owner: `${prev.ownerFirstName} ${v}`.trim(), owner_id: null }));
                            }}
                            onBlur={() => fetchExistingPatientsFor(form.owner.trim(), null)}
                            style={{ width: '100%', border: 'none', borderBottom: '1.5px solid #cbd5e1', background: 'transparent', fontSize: 13, fontWeight: 600, color: 'var(--text, #1e293b)', outline: 'none', padding: '2px 0', fontFamily: 'inherit', boxSizing: 'border-box' }} />
                        </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                        <div>
                          ...
                          <CustomSelect
                            value={form.ownerSex}
                            onChange={val => setForm(prev => ({ ...prev, ownerSex: val }))}
                            options={['Male', 'Female']}
                            placeholder="— Select Sex —"
                          />
                        </div>
                        <div>
                          ...
                          <input type="text" placeholder="e.g. 09170000000" value={form.contact}
                            onChange={e => setForm(prev => ({ ...prev, contact: sanitizeContact(e.target.value) }))}
              ...
              <div style={{ borderBottom: '1.5px solid #e2e8f0' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: '1px solid #e2e8f0' }}>
                  <div style={{ padding: '10px 16px', borderRight: '1px solid #e2e8f0' }}>
                    ...
                    <CustomSelect
                      value={form.purpose}
                      onChange={val => setForm(prev => ({ ...prev, purpose: val }))}
                      options={PURPOSES}
                    />
                  </div>
                  <div style={{ padding: '10px 16px' }}>
                ...
                <div style={{ padding: '10px 16px' }}>
                  ...
                  <CustomSelect
                    ...
                    options={rooms
                      .filter(r => r.status === 'Available' || r.number === form.room)
                      .map(r => ({
                        value: r.number,
              ...
              {!editItem && extraPets.some(p => p.purpose === 'Grooming') && getGroomingUsedForExtra(-1) > MAX_GROOMERS && (
                <div style={{ margin: '0 16px 16px', background: '#fef3c7', border: '1.5px solid #fcd34d', borderRadius: 8, padding: '12px 16px', fontSize: 13 }}>
                  <p style={{ margin: 0, fontWeight: 700, color: '#92400e' }}>One or more additional pets can't be booked for grooming — only {MAX_GROOMERS} groomer slots exist total.</p>
                </div>
              )}

              {!isFormValid() && !conflictType && (
                <div style={{ margin: '0 16px 16px', background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#9a3412', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#9a3412" strokeWidth="2" strokeLinecap="round" style={{ flexShrink: 0 }}><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>
                  <span>
                    {!isMainPetValid()
                      ? (form.mode === 'existing' ? 'Please select an existing pet.' : 'Please enter the patient (pet) name.')
                      : !isOwnerValid()
                        ? (ownerType === 'registered' ? 'Please select a registered client.' : (!form.owner.trim() ? 'Please enter the owner name.' : 'Contact number must be 11 digits.'))
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, padding: '14px 24px', borderTop: '2px solid var(--border)', background: 'var(--bg)', flexShrink: 0, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
               <button className="btn btn-ghost" style={S.btn} onClick={attemptCloseModal}>Cancel</button>
                <button className="btn btn-primary"
                  style={{ ...S.btn, background: '#0f172a', borderColor: '#0f172a', opacity: (!isFormValid() || saving) ? 0.5 : 1, cursor: (!isFormValid() || saving) ? 'not-allowed' : 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6 }}
                  onClick={saveWalkin} disabled={saving || !isFormValid()}>
                  {saving ? 'Saving...' : (
                    <>
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12" /></svg>
                      {editItem ? 'Save Changes' : extraPets.length > 0 ? `Register ${1 + extraPets.length} Walk-Ins` : 'Register Walk-In'}
                    </>
                  )}
                </button>
              </div>
      ...
      {dialog.show && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 99999, padding: 16 }}>
          <div style={{ background: 'var(--card)', borderRadius: 14, width: '100%', maxWidth: 380, boxShadow: '0 20px 60px rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '18px 22px 14px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 34, height: 34, borderRadius: 10, background: dialog.type === 'confirm' ? '#fef2f2' : '#eff6ff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {dialog.type === 'confirm'
                  ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#dc2626" strokeWidth="2.5" strokeLinecap="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>
                  : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#2563eb" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>
                }
              </div>
              <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: 'var(--text)' }}>{dialog.title}</h3>
            </div>
            <div style={{ padding: '16px 22px' }}>
              <p style={{ margin: 0, fontSize: 13, color: 'var(--muted)', lineHeight: 1.6 }}>{dialog.message}</p>
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, padding: '12px 22px', borderTop: '1px solid var(--border)', background: 'var(--bg)' }}>
              {dialog.type === 'confirm' ? (
                <>
                  <button className="btn btn-ghost" style={{ width: 'auto' }} onClick={() => setDialog(d => ({ ...d, show: false }))}>Cancel</button>
                  <button className="btn btn-danger" style={{ width: 'auto' }} onClick={() => { dialog.onConfirm?.(); setDialog(d => ({ ...d, show: false })); }}>Confirm</button>
                </>
              ) : (
                <button className="btn btn-primary" style={{ width: 'auto', background: '#0f172a', borderColor: '#0f172a' }} onClick={() => setDialog(d => ({ ...d, show: false }))}>OK</button>
              )}
            </div>
          </div>
        </div>
