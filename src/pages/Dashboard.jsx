...
const SkeletonStyles = () => (
  <style>{`
    ...
    .sk { ${Object.entries(skeletonStyle).map(([k, v]) => `${k.replace(/([A-Z])/g, '-$1').toLowerCase()}:${v}`).join(';')} }
    .dash-fade-in { animation: fadeSlideUp 0.4s ease both; }
    .stat-card-v2 {
      background: var(--card);
    ...
    .stat-card-v2.purple::before { background: linear-gradient(90deg,#7c3aed,#a855f7); }
    ...
    .stat-card-v2 .stat-icon-v2.purple { background: #f3f0ff; }
    ...
    .stat-card-v2 .stat-icon-v2.purple img { filter: brightness(0) saturate(100%) invert(22%) sepia(96%) saturate(1600%) hue-rotate(255deg) brightness(0.85); }
    ...
    .quick-link-v2:hover .ql-icon-wrap {
    ...
    .walkin-row:hover { background: var(--bg); }
    .pending-dot {
...
// ─── DashCard ─────────────────────────────────────────────────────────────────
const DashCard = ({ title, subtitle, badge, viewAllTo, viewAllLabel = 'View All →', children }) => (
  <div style={{
    background: 'var(--card)',
    border: '1.5px solid var(--border)',
    borderRadius: 16,
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div>
          ...
          {subtitle && <p style={{ margin: 0, fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>{subtitle}</p>}
        </div>
        {badge != null && badge > 0 && (
          <span style={{
...
const EmptySlate = ({ icon, text }) => (
  <div style={{ padding: '40px 20px', textAlign: 'center' }}>
    <div style={{
      ...
      color: '#94a3b8', margin: '0 auto 12px',
    }}>{icon}</div>
    <p style={{ color: 'var(--muted)', fontSize: 13, margin: 0, lineHeight: 1.5 }}>{text}</p>
  </div>
);
...
const TodayAppointments = ({ appts, loading, pendingCount }) => (
  ...
  >
    {loading ? (
      ...
      <EmptySlate icon={<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>} text="No appointments scheduled for today" />
    ) : (
      appts.map((a) => (
        <div key={a.id} className="appt-row">
          ...
          <div style={{ textAlign: 'center', minWidth: 52, flexShrink: 0 }}>
            <span style={{
              fontSize: 11, fontWeight: 800, color: 'var(--royal)',
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {a.owner && (
              <span style={{ fontSize: 11, color: 'var(--muted)' }}>Owner: {a.owner}</span>
            )}
          </div>

          {/* Badge */}
          <div style={{ flexShrink: 0, textAlign: 'right' }}>
            <span className={`badge ${STATUS_BADGE[a.status] || 'badge-gray'}`} style={{ fontSize: 10 }}>
              {a.status}
            </span>
            {a.status === 'Pending' && (
              <p style={{ margin: '3px 0 0', fontSize: 10, color: '#d97706', fontWeight: 600 }}>
...
const TodayWalkins = ({ walkins, loading }) => (
  ...
  >
    {loading ? (
      ...
      <EmptySlate icon={<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><circle cx="12" cy="5" r="2"/><path d="M9 12l-2 7M15 12l2 7M9 12h6M9 12l-1-4M15 12l1-4"/></svg>} text="No walk-ins recorded for today" />
    ) : (
      walkins.map((w, i) => {
        const name = w.patient_name || w.pet_name || '—';
        const initial = (w.patient_name || w.owner_name || '?').charAt(0).toUpperCase();
        return (
          <div key={w.id || i} className="walkin-row">
            ...
            <span style={{ fontSize: 11, color: 'var(--muted)', flexShrink: 0 }}>
              {w.time || (w.created_at
                ? new Date(w.created_at).toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit' })
                : '')}
            </span>
          </div>
        );
...
const Dashboard = () => {
  const { user, isAdmin, isManager, isEmployee, seeAllBranches, loading: userLoading } = useCurrentUser();
  const { applyFilter } = useBranchFilter();

  const location = useLocation();
  const [stats, setStats] = useState({
    patients: 0, appointments: 0, inventory: 0,
    sales: 0, messages: 0, pendingAppts: 0, walkins: 0,
  });
  ...
  const fetchDashboard = async () => {
    ...
    setLoading(true);
    try {
      const patientsQ = applyFilter(supabase.from('patients').select('id', { count: 'exact', head: true }));
      const inventoryQ = applyFilter(supabase.from('inventory').select('id,qty,threshold'));
      const transactionsQ = applyFilter(supabase.from('transactions').select('total').gte('created_at', today));
      const messagesQ = applyFilter(supabase.from('messages').select('id', { count: 'exact', head: true }).eq('is_read', false));

      const addBranchOverride = (q) => {
        if (seeAllBranches && branchFilter) return q.eq('branch_id', branchFilter);
        return q;
      };
      ...
      const todaySales = (transactions.data || []).reduce((s, t) => s + Number(t.total || 0), 0);
      const lowStockCount = (inventory.data || []).filter(i => i.qty <= (i.threshold ?? 10)).length;

      let apptQ = supabase.from('appointments').select('*').eq('date', today).order('time', { ascending: true });
      apptQ = applyFilter(apptQ);
      if (seeAllBranches && branchFilter) apptQ = apptQ.eq('branch_id', branchFilter);
      const { data: appts } = await apptQ;
      const apptList = appts || [];
      setTodayAppts(apptList);

      let wkQ = supabase.from('walkins').select('*')
        .gte('created_at', `${today}T00:00:00`)
        .lte('created_at', `${today}T23:59:59`)
        .order('created_at', { ascending: true });
      wkQ = applyFilter(wkQ);
      if (seeAllBranches && branchFilter) wkQ = wkQ.eq('branch_id', branchFilter);
  ...
  const STAT_CARDS = useMemo(() => {
    const cardPatients = {
      ...
      icon: '/icon/active_acc.png', color: 'purple',
      sub: 'Total registered', subColor: 'var(--muted)', to: '/patient-records',
      trend: null,
    };
    const cardLowStock = {
      label: 'Low Stock Items', value: stats.inventory,
      icon: '/icon/warning.png', color: 'yellow',
      sub: stats.inventory > 0 ? 'Reorder needed' : 'All stocked',
      subColor: stats.inventory > 0 ? '#dc2626' : '#16a34a',
      subIcon: stats.inventory > 0
    ...
    const cardMessages = {
      ...
      icon: '/icon/chat_bubble.png', color: 'blue',
      sub: 'From clients', subColor: 'var(--muted)', to: '/messages',
    };
    const cardAppointments = {
      label: "Today's Appointments", value: stats.appointments,
      icon: '/icon/appointment.png', color: 'blue',
      sub: `${stats.pendingAppts} pending approval`,
      subColor: stats.pendingAppts > 0 ? '#d97706' : 'var(--muted)',
      subIcon: stats.pendingAppts > 0
    ...
    const cardPending = {
      label: 'Pending Approvals', value: stats.pendingAppts,
      icon: '/icon/pending.png', color: 'yellow',
    ...
    const cardSales = {
      label: "Today's Sales", value: `₱${stats.sales.toLocaleString()}`,
      icon: '/icon/money_bag.png', color: 'green',
      sub: 'Revenue today', subColor: '#16a34a', subIcon: <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="19" x2="12" y2="5"/><polyline points="5 12 12 5 19 12"/></svg>,
      to: '/point-of-sale',
    };
    const cardWalkins = {
      label: "Today's Walk-ins", value: stats.walkins,
      icon: '/icon/walkin.png', color: 'blue',
      sub: 'Walk-ins recorded', subColor: 'var(--muted)', to: '/walk-in',
    };

    if (isAdmin) return [cardAppointments, cardPending, cardSales, cardPatients, cardLowStock, cardMessages];
    if (isManager) return [cardAppointments, cardPending, cardSales, cardPatients, cardLowStock, cardMessages];
    return [cardAppointments, cardPending, cardPatients, cardLowStock, cardMessages];
  ...
  if (userLoading) {
    return (
      <Layout>
        <SkeletonStyles />
        <div style={{ width: '100%' }}>
          <div style={{ padding: '20px 32px', paddingTop: 'var(--topbar-h)', boxSizing: 'border-box' }}>
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div className="sk" style={{ width: 280, height: 24, borderRadius: 8, background: 'rgba(255,255,255,0.15)' }} />
                <div className="sk" style={{ width: 360, height: 14, borderRadius: 6, background: 'rgba(255,255,255,0.10)' }} />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end' }}>
                <div className="sk" style={{ width: 200, height: 16, borderRadius: 6, background: 'rgba(255,255,255,0.15)' }} />
                <div className="sk" style={{ width: 110, height: 13, borderRadius: 6, background: 'rgba(255,255,255,0.10)' }} />
              </div>
            </div>
            <div className="sk" style={{ width: 80, height: 11, borderRadius: 4, marginBottom: 14 }} />
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 14, marginBottom: 24 }}>
              {Array.from({ length: 6 }).map((_, i) => <StatCardSkeleton key={i} />)}
            </div>
            <div className="sk" style={{ width: 100, height: 11, borderRadius: 4, marginBottom: 14 }} />
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 12, marginBottom: 24 }}>
              {Array.from({ length: 6 }).map((_, i) => <QuickLinkSkeleton key={i} />)}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
              {[0, 1].map(i => (
                <div key={i} style={{ background: 'var(--card)', border: '1.5px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
                  <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)' }}>
                    <div className="sk" style={{ width: 150, height: 14, borderRadius: 6 }} />
                  </div>
                  {[0,1,2,3].map(j => <AppointmentRowSkeleton key={j} />)}
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{ width: '100%' }}>
        <div style={{
          ...
          <div className="dash-fade-in" style={{
            background: 'linear-gradient(135deg,#0f0c4a 0%,#1e3a8a 55%,#2d5fbf 100%)',
            borderRadius: 18, padding: '28px 32px', marginBottom: 24,
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            ...
            <div style={{ position: 'absolute', bottom: -40, right: 120, width: 160, height: 160, borderRadius: '50%', background: 'rgba(255,255,255,0.03)', pointerEvents: 'none' }} />
            <div style={{ position: 'absolute', top: 20, right: '38%', width: 6, height: 6, borderRadius: '50%', background: 'rgba(125,211,252,0.5)', pointerEvents: 'none' }} />

            <div style={{ position: 'relative', zIndex: 1 }}>
              <p style={{ fontSize: 12, fontWeight: 600, color: 'rgba(125,211,252,0.8)', letterSpacing: 1.5, textTransform: 'uppercase', margin: '0 0 6px' }}>
              ...
              <p style={{ fontSize: 13.5, color: 'rgba(255,255,255,0.55)', margin: '8px 0 0' }}>
                Here's a snapshot of everything happening today.
              </p>
            </div>

            <div style={{ textAlign: 'right', position: 'relative', zIndex: 1 }}>
              <strong style={{ display: 'block', fontSize: 15, color: '#fff', fontWeight: 700, marginBottom: 4 }}>{dateStr}</strong>
              <span style={{
              ...
              : STAT_CARDS.map((sc, i) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <h3 style={{ margin: '4px 0 6px', fontSize: 26, fontWeight: 800, lineHeight: 1 }}>
                      {sc.value}
                    </h3>
                    <span style={{ fontSize: 11, fontWeight: 600, color: sc.subColor, display: 'flex', alignItems: 'center', gap: 4 }}>
                      {sc.subIcon && <span style={{ display: 'flex', alignItems: 'center', flexShrink: 0 }}>{sc.subIcon}</span>}
                      {sc.sub}
                    </span>
                  </div>
                </Link>
                ...
                >
                  <div className="ql-icon-wrap">
                    <img
                      ...
                      style={{
                        width: 22, height: 22,
                        filter: 'brightness(0) saturate(100%) invert(17%) sepia(82%) saturate(1200%) hue-rotate(210deg)',
                      }}
                    />
                  </div>
                  <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--muted)', lineHeight: 1.3 }}>
                    {ql.label}
                  </span>
                </Link>
              ))
          ...
          {/* ── Two bottom cards ──────────────────────────────────────────── */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            <TodayAppointments appts={todayAppts} loading={loading} pendingCount={stats.pendingAppts} />
