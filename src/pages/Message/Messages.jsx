    ...
    const run = async () => {
      ...
      const filtered = (data || [])
        ...
        .filter((p) => {
          ...
          if (normRole(role) === "super_admin" || normRole(p.role) === "super_admin") return true;
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          ...
          {!loading && results.length === 0 && (
            <div style={{ padding: 32, textAlign: "center" }}>
              <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginBottom: 8 }}>
...
const Messages = () => {
  ...
  const fetchClients = useCallback(async () => {
    ...
    try {
      ...
      (sameResult.data || []).forEach((m) => {
        ...
        if (!other) return;
        if (!lastMsgMap[other] || new Date(m.created_at) > new Date(lastMsgMap[other])) {
      ...
      const allPartnerIds = [...new Set([...sameBranchPartnerIds, ...crossBranchPartnerIds])].filter(Boolean);
      ...
      const contacts = allPartnerIds
        ...
        .map((p) => ({
          ...
          isCrossBranch: crossSet.has(p.id)
      ...
      setClients((prev) => {
        const same = prev.length === contacts.length &&
  ...
  const fetchMessages = useCallback(async (partner, keepOptimistic = false) => {
    ...
    const isCrossRead = !!partner.isCrossBranch
      || normRole(currentUser.role) === "super_admin"
    ...
    const [crossResult, sameResult] = await Promise.all([
      supabase.from(CROSS_BRANCH_TABLE).select("*")
      ...
      supabase.from(MESSAGES_TABLE).select("*")
    ...
    const sameMsgs = (sameResult.data || []).map((m) => ({ ...m, _source: "same" }));
    ...
    const globalCh = supabase
      ...
      .on("postgres_changes", {
        ...
        if (msg.recipient_id === currentUser.id || msg.sender_id === currentUser.id) {
          ...
          setSelected((prev) => {
            if (prev && (prev.id === msg.sender_id || prev.id === msg.recipient_id)) {
              fetchMessages(prev);
            }
            return prev;
          });
  ...
  useEffect(() => {
    ...
    supabase.from(CROSS_BRANCH_TABLE).update({ is_read: true })
      .eq("recipient_id", currentUser.id).eq("sender_id", selected.id).eq("is_read", false);
    supabase.from(MESSAGES_TABLE).update({ is_read: true })
      .eq("receiver_id", currentUser.id).eq("sender_id", selected.id).eq("is_read", false);
    setUnread((prev) => ({ ...prev, [selected.id]: 0 }));

    // Subscribe to BOTH tables so manager always sees super_admin messages in real time
    const channel = supabase.channel(`messages-rt-both-${selected.id}`)
      .on("postgres_changes", { event: "*", schema: "public", table: CROSS_BRANCH_TABLE }, (payload) => {
        const msg = payload.new || payload.old;
        if (!msg) return;
        const isRelevant = (msg.sender_id === currentUser.id && msg.recipient_id === selected.id) ||
          (msg.sender_id === selected.id && msg.recipient_id === currentUser.id);
        if (isRelevant) {
          ...
          if (payload.eventType === "UPDATE" || msg.sender_id !== currentUser.id) fetchMessages(selected, true);
          if (fetchClientsTimer.current) clearTimeout(fetchClientsTimer.current);
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: MESSAGES_TABLE }, (payload) => {
        const msg = payload.new || payload.old;
        if (!msg) return;
        const isRelevant = (msg.sender_id === currentUser.id && msg.receiver_id === selected.id) ||
          (msg.sender_id === selected.id && msg.receiver_id === currentUser.id);
        if (isRelevant) {
          if (payload.eventType === "UPDATE" || msg.sender_id !== currentUser.id) fetchMessages(selected, true);
          if (fetchClientsTimer.current) clearTimeout(fetchClientsTimer.current);
          fetchClientsTimer.current = setTimeout(() => fetchClients(), 1500);
  ...
  useEffect(() => {
    ...
    const recompute = async () => {
      ...
      const [{ data: sameUnread }, { data: crossUnread }] = await Promise.all([
        supabase.from(MESSAGES_TABLE).select("sender_id").eq("receiver_id", currentUser.id).eq("is_read", false),
        supabase.from(CROSS_BRANCH_TABLE).select("sender_id").eq("recipient_id", currentUser.id).eq("is_read", false),
      ]);
  ...
  const sendMessage = async () => {
    ...
    const isCrossWrite =
      ...
      (!!selected.isCrossBranch && normBranch(currentUser.branch) !== normBranch(selected.branch || ""));
    ...
    if (error) {
      ...
      setClients((prev) =>
        [...prev]
          .map((c) => c.id === selected.id ? { ...c, lastMessageAt: sentAt } : c)
          .sort((a, b) => new Date(b.lastMessageAt || 0) - new Date(a.lastMessageAt || 0))
      );
    }
  };
  ...
  const handleDeleteConversation = () => {
    ...
    showModal("Delete Conversation", `Permanently delete your conversation with ${selected?.full_name || selected?.email}?`, "confirm",
      async () => {
        ...
        if (crossIds.length) await supabase.from(CROSS_BRANCH_TABLE).delete().in("id", crossIds);
        if (sameIds.length) await supabase.from(MESSAGES_TABLE).delete().in("id", sameIds);
 ...
 const mergedContacts = useMemo(() => [
  ...
  ].sort((a, b) => new Date(b.lastMessageAt || 0) - new Date(a.lastMessageAt || 0)), [clients, crossContacts]);
  ...
  return (
    <Layout>
      ...
      <div className="msg-page" style={{ position: "fixed", top: 68, left: 0, right: 0, bottom: 0, display: "grid", gridTemplateColumns: isMobile ? "1fr" : "300px 1fr", background: "var(--bg)", overflow: "hidden" }}>

        {/* ══ LEFT SIDEBAR ══════════════════════════════════════════════════════ */}
        <div style={{ display: isMobile && mobileView === "chat" ? "none" : "flex", flexDirection: "column", background: "var(--card)", borderRight: "1.5px solid var(--border)", minHeight: 0, overflow: "hidden" }}>
          <div style={{ padding: "18px 18px 12px", borderBottom: "1px solid var(--border)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
              <Avatar name={currentUser?.name} size={40} me />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 700, fontSize: 14, color: "var(--text)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{currentUser?.name || "Loading…"}</div>
                <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 1 }}>Messages</div>              </div>
              {totalUnread > 0 && (
                <div style={{ background: "#ef4444", color: "#fff", borderRadius: 99, fontSize: 10, fontWeight: 700, minWidth: 18, height: 18, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 5px", flexShrink: 0 }}>
            ...
            <div style={{ position: "relative" }}>
             ...
             <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search conversations…"
                style={{ width: "100%", padding: "9px 12px 9px 32px", borderRadius: 10, border: "1.5px solid var(--border)", background: "var(--bg)", fontSize: 13, color: "var(--text)", outline: "none", fontFamily: "inherit" }} />
            </div>
          </div>

           <div style={{ padding: "10px 18px 6px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.6px" }}>Conversations</span>
              {filteredClients.length > 0 && <span style={{ fontSize: 11, color: "var(--muted)" }}>{filteredClients.length}</span>}
            </div>
            <button onClick={() => setShowAdd(true)}
              style={{ width: 24, height: 24, borderRadius: "50%", border: "none", background: "linear-gradient(135deg,#6366f1,#8b5cf6)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.8" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
            </button>
          </div>
          ...
          <div className="sidebar-scroll" style={{ flex: 1, overflowY: "auto" }}>
            {loading ? (
              ...
              <div style={{ padding: 28, textAlign: "center" }}>
               <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#c7d2fe" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginBottom: 10 }}>
              ...
              return (
                <div key={c.id} className={`client-row ${isActive ? "active" : ""}`}
                  ...
                  <Avatar name={c.full_name || c.email} size={44} online={onlineIds.has(c.id)} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: badge > 0 ? 700 : 600, fontSize: 13.5, color: "var(--text)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", display: "flex", alignItems: "center" }}>
                    ...
                    <div style={{ fontSize: 11.5, color: badge > 0 ? "#6366f1" : "var(--muted)", marginTop: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontWeight: badge > 0 ? 600 : 400 }}>
                      {c.email || "—"}
                    </div>
                  </div>
                  {badge > 0 && (
        ...
        {/* ══ RIGHT CHAT PANE ═══════════════════════════════════════════════════ */}
        <div style={{ display: isMobile && mobileView === "list" ? "none" : "flex", flexDirection: "column", overflow: "hidden", minHeight: 0 }}>
          {!selected ? (
           <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, background: "var(--bg)" }}>              <div style={{ width: 90, height: 90, borderRadius: "50%", background: "linear-gradient(135deg,#ede9fe,#dbeafe)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              ...
              <div style={{ textAlign: "center" }}>
               <div style={{ fontSize: 18, fontWeight: 700, color: "var(--text)", marginBottom: 6 }}>Your Messages</div>
                <div style={{ fontSize: 13, color: "var(--muted)", marginBottom: 20 }}>Select a conversation or start a new one</div>
                <button onClick={() => setShowAdd(true)}
            ...
            <>
             <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 20px", background: "var(--card)", borderBottom: "1.5px solid var(--border)", flexShrink: 0 }}>
                {isMobile && (
                ...
                <Avatar name={selected.full_name || selected.email || "?"} size={44} online={onlineIds.has(selected.id)} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: 15, color: "var(--text)", display: "flex", alignItems: "center" }}>
              ...
              <div className="chat-scroll" style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 2, background: "#f4f6fb", minHeight: 0 }}>
                {messages.length === 0 && !deleting && (
                  <div style={{ margin: "auto", textAlign: "center" }}>
                     <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#c7d2fe" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginBottom: 8 }}>
                    ...
                    <div key={item.id} className="bubble" style={{ display: "flex", justifyContent: me ? "flex-end" : "flex-start", alignItems: "flex-end", gap: 8, marginTop: 3 }}>
                      ...
                      <div style={{ maxWidth: "62%" }}>
                        ...
                        <div style={{ fontSize: 10.5, color: "#9ca3af", marginTop: 4, display: "flex", alignItems: "center", gap: 4, justifyContent: me ? "flex-end" : "flex-start" }}>
                          ...
                          {me && !item._pending && (
                            item.is_read ? (
                              <span style={{ color: "#6366f1", fontWeight: 600 }}>Seen</span>
                            ) : (
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2.5"><path d="M20 6 9 17l-5-5" /></svg>
                            )
                          )}
                        </div>
                      </div>
              ...
              <div style={{ background: "#fff", borderTop: "1.5px solid #eef0f6", flexShrink: 0, zIndex: 2 }}>
                <div className="emoji-quick" style={{ padding: "8px 18px 0", display: "flex", gap: 2 }}>
                  {["👍", "❤️", "😊", "😂", "🙏", "👋", "✅", "🐾"].map((text) => (
                    <span key={text} title={text} onClick={() => setNewMsg((p) => p + text)}
                      style={{ width: 32, height: 32, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 18, transition: "background 0.12s, transform 0.1s" }}
                      onMouseOver={(e) => { e.currentTarget.style.background = "#f0f0f6"; }}
                      onMouseOut={(e) => { e.currentTarget.style.background = "transparent"; }}>
                      {text}
                    </span>
                  ))}
