...
const generateEmail = (firstName, lastName = "", role = "", existingEmails = [], branchAbbr = "") => {
  const cleanFirst = (firstName || "").trim().toLowerCase().replace(/\s+/g, "").replace(/[^a-z0-9]/g, "");
  const cleanLast = (lastName || "").trim().toLowerCase().replace(/\s+/g, "").replace(/[^a-z0-9]/g, "");
  const base = cleanFirst && cleanLast ? `${cleanFirst}${cleanLast}` : cleanFirst || cleanLast || "user";
  const prefix = branchAbbr ? `${branchAbbr}.` : "";
  const domain = role === "Customer" ? `${prefix}customer.com`
    : role === "Manager" ? `${prefix}manager.com`
      : `${prefix}employee.com`;
    ...
    <>
      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 1055, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, pointerEvents: "none" }}>
        <div style={{ width: "100%", maxWidth: 480, pointerEvents: "all" }}>
          <div style={{ background: "var(--card)", borderRadius: 16, overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
            ...
            <div style={{ padding: "12px 20px 16px", display: "flex", justifyContent: "flex-end", gap: 8 }}>
              <button onClick={onCancel} style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 8, padding: "8px 18px", fontFamily: "inherit", fontSize: 13, fontWeight: 600, cursor: "pointer", color: "var(--text)" }}>Cancel</button>
...
const ManagerControl = () => {
  ...
  const { user, isAdmin, isSuperAdmin, seeAllBranches, loading: userLoading } = useCurrentUser();
  ...
  const branchAbbr = branchName
    ? branchName.toLowerCase().replace(/\s+/g, "").replace(/[^a-z0-9]/g, "").slice(0, 3)
    : "";
 ...
 const [saving, setSaving] = useState(false);
  ...
  const [showDeletedModal, setShowDeletedModal] = useState(false);  const [showAddModal, setShowAddModal] = useState(false);
  const [showPassword, setShowPassword] = useState(false);
  const [addErrors, setAddErrors] = useState({});
  const [successModal, setSuccessModal] = useState(null);
  const [addForm, setAddForm] = useState({
    first_name: "", last_name: "", email: "", password: "",
    role: "Employee", status: "Pending Approval", sex: "Male", emailLocked: false,
  });
  ...
  const fetchUsers = useCallback(async () => {
    if (!user) return;
    setLoading(true);
    let q = supabase.from("profiles").select("*").is("deleted_at", null).order("created_at", { ascending: false });
    if (!seeAllBranches && user.branchId) q = q.eq("branch_id", user.branchId);
    const { data, error } = await q;
    if (!error) { const fetched = data || []; setUsers(fetched); }
    setLoading(false);
  }, [user, seeAllBranches]);
  ...
  const fetchDeletedUsers = useCallback(async () => {
    if (!user) return;
    let q = supabase.from("profiles").select("*").not("deleted_at", "is", null).order("deleted_at", { ascending: false });
    if (!seeAllBranches && user.branchId) q = q.eq("branch_id", user.branchId);
    const { data, error } = await q;
    if (error) return;
    const now = Date.now();
    const expired = (data || []).filter(u => now - new Date(u.deleted_at).getTime() > THIRTY_DAYS_MS);
    if (expired.length > 0) await supabase.from("profiles").delete().in("id", expired.map(u => u.id));
  ...
  const fetchPending = useCallback(async () => {
    ...
    const pendingSub = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'pending_users' }, (payload) => {
        ...
        setPending(prev => prev.find(r => r.id === req.id) ? prev : [req, ...prev]);
      })
    ...
    const profilesSub = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'profiles' }, (payload) => {
        const u = payload.new;
        if (!seeAllBranches && String(u.branch_id) !== String(user.branchId)) return;
        setUsers(prev => prev.find(p => p.id === u.id) ? prev : [u, ...prev]);
      })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'profiles' }, (payload) => {
        const u = payload.new;
        if (!seeAllBranches && String(u.branch_id) !== String(user.branchId)) return;
        setUsers(prev => prev.find(p => p.id === u.id)
  ...
  }, [fetchUsers, fetchPending, userLoading, user, seeAllBranches, branchLabel]);
  ...
  const filteredLogs = logs.filter(l => {
    ...
    const isOwnLog = l.user_id === user?.id || (l.user_name || "").toLowerCase() === (user?.fullName || user?.email || "").toLowerCase();
    return isOwnLog &&
      (!logSearch || (l.user_name || "").toLowerCase().includes(q) || (l.action || "").toLowerCase().includes(q) || (l.details || "").toLowerCase().includes(q)) &&
  ...
  const openAddModal = () => {
    ...
    setAddForm({ first_name: "", last_name: "", email: "", password: pwd, role: "Employee", status: "Pending Approval", sex: "Male", emailLocked: false });
  ...
  const handleFirstNameChange = (rawVal) => {
    ...
    setAddForm(f => ({ ...f, first_name: val, email: f.emailLocked ? f.email : generateEmail(val, f.last_name, f.role, existingEmails, branchAbbr) }));
  ...
  const handleSubmitRequest = async () => {
    ...
    if (addForm.role === "Customer") {
      const { data: signUpData, error: signUpError } = await supabase.auth.signUp({
        ...
        options: { data: { full_name: `${addForm.first_name} ${addForm.last_name}`, role: "customer" } },
      });
      if (signUpError) {
        setSaving(false);
      ...
      fetchUsers();
      setSuccessModal({ name: `${addForm.first_name} ${addForm.last_name}`, approved: true });
    ...
    setSaving(false);
    if (error) { setSuccessModal({ error: true, message: error.message }); return; }
    setShowAddModal(false);
    fetchPending();
  ...
  const handleApprove = (req) => {
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        if (error) { setSuccessModal({ error: true, message: error.message }); return; }
        if (newProfile) setUsers(prev => prev.find(p => p.id === newProfile.id) ? prev : [newProfile, ...prev]);
        ...
        setSuccessModal({ name: `${req.first_name} ${req.last_name}`, approved: true });
      },
    });
  };
  ...
  const handleReject = (req) => {
    ...
    setConfirm({
      ...
      onConfirm: async () => {
        setConfirm(null);
        const { error } = await supabase.from("profiles").update({ deleted_at: new Date().toISOString() }).eq("id", u.id);
        if (error) { setSuccessModal({ error: true, message: error.message }); return; }
  ...
  const restoreUser = (u) => {
    setConfirm({
      ...
      type: "success", confirmLabel: "Restore",
      onConfirm: async () => {
        setConfirm(null);
      ...
      <div style={{ padding: "40px 28px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, marginBottom: 24 }}>
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} style={{ borderRadius: 16, border: "1.5px solid var(--border)", padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ width: 46, height: 46, borderRadius: 12, background: "#e5e7eb", animation: "pulse 1.5s ease-in-out infinite" }} />
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <div style={{ width: 80, height: 11, borderRadius: 6, background: "#e5e7eb", animation: "pulse 1.5s ease-in-out infinite" }} />
                <div style={{ width: 50, height: 26, borderRadius: 6, background: "#e5e7eb", animation: "pulse 1.5s ease-in-out infinite" }} />
                <div style={{ width: 100, height: 11, borderRadius: 6, background: "#e5e7eb", animation: "pulse 1.5s ease-in-out infinite" }} />
              </div>
            </div>
          ))}
        </div>
        <div style={{ borderRadius: 12, border: "1.5px solid var(--border)", padding: 20, display: "flex", flexDirection: "column", gap: 12 }}>
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "2fr 2fr 1fr 1fr 1fr", gap: 12, paddingBottom: 12, borderBottom: "1px solid var(--border)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#e5e7eb", animation: "pulse 1.5s ease-in-out infinite" }} />
                <div style={{ width: 110, height: 13, borderRadius: 6, background: "#e5e7eb", animation: "pulse 1.5s ease-in-out infinite" }} />
              </div>
              {[130, 60, 60, 70].map((w, j) => (
                <div key={j} style={{ width: w, height: 13, borderRadius: 6, background: "#e5e7eb", animation: "pulse 1.5s ease-in-out infinite", alignSelf: "center" }} />
              ))}
            </div>
          ))}
        </div>
    ...
    <Layout>
      <div className="mc-page">
        <div className="mc-topbar mc-topbar-pos">
          <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
            <img src="/icon/manager.png" alt="" style={{ width: 22, height: 22, flexShrink: 0, filter: "brightness(0) saturate(100%) invert(17%) sepia(82%) saturate(1200%) hue-rotate(210deg)" }} />
            <div style={{ minWidth: 0 }}>
              <h1 style={{ fontSize: 20, fontWeight: 800, color: "var(--text)", margin: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>Manager Control</h1>
              <p className="mc-subtitle" style={{ fontSize: 13, color: "var(--muted)", margin: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{branchLabel} · User management &amp; access logs</p>
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
            <button
              onClick={() => setShowDeletedModal(true)}
              style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "#fef2f2", border: "1.5px solid #fca5a5", color: "#dc2626", borderRadius: 8, padding: "6px 12px", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}
            >
            ...
            <span style={{ fontSize: 11, fontWeight: 700, padding: "4px 10px", borderRadius: 6, background: "#dbeafe", color: "#1e3a8a", border: "1px solid #bfdbfe", whiteSpace: "nowrap" }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" style={{ marginRight: 4 }}><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>
        ...
        <div style={{ position: "fixed", bottom: 28, right: 28, zIndex: 999, display: "flex", alignItems: "center", gap: 10 }}
          ...
          <span className="fab-tooltip" style={{
            ...
            <span style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.2 }}>
              <span style={{ fontSize: 12, fontWeight: 800, color: '#fff' }}>Request New User</span>
              <span style={{ fontSize: 10, fontWeight: 500, color: 'rgba(255,255,255,0.55)' }}>Send for Admin approval</span>
            </span>
            <span style={{
          ...
          <button onClick={openAddModal} className="fab-btn" style={{ width: 56, height: 56, borderRadius: "50%", background: "linear-gradient(135deg,#1e3a8a,#3b82f6)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 20px rgba(30,58,138,0.4)", transition: "transform 0.2s, box-shadow 0.2s", flexShrink: 0 }}>
        ...
        <div className="mc-content mc-content-pos">
          {/* Stat cards */}
          <div className="mc-stats-grid" style={{ display: "grid", gap: 16, marginBottom: 24 }}>
            {[
              { label: "Total Users", value: counts.total, icon: "/icon/total_user.png", color: "blue", sub: "All accounts" },
              { label: "Active", value: counts.active, icon: "/icon/active_acc.png", color: "green", sub: "Currently active" },
              { label: "Pending Approval", value: counts.pending, icon: "/icon/pending.png", color: "yellow", sub: counts.pending > 0 ? "Needs attention" : "All cleared" },
              { label: "Staff", value: counts.staff, icon: "/icon/staff.png", color: "purple", sub: "Non-customer users" },
            ].map((sc, i) => (
              <div key={i} className={`stat-card-v2 ${sc.color}`} onClick={() => sc.color === "yellow" && setTab("pending")} style={{ cursor: sc.color === "yellow" ? "pointer" : "default" }}>
                <div style={{ display: "flex", alignItems: "flex-start" }}>
                  <div className={`stat-icon-v2 ${sc.color}`}>
                    <img src={sc.icon} alt={sc.label} style={{ width: 24, height: 24 }} />
                  </div>
                </div>
                <div>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 600, color: "var(--muted)" }}>{sc.sub}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          {pending.filter(r => r.status === "pending").length > 0 && (
            <div style={{ background: "#fffbeb", border: "1.5px solid #fde68a", borderRadius: 10, padding: "12px 20px", marginBottom: 20, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  <p style={{ margin: 0, fontWeight: 700, fontSize: 14, color: "#92400e" }}>{pending.filter(r => r.status === "pending").length} Account Request{pending.filter(r => r.status === "pending").length > 1 ? "s" : ""} Awaiting Admin Approval</p>
                  <p style={{ margin: 0, fontSize: 12, color: "#b45309" }}>These requests need an Admin to approve before accounts are created.</p>
                </div>
              </div>
              <button className="btn mc-btn-auto" style={{ background: "#d97706", color: "#fff", border: "none", fontSize: 13, whiteSpace: "nowrap" }} onClick={() => setTab("pending")}>View Requests</button>
            </div>
          )}
          ...
          <div style={{ background: "var(--card)", borderRadius: "var(--radius-lg)", border: "1px solid var(--border)", marginBottom: 20 }}>
            <div style={{ display: "flex", borderBottom: "1px solid var(--border)", padding: "0 20px", overflowX: "auto" }}>
              {[
                ...
                { key: "pending", label: `Pending Requests${pending.length > 0 ? ` (${pending.length})` : ""}` },
            ...
            {tab === "users" && (
              <div style={{ padding: 20 }}>
                <div className="mc-search-row" style={{ display: "flex", gap: 10, marginBottom: 16, alignItems: "center", flexWrap: "wrap" }}>
                  ...
                  <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)} className="mc-input" style={{ width: 140 }}>
                ...
                <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 8, padding: "10px 14px", marginBottom: 14, fontSize: 12, color: "#1e40af", display: "flex", alignItems: "center", gap: 8 }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#1e40af" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>
                  ...
                  {loading ? (
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {Array.from({ length: 5 }).map((_, i) => (
                        <div key={i} style={{ display: "grid", gridTemplateColumns: "2fr 2fr 1fr 1fr 1fr", gap: 12, padding: "12px 14px", borderBottom: "1px solid var(--border)" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#e5e7eb", animation: "pulse 1.5s ease-in-out infinite" }} />
                            <div style={{ width: 100, height: 13, borderRadius: 6, background: "#e5e7eb", animation: "pulse 1.5s ease-in-out infinite" }} />
                          </div>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                      <thead><tr>{["User", "Email", "Sex", "Role", "Status", "Joined", "Actions"].map(h => <th key={h} className="mc-th">{h}</th>)}</tr></thead>
                      <tbody>
                        {paginated.length === 0 ? (
                            ...
                            <td className="mc-td">
                              {u.sex ? (
                                <span style={{
                                  ...
                                  background: u.sex === "Male" ? "#eff6ff" : u.sex === "Female" ? "#fdf2f8" : "#f3f4f6",
                                  color: u.sex === "Male" ? "#1d4ed8" : u.sex === "Female" ? "#be185d" : "#6b7280",
                                  border: `1px solid ${u.sex === "Male" ? "#bfdbfe" : u.sex === "Female" ? "#fbcfe8" : "#e5e7eb"}`,
                                }}>
                                  {u.sex === "Male" ? "♂" : u.sex === "Female" ? "♀" : ""} {u.sex}
                                </span>
                              ) : <span style={{ color: "var(--muted)", fontSize: 12 }}>—</span>}
                            </td>
                            <td className="mc-td"><span className={`badge ${ROLE_BADGE[u.role] || "badge-gray"}`}>{u.role || "—"}</span></td>
                            <td className="mc-td"><span className={`badge ${u.status === "Active" ? "badge-green" : "badge-red"}`}>{u.status || "Active"}</span></td>
                             <td className="mc-td"><span style={{ fontSize: 12, color: "var(--muted)" }}>{fmtDate(u.created_at)}</span></td>
                            <td className="mc-td">
                               <button
                                title="Delete user — moves to Recently Deleted"
                                onClick={() => handleRequestDelete(u)}
                                style={{ background: "none", border: "1px solid #fca5a5", borderRadius: 6, width: 30, height: 30, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#dc2626" }}
                              >
              ...
              <div style={{ padding: 24 }}>
                <div style={{ marginBottom: 20 }}>
                  ...
                  <p style={{ fontSize: 13, color: "var(--muted)", margin: 0 }}>
                    View-only — requests submitted by this branch awaiting Admin approval.
                  </p>
                </div>
                {pending.length === 0 ? (
                  <div style={{ padding: "40px 0", textAlign: "center" }}>
                    <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#f0fdf4", border: "1.5px solid #86efac", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px" }}><svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12" /></svg></div>
                    <p style={{ color: "var(--muted)", fontSize: 14 }}>No pending requests — all caught up!</p>
                  </div>
                ) : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    {pending.map(req => (
                      <div key={req.id} style={{ background: "#fffbeb", border: "1.5px solid #fde68a", borderRadius: 12, padding: "16px 18px", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                        <Avatar firstName={req.first_name} lastName={req.last_name} role={req.role || "Employee"} size={42} />
                        <div style={{ flex: 1 }}>
                          <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: "#1e293b" }}>{[req.first_name, req.last_name].filter(Boolean).join(" ") || "—"}</p>
                          <p style={{ margin: 0, fontSize: 12, color: "#64748b" }}>{req.email}</p>
                          <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                            ...
                            <span style={{
                              fontSize: 10, borderRadius: 99, padding: "1px 8px", fontWeight: 700,
                              ...
                              {req.status === "approved" ? <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}><svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"><polyline points="20 6 9 17 4 12" /></svg>Approved</span> : req.status === "rejected" ? <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}><svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>Rejected</span> : <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}><svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>Awaiting Approval</span>}
                            </span>
                          </div>
                          <p style={{ margin: "4px 0 0", fontSize: 11, color: "#94a3b8" }}>Requested {fmtDate(req.created_at)} · {req.branch}</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {tab === "logs" && (
              <div style={{ padding: 20 }}>
                ...
                <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
                  ...
                  <select value={logRole} onChange={e => setLogRole(e.target.value)} className="mc-input" style={{ width: 140 }}>
                    <option value="">All Roles</option>
                    <option>Admin</option><option>Manager</option><option>Employee</option><option>Customer</option>
                  </select>
                </div>
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    <thead><tr>{["User", "Role", "Action", "Time", "Status"].map(h => <th key={h} className="mc-th">{h}</th>)}</tr></thead>
                    <tbody>
                      {filteredLogs.length === 0 ? (
                        <tr><td colSpan={5} style={{ textAlign: "center", padding: 40, color: "var(--muted)" }}>No logs found</td></tr>
                      ) : paginatedLogs.map(l => (
                        <tr key={l.id}>
                          <td className="mc-td">
                          ...
                          <td className="mc-td"><span className={`badge ${ROLE_BADGE[l.user_role] || "badge-gray"}`}>{l.user_role || "—"}</span></td>
                          <td className="mc-td">
                          ...
                          <td className="mc-td"><span style={{ fontSize: 12, color: "var(--muted)" }}>{l.created_at ? fmtTime(new Date(l.created_at)) : "—"}</span></td>
                          <td className="mc-td">
                ...
                {logsTotalPages > 1 && (
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 4, padding: "14px 18px", borderTop: "1px solid var(--border)", marginTop: 8 }}>
                    ...
                    {Array.from({ length: logsTotalPages }, (_, i) => i + 1).map(pg => (
                ...
                <div style={{ marginTop: 6, fontSize: 12, color: "var(--muted)", display: "flex", justifyContent: "space-between", padding: "0 4px" }}>
                  ...
                  <button className="btn btn-ghost btn-sm mc-btn-auto" onClick={fetchLogs}>↻ Refresh</button>
                </div>
              </div>
            )}
          </div>
      ...
      {showAddModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: 16 }}>
          <div style={{ background: "var(--card)", borderRadius: 14, width: "100%", maxWidth: 500, maxHeight: "90vh", overflowY: "auto", display: "flex", flexDirection: "column", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
            <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: "#fffbeb", border: "1.5px solid #fde68a", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#d97706" strokeWidth="2" strokeLinecap="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><line x1="19" y1="8" x2="19" y2="14" /><line x1="22" y1="11" x2="16" y2="11" /></svg>
                </div>
                <div>
                  <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: "#1e293b", lineHeight: 1.3, fontFamily: "inherit" }}>Request New User Account</h3>
                  <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 4 }}>
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#d97706" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>
                    <span style={{ fontSize: 12, color: "#d97706", fontWeight: 600, fontFamily: "inherit" }}>Will be sent to Admin for approval</span>
                  </div>
                </div>
              </div>
              <button onClick={() => setShowAddModal(false)} style={{ background: "#f3f4f6", border: "none", borderRadius: 8, width: 30, height: 30, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#64748b", fontSize: 16, flexShrink: 0 }}>✕</button>
            </div>
            <div className="modal-body" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div className="form-group" style={{ margin: 0 }}>
                  ...
                  <input type="text" placeholder="e.g. Jane" value={addForm.first_name} onChange={e => handleFirstNameChange(e.target.value)} className={addErrors.first_name ? "mc-input-err" : ""} />
                  {addErrors.first_name && <div className="mc-err-msg">{addErrors.first_name}</div>}
                </div>
                <div className="form-group" style={{ margin: 0 }}>
                  <label>Last Name <span style={{ color: "#dc2626" }}>*</span></label>
                  <input type="text" placeholder="e.g. Doe" value={addForm.last_name} onChange={e => { const val = sanitizeName(e.target.value); const existingEmails = users.map(u => u.email?.toLowerCase()).filter(Boolean); setAddForm(f => ({ ...f, last_name: val, email: f.emailLocked ? f.email : generateEmail(f.first_name, val, f.role, existingEmails, branchAbbr) })); setAddErrors(er => ({ ...er, last_name: "" })); }} className={addErrors.last_name ? "mc-input-err" : ""} />
                  {addErrors.last_name && <div className="mc-err-msg">{addErrors.last_name}</div>}
                </div>
              </div>
              ...
              <div className="form-group" style={{ margin: 0 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  ...
                  {!addForm.emailLocked && addForm.email && <span style={{ fontSize: 11, color: "#16a34a", fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="3" /><path d="M12 1v4M12 19v4M4.22 4.22l2.83 2.83M16.95 16.95l2.83 2.83M1 12h4M19 12h4M4.22 19.78l2.83-2.83M16.95 7.05l2.83-2.83" /></svg> Auto-generated</span>}
                  {addForm.emailLocked && (
                    <button type="button" onClick={() => { const existing = users.map(u => u.email?.toLowerCase()).filter(Boolean); setAddForm(f => ({ ...f, email: generateEmail(f.first_name, f.last_name, f.role, existing, branchAbbr), emailLocked: false })); }}
                      style={{ fontSize: 11, color: "#2563eb", background: "none", border: "none", cursor: "pointer", fontWeight: 600, padding: 0 }}>↻ Reset to auto</button>
                  )}
                </div>
                <input type="email" placeholder="Type first name to auto-generate…" value={addForm.email} onChange={e => handleEmailChange(e.target.value)} className={addErrors.email ? "mc-input-err" : ""} />
                {addErrors.email && <div className="mc-err-msg">{addErrors.email}</div>}
              </div>

              <div className="form-group" style={{ margin: 0 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  <label style={{ margin: 0 }}>Suggested Password <span style={{ color: "#dc2626" }}>*</span></label>
                  <span style={{ fontSize: 11, color: "#16a34a", fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg> Auto-generated</span>
                </div>
                <div style={{ position: "relative" }}>
                  <input type={showPassword ? "text" : "password"} value={addForm.password} onChange={e => { setAddForm(f => ({ ...f, password: e.target.value })); setAddErrors(er => ({ ...er, password: "" })); }}
                    className={addErrors.password ? "mc-input-err" : "mc-input"}
                    style={{ width: "100%", boxSizing: "border-box", fontFamily: showPassword ? "monospace" : "inherit", paddingRight: 88 }} />
                  <button type="button" onClick={() => setShowPassword(v => !v)} style={{ position: "absolute", right: 46, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "var(--muted)", padding: "0 4px", display: "flex", alignItems: "center" }}>
                    {showPassword ? (
                ...
                {addErrors.password && <div className="mc-err-msg">{addErrors.password}</div>}
              ...
              <div className="form-group" style={{ margin: 0 }}>
                ...
                <select value={addForm.sex} onChange={e => { setAddForm(f => ({ ...f, sex: e.target.value })); setAddErrors(er => ({ ...er, sex: "" })); }} className={addErrors.sex ? "mc-input-err" : ""}>
                  <option>Male</option><option>Female</option>
                </select>
                {addErrors.sex && <div className="mc-err-msg">{addErrors.sex}</div>}
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div className="form-group" style={{ margin: 0 }}>
                  ...
                  <select value={addForm.role} onChange={e => {
                    ...
                    const existing = users.map(u => u.email?.toLowerCase()).filter(Boolean);
                    setAddForm(f => ({ ...f, role: newRole, email: f.emailLocked ? f.email : generateEmail(f.first_name, f.last_name, newRole, existing, branchAbbr) }));
                    setAddErrors(er => ({ ...er, role: "" }));
                  }} className={addErrors.role ? "mc-input-err" : ""}>
                    <option>Employee</option><option>Manager</option><option>Customer</option>
                  </select>
                  {addErrors.role && <div className="mc-err-msg">{addErrors.role}</div>}
                </div>
                <div className="form-group" style={{ margin: 0 }}>
                  <label>Branch</label>
                  <input type="text" value={branchLabel} readOnly className="mc-input" style={{ background: "var(--bg)", color: "var(--muted)" }} />
                </div>
              </div>
            ...
            <div className="modal-footer">
              <button className="btn btn-ghost mc-btn-auto" onClick={() => setShowAddModal(false)}>Cancel</button>
              <button className="btn mc-btn-auto" style={{ background: "#d97706", color: "#fff", border: "none", opacity: (!isRequestFormValid() || saving) ? 0.5 : 1, cursor: (!isRequestFormValid() || saving) ? "not-allowed" : "pointer" }} onClick={handleSubmitRequest} disabled={saving || !isRequestFormValid()}>
                {saving ? "Submitting…" : 
                <span style={{ display: "flex", alignItems: "center", gap: 6 }}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg> Submit for Approval</span>}
              </button>
            </div>
          </div>
        </div>
      ...
      {successModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }}>
          <div style={{ background: "var(--card)", borderRadius: 16, padding: 32, maxWidth: 400, width: "100%", textAlign: "center", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
            <div style={{ width: 56, height: 56, borderRadius: "50%", background: successModal.error ? "#fef2f2" : "#f0fdf4", border: `1.5px solid ${successModal.error ? "#fca5a5" : "#86efac"}`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px" }}>{successModal.error ? <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#dc2626" strokeWidth="2.5" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg> : <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12" /></svg>}</div>
           <h3 style={{ fontSize: 18, fontWeight: 800, color: "#1e293b", margin: "0 0 8px" }}>
              {successModal.error ? "Action Not Allowed" : successModal.approved ? "Account Created!" : successModal.deletion ? "Deletion Request Sent!" : "Request Submitted!"}
            </h3>
            <p style={{ fontSize: 14, color: "#64748b", margin: "0 0 24px" }}>
             ...
             <button onClick={() => setSuccessModal(null)} style={{ background: successModal.error ? "#dc2626" : "#2563eb", color: "#fff", border: "none", borderRadius: 8, padding: "10px 32px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
      ...
      {showDeletedModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: 16 }}>
          <div style={{ background: "var(--card)", borderRadius: 14, width: "100%", maxWidth: 560, maxHeight: "80vh", display: "flex", flexDirection: "column", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
            <div style={{ padding: "18px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: "var(--text)" }}>Recently Deleted</h3>
            ...
            <div style={{ padding: "14px 20px", borderTop: "1px solid var(--border)", display: "flex", justifyContent: "flex-end" }}>
              <button className="btn btn-ghost mc-btn-auto" onClick={() => setShowDeletedModal(false)}>Close</button>
            </div>
          </div>
        </div>
      )}
