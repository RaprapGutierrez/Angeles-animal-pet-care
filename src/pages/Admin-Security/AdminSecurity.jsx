...
const Skel = ({ w = "100%", h = 16, style }) => (
  <span
    className="skel"
    style={{ width: w, height: h, borderRadius: 8, display: "block", ...style }}
  />
);
      ...
      <div className="usr-confirm-wrap" style={{ zIndex: 10051 }}>
        <div className="usr-confirm-box">
          <div className="usr-confirm-card">
            <div
              className="usr-confirm-header"
              style={{ backgroundColor: color }}
            >
              <h5 className="usr-confirm-title">{title}</h5>
              <button onClick={onCancel} className="usr-confirm-close">
                ✕
              </button>
            </div>
            ...
            <div className="usr-confirm-footer">
              <button onClick={onCancel} className="usr-confirm-cancel">
                Cancel
              </button>
              <button
                onClick={onConfirm}
                className="usr-confirm-confirm"
                style={{ backgroundColor: color }}
              >
                {confirmLabel}
              </button>
    ...
    <div className="usr-modal-overlay z-1200">
      <div className="usr-modal-box max-w-440">
        <div className="usr-cred-header">
          <div className="usr-cred-header-row">
            ...
            <div>
              ...
              <p className="usr-cred-header-sub">
        ...
        <div className="usr-cred-body">
          {rows.map(([label, value]) => (
            <div key={label} className="usr-cred-field">
              ...
              <div
                className={`usr-cred-field-value${label.includes("Password") ? " mono" : ""}`}
              >
                {value}
              </div>
            </div>
          ...
          {credentials.pendingVerification && (
            <div className="usr-cred-info">
              ...
              <span>
                This address must be verified by entering the 6-digit code sent
                to it. The account stays inactive until verification is
                complete.
              </span>
            </div>
          )}
        ...
        <div className="usr-cred-footer">
          <button className="btn btn-ghost usr-w-auto" onClick={copyAll}>
            {copied ? "✓ Copied!" : "Copy All"}
          </button>
          <button className="btn btn-primary usr-w-auto" onClick={onClose}>
            Done
          </button>
        </div>
      </div>
...
const LiveToast = ({ message, show, type = "success" }) => {
  ...
  return (
    <div className={`usr-toast ${show ? "show" : "hide"}`}>
      ...
      <div className="usr-toast-body">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ marginBottom: 5 }}>
            <span
              className="usr-toast-label"
              style={{ color: c.labelColor, background: c.labelBg }}
            >
              {c.label}
            </span>
          </div>
      ...
      <div
        className="usr-toast-progress-track"
        style={{ background: `${c.accent}22` }}
      >
        <div
          className="usr-toast-progress-fill"
          style={{
            background: c.accent,
            width: show ? "0%" : "100%",
      ...
      >
        ...
        >
          ...
          >
            ...
            <path d="M2 7l8.5 6a2 2 0 0 0 3 0L22 7" />
          </svg>
        </div>
        <h3
          style={{
...
const AdminSecurity = () => {
  ...
  const fetchPwdRequests = useCallback(async () => {
    ...
    if (!canSeeAllBranches && currentUser?.branchId && rows.length) {
      ...
      rows = rows.filter(
        (r) => branchById.get(r.user_id) === currentUser.branchId,
      );
    }
    setPwdRequests(rows);
  ...
  const fetchDeletedUsers = useCallback(async () => {
    ...
    if (!canSeeAllBranches && currentUser.branchId)
      query = query.eq("branch_id", currentUser.branchId);
    else if (canSeeAllBranches && branchFilter)
      query = query.eq("branch_id", branchFilter);
  ...
  const syncMissingOwnerAccounts = async () => {
    ...
    try {
      ...
      if (patErr) {
        alert("Error fetching patients: " + patErr.message);
      ...
      if (targets.length === 0) {
        showToast("No patient owners found", "info");
        setSyncingOwners(false);
        return;
      }
      ...
      for (const p of targets) {
        ...
        if (!userId) {
          ...
          if (existingProfile?.id) {
            ...
            if (
              ...
              (existingProfile.role || "").toLowerCase() !== "customer"
            )
              needsFix = true;
          } else {
        ...
        if (userId && userId !== p.owner_user_id) {
          await supabase
            .from("patients")
            .update({ owner_user_id: userId, owner_email: ownerEmail })
            .eq("id", p.id);
        }
      }
      ...
      showToast(
        `Synced owners — created ${created}, fixed ${fixed}, skipped ${skipped}`,
      );
      fetchUsers();
    } catch (err) {
  ...
  useEffect(() => {
    const profilesSub = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (!canSeeAllBranches && newUser.branch_id !== currentUser?.branchId)
            return;
          setUsers((prev) =>
            prev.find((u) => u.id === newUser.id) ? prev : [newUser, ...prev],
          );
      ...
      .on(
        ...
        (payload) => {
          ...
          if (!canSeeAllBranches && updated.branch_id !== currentUser?.branchId)
            return;
      ...
      .on(
        ...
        (payload) => {
          ...
          if (newReq.status === "pending") {
            if (
              !canSeeAllBranches &&
              newReq.branch_id !== currentUser?.branchId
            )
              return;
            setPendingRequests((prev) =>
              prev.find((r) => r.id === newReq.id) ? prev : [newReq, ...prev],
            );
            showToast(
              `New account request from ${newReq.first_name || newReq.email}`,
            );
          } else {
    ...
    const rolesSub = supabase
      ...
      .on(
        ...
        (payload) => {
          setCustomRoles((prev) =>
            prev.find((r) => r.id === payload.new.id)
              ? prev
              : [...prev, payload.new],
          );
        ...
        try {
          ...
          if (authError) {
            if (authError.message?.includes("already been registered")) {
              ...
              if (!existing) {
                alert("Could not find existing user. Please check manually.");
                return;
              }
              userId = existing.id;
            } else {
          ...
          await supabase
            ...
            .eq("id", req.id);
          await supabase.from("pending_users").delete().eq("id", req.id);
          setPendingRequests((prev) => prev.filter((r) => r.id !== req.id));
  ...
  const handleRejectPending = (req) => {
    setConfirm({
      title: "Reject Request",
      message: `Reject account request for ${[req.first_name, req.last_name].filter(Boolean).join(" ")}?`,
      type: "danger",
      confirmLabel: "Reject",
      onConfirm: async () => {
        setConfirm(null);
  ...
  const hasUnsavedEditChanges = () =>
    editFormOriginal !== null && JSON.stringify(editForm) !== editFormOriginal;
  ...
  const attemptCloseEditModal = () => {
    if (hasUnsavedEditChanges()) {
      setConfirm({
        ...
        message:
          "You have unsaved changes to this user's record. Do you want to discard them?",
        type: "danger",
      ...
      if (isSuperAdmin && editForm.email && editForm.email !== editUser.email) {
        const isPersonalEmail =
          [
            ...
            "Super Admin",
            "super_admin",
          ].includes(editForm.role) && editForm.usePersonalEmail;
    ...
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (unbanError)
            console.warn("Unban warning (non-fatal):", unbanError.message);
  ...
  const handleConfirmEmailChange = useCallback(async () => {
    ...
    try {
      ...
      const { error: insertErr } = await supabaseAdmin
        .from("email_change_codes")
        .insert([
          { user_id: userId, new_email: newEmail, code, expires_at: expiresAt },
        ]);
  ...
  useEffect(() => {
    if (
      pendingEmailChange &&
      !pendingEmailChange.sent &&
      !pendingEmailChange.sending
    ) {
  ...
  const validateRoleForm = () => {
    ...
    if (!roleForm.role.trim()) errs.role = "Role name is required";
      ...
      allRoles.some(
        (r) =>
          r.role.toLowerCase() === roleForm.role.trim().toLowerCase() &&
          r.id !== editRoleTarget?.id,
      )
    )
      errs.role = "Role already exists";
    if (!roleForm.desc.trim()) errs.desc = "Description is required";
    if (roleForm.perms.length === 0)
      errs.perms = "Select at least one permission";
    return errs;
  };
  ...
  const filtered = users.filter((u) => {
    ...
    const matchSearch =
      !search || `${fullName(u)} ${u.email || ""}`.toLowerCase().includes(q);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [search, roleFilter, branchFilter, statusFilter, adminsOnlyFilter]);
  useEffect(() => {
    setLogsPage(1);
  }, [logSearch, logRole]);
  ...
  const sortedUsers = (() => {
    ...
    arr.sort((a, b) => {
      ...
      if (key === "name") {
        ...
        bv = (
          b.branches?.name ||
          branches.find((b) => b.id === b.branch_id)?.name ||
          ""
        ).toLowerCase();
  ...
  const TAB_LIST = [
    ...
    {
      key: "pending",
      label: `Pending Approval${pendingRequests.length > 0 ? ` (${pendingRequests.length})` : ""}`,
    },
    {
      key: "pwdrequests",
  ...
  return (
    <Layout>
      ...
      <div style={S.page}>
        ...
        <div style={S.cont} className="usr-content">
          <div
            className="usr-stats-grid"
            style={{
              display: "grid",
            ...
            {[
              ...
              },
              {
                ...
                onClick: () => {
                  setTab("users");
                  setAdminsOnlyFilter(false);
                  setStatusFilter((f) => (f === "Active" ? "" : "Active"));
                },
              },
              {
                label: "Admins",
                        ...
                        filter:
                          sc.color === "blue"
                            ...
                            : sc.color === "green"
                              ...
                              : sc.color === "purple"
                                ? "brightness(0) saturate(100%) invert(22%) sepia(96%) saturate(1600%) hue-rotate(255deg) brightness(0.85)"
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  ...
                  >
                    {pendingRequests.length} Account Request
                    ...
                    <div style={{ padding: "8px 0", minWidth: 600 }}>
                      ...
                      {[1, 2, 3, 4, 5, 6].map((i) => (
                        ...
                        >
                          ...
                          {/* Action buttons */}
                          <div
                            style={{ width: "18%", display: "flex", gap: 6 }}
                          >
                            <Skel w="30%" h={28} />
                            <Skel w="30%" h={28} />
                            <Skel w="30%" h={28} />
                        ...
                        {filtered.length === 0 ? (
                          ...
                          paginated.map((u, idx) => (
                            ...
                            >
                              ...
                              {canSeeAllBranches && (
                                <td style={{ ...S.td, overflow: "hidden" }}>
                                  <span className="usr-td-branch-tag">
                                    {u.branches?.name ||
                                      branches.find((b) => b.id === u.branch_id)
                                        ?.name ||
                                      "—"}
                                  </span>
                                </td>
                              ...
                              <td style={S.td}>
                                {u.status === "Inactive" ? (
                                  <span className="usr-pill deactivated">
                                  ...
                                  <span className="usr-pill inactive">
                                    <span className="usr-pill-dot" />
                                    Inactive
                                  </span>
                                )}{" "}
            ...
            {tab === "pending" && (
              <div style={{ padding: 24 }}>
                <div style={{ marginBottom: 20 }}>
                  ...
                  >
                    {canSeeAllBranches
                      ? "Requests from all branches awaiting approval."
                      : "Accounts awaiting your approval."}
                  </p>
                </div>
                {pendingRequests.length === 0 ? (
              ...
              <div style={{ padding: 24 }}>
                <div style={{ marginBottom: 20 }}>
                  ...
                  <p style={{ fontSize: 11, color: "var(--muted)", margin: 0 }}>
                    Employees and customers submit a new password here; it only
                    takes effect once you approve it.
                  </p>
                </div>
              ...
              <div style={{ padding: 24 }}>
                ...
                >
                  <div>
                    ...
                    >
                      System roles are built-in. Custom roles can be created and
                      deleted.
                    </p>
                  </div>
                  {isSuperAdmin && (
                    ...
                    {Array.from({ length: logsTotalPages }, (_, i) => i + 1)
                      .filter(
                        (pg) =>
                          pg === 1 ||
                          pg === logsTotalPages ||
                          (pg >= safeLogsPage - 1 && pg <= safeLogsPage + 1),
                      )
        ...
        <div className="usr-modal-overlay">
          <div className="usr-modal-box">
                  ...
                  {(addForm.role === "Employee" ||
                    ...
                    >
                      <input
                        type="checkbox"
                        id="addPersonalEmail"
                        checked={!!addForm.usePersonalEmail}
                        onChange={(e) =>
                          setAddForm((f) => ({
                            ...f,
                            usePersonalEmail: e.target.checked,
                          }))
                        }
                        style={{ width: 14, height: 14, cursor: "pointer" }}
                      />
                      <label
                        htmlFor="addPersonalEmail"
                        style={{
                          fontSize: 11,
                          color: "var(--muted)",
                          cursor: "pointer",
                      ...
                      >
                        Use personal email (a 6-digit verification code will be
            ...
            <div className="usr-modal-actions">
              <button
                className="btn btn-ghost usr-w-auto"
                onClick={() => setShowAddModal(false)}
              >
                Cancel
              </button>
              <button
                ...
                style={{
                  ...
                  alignItems: "center",
                  gap: 6,
                  opacity: !isAddUserFormValid() || saving ? 0.5 : 1,
                  cursor:
                    !isAddUserFormValid() || saving ? "not-allowed" : "pointer",
                }}
                onClick={handleAddUser}
                disabled={saving || !isAddUserFormValid()}
              >
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                <span
                  className={`badge ${ROLE_BADGE[viewUser.role] || "badge-gray"}`}
                >
                  {viewUser.role || "—"}
                </span>
                <span
            ...
            <div style={{ padding: "16px 24px", flex: 1 }}>
              ...
              <DetailRow
                label="Branch"
                value={
                  viewUser.branches?.name ||
                  branches.find((b) => b.id === viewUser.branch_id)?.name ||
                  "—"
                }
              />
                    ...
                    <div style={{ marginTop: 8 }}>
                      <button
                        ...
                        onClick={() => {
                          ...
                          setShowPersonalEmailPrompt(true);
                        }}
                        style={{
                          fontSize: 12,
                          fontWeight: 600,
            ...
            >
              ...
              >
                <div className="form-group" style={{ margin: 0 }}>
                  ...
                  <input
                    ...
                    value={roleForm.role}
                    onChange={(e) => {
                      setRoleForm((f) => ({ ...f, role: e.target.value }));
                      setRoleFormErrors((er) => ({ ...er, role: "" }));
                    }}
                    style={roleFormErrors.role ? S.inpErr : {}}
                  />
                  {roleFormErrors.role && (
                    <div style={S.errMsg}>{roleFormErrors.role}</div>
                  )}
                </div>
                <div className="form-group" style={{ margin: 0 }}>
                  <label>Color</label>
                  <input
                    ...
                    onChange={(e) =>
                      setRoleForm((f) => ({ ...f, color: e.target.value }))
                    }
                    style={{
                      width: 44,
                      height: 38,
              ...
              <div className="form-group" style={{ margin: 0 }}>
                ...
                <textarea
                  ...
                  value={roleForm.desc}
                  onChange={(e) => {
                    setRoleForm((f) => ({ ...f, desc: e.target.value }));
                    setRoleFormErrors((er) => ({ ...er, desc: "" }));
                  }}
                  style={{
                    ...(roleFormErrors.desc ? S.inpErr : S.inp),
                    width: "100%",
                    resize: "vertical",
                    boxSizing: "border-box",
                  }}
                />
                {roleFormErrors.desc && (
                  <div style={S.errMsg}>{roleFormErrors.desc}</div>
                )}
              </div>
              <div>
                <label
                ...
                <div
                  style={{
                    ...
                    gap: "6px 14px",
                    background: "var(--bg)",
                    border: `1.5px solid ${roleFormErrors.perms ? "#f87171" : "var(--border)"}`,
                    borderRadius: 8,
                    padding: "12px 14px",
                  }}
                >
                  {ALL_PERMISSIONS.map((p) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={roleForm.perms.includes(p.key)}
                        onChange={() => togglePerm(p.key)}
                        style={{
                          accentColor: roleForm.color,
                          width: 14,
                          height: 14,
                        }}
                      />
                ...
                {roleFormErrors.perms && (
                  <div style={S.errMsg}>{roleFormErrors.perms}</div>
                )}
              </div>
            </div>
            <div className="modal-footer">
          ...
          >
            ...
            <input
              ...
              style={{
                ...
                padding: "10px 14px",
                fontSize: 13,
                border: `1.5px solid ${personalEmailError ? "#f87171" : "var(--border)"}`,
                borderRadius: 8,
                background: "var(--bg)",
                color: "var(--text)",
                outline: "none",
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
              <button
                ...
                onClick={() => setShowPersonalEmailPrompt(false)}
              >
                Cancel
              </button>
              <button
                ...
                onClick={() => {
                  ...
                  if (val === (editUser?.email || "").toLowerCase()) {
                    setPersonalEmailError("This is already the current email");
                    return;
                  }
        ...
        pendingEmailChange.showOtp && (
          ...
          >
            ...
            >
              ...
              {otpError && (
                <div
                  style={{ fontSize: 11, color: "#dc2626", marginBottom: 10 }}
                >
                  {otpError}
                </div>
              )}
              <button
                ...
                style={{
                  width: "100%",
                  marginTop: 12,
                  opacity: otpInput.some((d) => !d) ? 0.6 : 1,
                }}
