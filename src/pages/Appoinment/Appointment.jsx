...
const Skel = ({ w = "100%", h = 14, style = {} }) => (
  <span
    className="skel"
    style={{ display: "block", width: w, height: h, borderRadius: 6, ...style }}
  />
);
  ...
  >
    ...
    {Array.from({ length: 35 }).map((_, i) => (
      ...
      >
        ...
        {i % 4 === 0 && (
          <Skel w="90%" h={18} style={{ marginBottom: 3, borderRadius: 4 }} />
        )}
...
const Appointment = () => {
  ...
  useEffect(() => {
    fetchRooms();
    const ch = supabase
      .channel(`appt-rooms-rt-${user?.branchId || "all"}`)
      .on(
        "postgres_changes",
  ...
  useEffect(() => {
    if (!user) return;
    const ch = supabase
      .channel(`appointments-rt-${user.branchId || "all"}`)
      .on(
        "postgres_changes",
  ...
  const openEdit = (a) => {
    ...
    setSelectedAppt(a);
    setForm({ ...EMPTY, ...a, room: a.room || "", ownerId: a.user_id || "" });
    setEditMode(true);
    setConflictType(null);
    setExtraPets([]);
    setExistingPatients([]);
  ...
  const selectService = (val) => {
    const branchForLookup = seeAllBranches
  ...
  const addExtraPet = () =>
    setExtraPets((prev) => [
      ...prev,
      { ...EMPTY_PET_APPT, purpose: form.purpose, price: form.price },
    ]);
  ...
  const saveAppointment = async () => {
    ...
    if (!form.vet) {
      showAlert("Missing Fields", "Please select a veterinarian.");
      return;
    }
    ...
    if (!editMode) {
      for (const p of extraPets) {
        ...
        if (p.purpose === "Imaging" && !p.imagingType) {
          showAlert(
            "Missing Fields",
            "Please select an imaging type for each pet receiving imaging.",
          );
          return;
        }
    ...
    if (editMode && selectedAppt?.id)
      conflictQuery = conflictQuery.neq("id", selectedAppt.id);
    ...
    const payload = {
      ...
      notes: form.notes,
      status: finalStatus,
      ...(!editMode && !seeAllBranches && user?.branchId
        ? { branch_id: user.branchId }
        : {}),
      ...(!editMode && seeAllBranches && branchFilter
    ...
    logActivity(
      user,
      "Created appointment",
      `Booked appointment for: ${form.patient}`,
    );
    if (form.patient.trim() && form.owner.trim()) {
      const { data: existing, error: existingErr } = await supabase
        ...
        .maybeSingle();
      if (existingErr)
        console.error("Patient lookup failed:", existingErr.message);
      if (!existing) {
        ...
        if (patientErr) {
          console.error(
            "Auto patient-record creation failed:",
            patientErr.message,
          );
          showToast(
            `⚠ Appointment booked, but patient record wasn't created: ${patientErr.message}`,
            "error",
          );
        }
      }
    ...
    for (const p of extraPets) {
      ...
      const extraPayload = {
        ...
        notes: p.notes,
        status: finalStatus,
        ...(!seeAllBranches && user?.branchId
          ? { branch_id: user.branchId }
          : {}),
        ...(seeAllBranches && branchFilter
          ? { branch_id: Number(branchFilter) }
          : {}),
      };
      const { data: insertedExtra, error: extraErr } = await supabase
        .from("appointments")
        .insert([extraPayload])
        .select()
        .single();
      if (extraErr) {
        console.warn("Extra pet appointment failed:", extraErr.message);
        continue;
      }
      ...
      if (p.mode === "new" && petName) {
        ...
        if (!existing) {
          ...
          if (patientErr) {
            console.error(
              "Auto patient-record creation failed (extra pet):",
              patientErr.message,
            );
            showToast(
              `⚠ Extra pet appointment booked, but patient record wasn't created: ${patientErr.message}`,
              "error",
            );
          }
        }
    ...
    if (isAdmin || isEmployee) {
      showToast(
        total > 1
          ? `✓ ${total} appointments confirmed successfully`
      ...
      showToast(
        total > 1
          ? `✓ ${total} requests submitted — awaiting staff approval`
          : "✓ Request submitted — awaiting staff approval",
        "warning",
      );
    }
  };
  ...
  const approveAppt = async (id, roomNumber = null) => {
    ...
    logActivity(
      ...
      `Confirmed appointment ID: ${id}${roomNumber ? ` — Room ${roomNumber}` : ""}`,
    );
    showToast(
      roomNumber
        ? `✓ Appointment approved & Room ${roomNumber} assigned`
        : "✓ Appointment approved (no room assigned)",
      "success",
    );
  };
  ...
  const confirmCancelAppt = async () => {
    ...
    logActivity(
      ...
      `Cancelled appointment ID: ${id} — Reason: ${finalReason}`,
    );
  ...
  const renderCalendar = () => {
    ...
    const apptColor = (a) => {
      if (a.purpose === "Deworming")
        return { bg: "#7c3aed", pill: "#f3e8ff", text: "#6d28d9" };
      if (a.purpose === "Diagnostics")
        return { bg: "#dc2626", pill: "#fee2e2", text: "#b91c1c" };
      if (a.status === "Missed")
        return { bg: "#6b7280", pill: "#f1f5f9", text: "#374151" };
      if (a.status === "Pending")
        return { bg: "#d97706", pill: "#fef3c7", text: "#92400e" };
      if (a.status === "Confirmed")
        return { bg: "#1e3a8a", pill: "#dbeafe", text: "#1e40af" };
      if (a.status === "Completed")
        return { bg: "#0891b2", pill: "#e0f2fe", text: "#0369a1" };
      if (a.status === "Cancelled")
        return { bg: "#9ca3af", pill: "#f1f5f9", text: "#64748b" };
        ...
        >
          {cells.map((day, i) => {
            ...
            const dayAppts = calendarAppts.filter((a) => {
              ...
              return `${a.patient} ${a.owner} ${a.vet}`
                .toLowerCase()
                .includes(q);
            });
                    ...
                    {hasAppts && (
                      <span
                        style={{
                          ...
                          color: "#fff",
                          background:
                            dayAppts.length > 2 ? "#dc2626" : "#1e3a8a",
                          borderRadius: 99,
                          padding: "1px 5px",
                          lineHeight: 1.6,
                        }}
                    ...
                    {dayAppts.slice(0, 2).map((a) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {a.time?.replace(" AM", "a").replace(" PM", "p")}
                          </span>
                          <span
                            style={{
                              overflow: "hidden",
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                {approveModal.appt.patient} — {approveModal.appt.date} at{" "}
                {approveModal.appt.time}
              </p>
            </div>
            <div style={{ padding: "18px 20px" }}>
              ...
              >
                {rooms.filter((r) => r.status === "Available").length} room(s)
                available. Customers can't pick a room when booking — assign one
                now if needed.
              </div>
            </div>
            <div
          ...
          >
            ...
            >
              <div>
                ...
                >
                  Export all appointments for a chosen period.
                </p>
              </div>
              <button
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontFamily: "inherit",
                      background:
                        exportPeriod === key ? "var(--royal)" : "var(--card)",
            ...
            >
              ...
              <button
                ...
                style={{ width: "auto", opacity: exporting ? 0.6 : 1 }}
                disabled={exporting}
                onClick={runExport}
              >
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                Please tell us why this appointment is being cancelled.
              </p>
            </div>
            <div style={{ padding: "18px 20px" }}>
              <div
      ...
      >
        <div className="topbar-title">
          ...
          <div>
            ...
            <p>
              {isAdmin
                ? "Manage & approve appointments — All Branches"
                : isCustomer
                  ? "Your appointments"
                  : "Manage & approve appointments"}
            </p>
          </div>
        </div>
      ...
      <div className="content appt-content">
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                ...
                >
                  {counts.pending} Appointment{counts.pending > 1 ? "s" : ""}{" "}
                  Awaiting Approval
                </p>
                <p style={{ margin: 0, fontSize: 12, color: "#b45309" }}>
                  Once approved, appointments are locked from further edits.
                </p>
              </div>
            </div>
            <button
          ...
          {loading
            ...
            : [
                ...
                },
                {
                  ...
                  subColor: counts.pending > 0 ? "#d97706" : undefined,
                },
                {
                  label: "Confirmed",
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <input
                    type="text"
                    placeholder="Search patient, vet, owner..."
                    value={search}
                ...
                <div
                  style={{ flex: "1 1 130px", minWidth: 130, maxWidth: 220 }}
                >
                  <CustomSelect
                    value={filterStatus}
                                  ...
                                  (isAdmin || isEmployee) && (
                                    <button
                                      ...
                                      onClick={() =>
                                        showConfirm(
                                          "Mark Complete",
                                          `Mark ${a.patient}'s appointment as completed?`,
                                          () => completeAppt(a.id),
                                          "Mark Complete",
                                          "#1e3a8a",
                                        )
          ...
          <div className="appt-card">
            ...
            >
              ...
              <div style={{ textAlign: "center" }}>
                ...
                >
                  {calMonth.toLocaleDateString("en-US", { month: "long" })}
                </div>
                <div
                  style={{
                    fontSize: 12,
            ...
            >
              {[
                ...
                >
                  ...
                  />
                  <span style={{ fontSize: 11, color: color, fontWeight: 700 }}>
                    {label}
                  </span>
                </div>
              ))}
            ...
            <div style={{ padding: "8px 8px 16px", overflowX: "auto" }}>
              <div style={{ minWidth: 320 }}>{renderCalendar()}</div>
            </div>
          </div>
        )}
                  ...
                  >
                    {isAdmin ? (
                      <>
                        ...
                        Admin booking — confirmed immediately
                      </>
                    ) : isCustomer ? (
                      <>
                        <svg
              ...
              <div style={{ overflowY: "auto", flex: 1 }}>
                ...
                {(editMode || bookStep !== "service") && (
                  <div style={{ borderBottom: "1.5px solid #e2e8f0" }}>
                    ...
                    >
                      {(() => {
                        const meta = SERVICE_META[form.purpose] || {
                          icon: (
                            <svg
                              viewBox="0 0 24 24"
                              fill="#475569"
                    ...
                    >
                      Price is fixed per service and can only be changed in the
                      system by staff.
                    </div>
                      ...
                      {form.mode === "existing" ? (
                        loadingExistingPatients ? (
                          ...
                          <CustomSelect
                            ...
                            options={existingPatients.map((ep) => ({
                              value: ep.id,
                              label: `${ep.name}${ep.species ? ` (${ep.species})` : ""}`,
                            }))}
                          />
                        )
                      ) : (
                      ...
                      >
                          ...
                          >
                            ...
                            <div
                              style={{
                                ...
                                fontWeight: 700,
                                color:
                                  (SERVICE_META[form.purpose] || {}).color ||
                                  "var(--text)",
                              }}
                            >
                              {form.purpose}
                              ...
                              <>
                                ...
                                <CustomSelect
                                  ...
                                  onChange={(val) => {
                                    ...
                                    updateExtraPet(idx, {
                                      imagingType: val,
                                      price: looked != null ? looked : p.price,
                                    });
                                  }}
                                  placeholder="— Select Imaging Type —"
                                  options={["X-ray", "Ultrasound", "CT-scan"]}
                        ...
                        {pTaken && (
                          ...
                          >
                            ...
                            >
                              This time slot is already taken. Please choose a
                              different time.
                            </p>
                          </div>
                        )}
                      ...
                      >
                        ...
                        <CustomSelect
                          ...
                          options={rooms
                            ...
                            .map((r) => ({
                              value: r.number,
                              label: `${r.number}${r.type ? ` · ${r.type}` : ""}${r.status !== "Available" && r.number !== form.room ? ` — ${r.status}` : ""}`,
                            }))}
                        />
                        <div
                          style={{
                    ...
                    >
                      {editMode ? (
                        ...
                        <>
                          ...
                          {extraPets.length > 0
                            ? `File ${1 + extraPets.length} Appointments`
                            : "File Appointment"}
                        </>
                      ) : extraPets.length > 0 ? (
                        `Submit ${1 + extraPets.length} Requests`
                      ) : (
                        "Submit Request"
                      )}
                    </button>
        ...
        {showView &&
          ...
          (() => {
            const statusConfig = {
              Pending: {
                hero: "linear-gradient(135deg, #78350f, #d97706)",
                badge: "#fef3c7",
                badgeText: "#92400e",
                border: "#fde68a",
                dot: "#d97706",
              ...
              Confirmed: {
                hero: "linear-gradient(135deg, #14532d, #16a34a)",
                badge: "#dcfce7",
                badgeText: "#14532d",
                border: "#86efac",
                dot: "#16a34a",
              ...
              Cancelled: {
                hero: "linear-gradient(135deg, #7f1d1d, #dc2626)",
                badge: "#fee2e2",
                badgeText: "#991b1b",
                border: "#fca5a5",
                dot: "#dc2626",
              ...
              Missed: {
                hero: "linear-gradient(135deg, #374151, #6b7280)",
                badge: "#f1f5f9",
                badgeText: "#374151",
                border: "#cbd5e1",
                dot: "#6b7280",
            ...
            const cfg =
              statusConfig[selectedAppt.status] || statusConfig.Pending;
            const purposeStyle = {
              Consultation: {
                bg: "#f8fafc",
                color: "#475569",
                  ...
                  >
                      ...
                      (isAdmin || isEmployee) && (
                        ...
                        >
                          <div>
                            ...
                            >
                              {selectedAppt.status === "Pending"
                                ? "Approve to lock this appointment from edits."
                                : "Mark as complete once the visit is done."}
                            </p>
                          </div>
                          <div
                    ...
                    >
                      {[
                        ...
                        },
                        {
                          ...
                          icon: (
                            <svg
                              ...
                              strokeLinecap="round"
                            >
                              <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13.5 19.79 19.79 0 0 1 1.6 4.9 2 2 0 0 1 3.58 2.72h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 10.1a16 16 0 0 0 6 6z" />
                            </svg>
                          ),
                        },
                        {
                          label: "Species",
                          value: selectedAppt.species || "—",
                          icon: (
                            <svg
                              width="12"
                              height="12"
                        ...
                        {
                          label: "Room",
                          value: selectedAppt.room || "Unassigned",
                          icon: (
                            <svg
                              width="12"
                              height="12"
                    ...
                    <div
                      style={{
                        background: purposeStyle.bg,
                        border: `1.5px solid ${purposeStyle.color}33`,
                        borderRadius: 10,
                        padding: "10px 14px",
                        marginBottom: 14,
                        display: "flex",
                    ...
                    >
                      <span
                        style={{
                          ...
                          height: 32,
                          borderRadius: 8,
                          background: `${purposeStyle.color}18`,
                          display: "flex",
                          alignItems: "center",
                          justifyContent: "center",
                          flexShrink: 0,
