...
const Report = () => {
  ...
  const [dialog, setDialog] = useState({ show: false, message: '', title: '', onConfirm: null });
  ...
  const handleExportPDF = () => {
    if (pdfDownloaded) {
      showConfirm('You already downloaded this PDF report. Download it again?', runExportPDF, 'Download Again?');
      return;
    }
    runExportPDF();
  };
  ...
  const runExportPDF = async () => {
    ...
    } catch (err) { console.error(err); showToast('PDF export failed. Please try again.', 'error'); }
  ...
  const handleExportExcel = () => {
    if (excelDownloaded) {
      showConfirm('You already downloaded this Excel report. Download it again?', runExportExcel, 'Download Again?');
      return;
    }
    runExportExcel();
  };
  ...
  const runExportExcel = async () => {
    ...
    } catch (err) { console.error(err); showToast('Excel export failed. Please try again.', 'error'); }
    finally { setExporting(''); }
  };
  ...
  return (
    <Layout>
      ...
      {exporting && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999998, padding: 16 }}>
          <div style={{ background: 'var(--card)', borderRadius: 16, width: '100%', maxWidth: 320, boxShadow: '0 20px 60px rgba(0,0,0,0.35)', padding: '30px 26px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, textAlign: 'center' }}>
            <div style={{
              width: 52, height: 52, borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center',
              background: exporting === 'pdf' ? '#fef2f2' : '#f0fdf4',
            }}>
              <span className="export-spin" style={{
                width: 24, height: 24,
                borderColor: exporting === 'pdf' ? '#dc2626' : '#16a34a',
                borderTopColor: 'transparent',
              }} />
            </div>
            <div>
              <h3 style={{ margin: '0 0 4px', fontSize: 15, fontWeight: 700, color: 'var(--text)' }}>
                {exporting === 'pdf' ? 'Generating PDF Report' : 'Generating Excel Report'}
              </h3>
              <p style={{ margin: 0, fontSize: 12.5, color: 'var(--muted)', lineHeight: 1.5 }}>
                Please wait while we prepare your {range.toLowerCase()} report for {branchLabel}…
              </p>
            </div>
          </div>
        </div>
       ...
       <div className="topbar" style={{ position: "fixed", top: 68, left: "var(--current-sidebar-w, 62px)", right: 0, zIndex: 40, background: "#fff" }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            ...
            <button
              ...
              onClick={() => { setShowAllTx(true); fetchAllTransactions(); }}
      ...
      {showAllTx && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 99999, padding: 16 }}>
          <div style={{ background: 'var(--card)', borderRadius: 14, width: '100%', maxWidth: 900, maxHeight: '90vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>
            <div style={{ padding: '18px 22px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>All Transactions — {branchLabel}</h3>
              <button className="btn btn-ghost btn-icon" style={{ width: 'auto' }} onClick={() => setShowAllTx(false)}>✕</button>
            </div>
            <div style={{ padding: '16px 22px', overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr>
                    {['Client', 'Total', 'Payment', 'Date', 'Status'].map(h => (
                ...
                <tbody>
                  {txLoading ? (
                    ...
                    <tr><td colSpan={5} style={{ textAlign: 'center', padding: 40, color: 'var(--muted)' }}>No transactions yet</td></tr>
                    ...
                    return (
                      <tr key={t.id} style={{ borderBottom: '1px solid var(--border)', opacity: isVoided ? 0.55 : 1, background: isVoided ? '#fef2f2' : 'transparent' }}>
                        ...
                        <td style={{ padding: '10px 12px', fontWeight: 700, color: 'var(--royal)' }}>₱{Number(t.total).toLocaleString('en-PH')}</td>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 99999, padding: 16 }}>
          <div style={{ background: 'var(--card)', borderRadius: 14, width: '100%', maxWidth: 380, boxShadow: '0 20px 60px rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, padding: '12px 22px', borderTop: '1px solid var(--border)', background: 'var(--bg)' }}>
              ...
              <button className="btn btn-primary" style={{ width: 'auto', background: '#0f172a', borderColor: '#0f172a' }} onClick={() => { dialog.onConfirm?.(); setDialog(d => ({ ...d, show: false })); }}>Download Again</button>
            </div>
          </div>
        </div>
      )}
