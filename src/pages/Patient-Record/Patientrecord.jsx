...
const DRUG_FORM_OPTIONS = ["Tablet", "Liquid", "Injectable", "Topical", "Capsule", "Powder", "Other"];
const VACCINE_OPTIONS = ["Rabies Vaccine", "DHPP (Distemper, Hepatitis, Parainfluenza, Parvovirus)", "Bordetella (Kennel Cough)", "Canine Influenza", "Leptospirosis", "Lyme Disease", "FVRCP (Feline Viral Rhinotracheitis, Calicivirus, Panleukopenia)", "FeLV (Feline Leukemia)", "FIV (Feline Immunodeficiency Virus)", "Chlamydia Felis", "Other"];
...
const DOG_BREEDS = ["Aspin (Mixed Breed)", "Shih Tzu", "Siberian Husky", "Labrador Retriever", "Golden Retriever", "German Shepherd", "Poodle", "Pomeranian", "Chihuahua", "Beagle", "Rottweiler", "Dachshund", "French Bulldog", "Bulldog", "Shiba Inu", "Japanese Spitz", "Belgian Malinois", "Doberman Pinscher", "Border Collie", "Cocker Spaniel", "Maltese", "Great Dane", "Boxer", "Pug", "Other"];
const CAT_BREEDS = ["Puspin (Mixed Breed)", "Persian", "Siamese", "British Shorthair", "Scottish Fold", "Maine Coon", "Ragdoll", "Munchkin", "American Shorthair", "Bengal", "Sphynx", "Himalayan", "Turkish Angora", "Exotic Shorthair", "Other"];
...
const VaxFields = ({ form, setForm }) => (
  <div className="form-grid">
    <div className="form-group"><label>Vaccine Name *</label><CustomSelect value={form.name} onChange={val => setForm({ ...form, name: val })} placeholder="Select vaccine" options={VACCINE_OPTIONS} /></div>
    <div className="form-group"><label>Date Given *</label><DatePicker value={form.date_given} onChange={val => setForm({ ...form, date_given: val })} placeholder="Select date" /></div>
    <div className="form-group"><label>Next Due Date</label><DatePicker value={form.next_due} onChange={val => setForm({ ...form, next_due: val })} placeholder="Select date" /></div>
    <div className="form-group"><label>Vet Attendant</label><CustomSelect value={form.given_by} onChange={val => setForm({ ...form, given_by: val })} placeholder="— Select Vet —" options={VET_OPTIONS} /></div>
    <div className="form-group"><label>Number of Doses</label><input type="text" inputMode="numeric" value={form.doses} onChange={e => setForm({ ...form, doses: e.target.value })} placeholder="e.g. 1" /></div>
    <div className="form-group"><label>Lot Number</label><input type="text" value={form.lot_number} onChange={e => setForm({ ...form, lot_number: e.target.value })} placeholder="e.g. LN-2026-0417" /></div>
  </div>
);
...
const TreatFields = ({ form, setForm }) => (
  <div className="form-grid">
    ...
    <div className="form-group"><label>Attending Vet</label><CustomSelect value={form.vet} onChange={val => setForm({ ...form, vet: val })} placeholder="— Select Vet —" options={VET_OPTIONS} /></div>
    ...
    <div className="form-group"><label>Temperature (°C) <span style={{ fontWeight: 400 }}>(optional)</span></label><input type="text" inputMode="numeric" value={form.heart_rate} onChange={e => setForm({ ...form, heart_rate: e.target.value.replace(/\D/g, '') })} placeholder="e.g. 120" /></div>
    <div className="form-group"><label>Heart Rate (bpm) <span style={{ fontWeight: 400 }}>(optional)</span></label><input type="text" inputMode="numeric" value={form.heart_rate} onChange={e => setForm({ ...form, heart_rate: e.target.value })} placeholder="e.g. 120" /></div>
    <div className="form-group"><label>Weight (kg) <span style={{ fontWeight: 400 }}>(optional)</span></label><input type="text" inputMode="decimal" value={form.weight} onChange={e => setForm({ ...form, weight: e.target.value.replace(/[^0-9.]/g, '') })} placeholder="e.g. 8.2" /></div>
...
const RoomSelect = ({ value, onChange, rooms, accent = "#6366f1" }) => {
  ...
  const label = value ? `${value}${selectedRoom?.type ? ` · ${selectedRoom.type}` : ""}` : "— No room assigned —";
  ...
  const selectRoom = (num) => { onChange({ target: { value: num } }); setOpen(false); };
  ...
  const OptionRow = ({ isEmpty, isSelected, optLabel, onClick }) => (
    <div
      ...
      onMouseEnter={e => { if (!isSelected && !isEmpty) e.currentTarget.style.background = "#f4f6fa"; }}
  ...
  const portal = open && typeof document !== "undefined" ? ReactDOM.createPortal(
    <div
      ...
      style={{
        ...
        background: "var(--card)", borderRadius: 12, zIndex: 99999,
        boxShadow: "0 16px 40px rgba(0,0,0,0.13), 0 4px 12px rgba(0,0,0,0.07), 0 0 0 1px rgba(0,0,0,0.06)",
        border: "1.5px solid #e8edf4", maxHeight: 260, overflowY: "auto", padding: "5px",
      }}
    >
      <OptionRow isEmpty optLabel="— No room assigned —" isSelected={!value} onClick={() => selectRoom("")} />
      {availableRooms.length > 0 && (
        <>
          <div style={{ padding: "6px 10px 2px", fontSize: 10, fontWeight: 800, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 0.5 }}>Available</div>
      ...
      {unavailableRooms.length > 0 && (
        <>
          <div style={{ padding: "6px 10px 2px", fontSize: 10, fontWeight: 800, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 0.5 }}>Unavailable</div>
          {unavailableRooms.map(r => (
            <div key={r.id} style={{ padding: "8px 10px", fontSize: 13, fontWeight: 500, color: "#cbd5e1", cursor: "not-allowed", borderRadius: 8, display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 6, height: 6, borderRadius: "50%", flexShrink: 0, border: "1.5px solid #e2e8f0" }} />
              <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {r.number}{r.type ? ` · ${r.type}` : ""} — {r.status}{r.patient ? ` (${r.patient})` : ""}
              </span>
            </div>
          ))}
        </>
      ...
      {rooms.length === 0 ? (
        ...
        <div style={{ position: "relative", width: "100%" }}>
          <div
            ...
            onClick={handleOpen}
            style={{
              width: "100%", padding: "8px 34px 8px 12px", border: "1.5px solid", borderRadius: 9,
              background: open ? "linear-gradient(135deg, #ffffff 0%, #f5f3ff 100%)" : "linear-gradient(to bottom, #ffffff 0%, #f8fafc 100%)",
              fontSize: 13, fontWeight: 600, color: value ? "var(--text)" : "#b0bac9",
              cursor: "pointer", userSelect: "none", boxSizing: "border-box",
              boxShadow: open ? `0 0 0 3px ${accent}22, 0 2px 8px rgba(0,0,0,0.08)` : "0 1px 3px rgba(0,0,0,0.06), inset 0 1px 0 rgba(255,255,255,0.9)",
              borderColor: open ? accent : "#dde3ec",
              transition: "border-color 0.18s, box-shadow 0.18s, background 0.18s",
              display: "flex", alignItems: "center", justifyContent: "space-between",
              gap: 8, position: "relative", minHeight: 36,
            }}
          ...
          >
            ...
            <div style={{
              ...
              <svg
                ...
                style={{ transition: "transform 0.2s, stroke 0.18s", transform: open ? "rotate(180deg)" : "rotate(0deg)" }}
              >
...
const CredentialCard = ({ credentials, onClose }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1200, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "var(--card)", borderRadius: 14, boxShadow: "0 24px 64px rgba(0,0,0,0.30)", width: "100%", maxWidth: 440, overflow: "hidden" }}>
        ...
        <div style={{ padding: "20px 24px" }}>
          {[["Name", credentials.fullName], ["Email", credentials.email], ["Password", credentials.password]].map(([label, value]) => (
            <div key={label} style={{ marginBottom: 12 }}>
              ...
              <div style={{ background: "var(--bg)", border: "1.5px solid var(--border)", borderRadius: 8, padding: "9px 14px", fontSize: 13, fontWeight: 600, color: "var(--text)", fontFamily: label === "Password" ? "monospace" : "inherit" }}>{value}</div>
            </div>
          ))}
  ...
  return (
    <div style={{ position: "relative", width: "100%" }}>
      ...
      <div
        ...
        onClick={handleOpen}
        style={{
          width: "100%", padding: "8px 34px 8px 12px", border: "1.5px solid",
          borderRadius: 9,
          background: open
...
const PatientInfoTab = ({ patient, treatments = [] }) => {
  ...
  const latestTreat = [...treatments].sort((a, b) => new Date(b.date || 0) - new Date(a.date || 0))[0];
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        ...
        {hasVitals && (
          <div style={{ gridColumn: '1 / -1' }}>
            <div style={{ background: '#eff6ff', border: '1.5px solid #bfdbfe', borderRadius: 10, padding: '14px 16px', display: 'flex', gap: 24, flexWrap: 'wrap' }}>
              <div style={{ flex: '0 0 100%' }}>
                <p style={{ margin: 0, fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1, color: '#1e40af' }}>
                  Latest Vital Signs{latestTreat ? ` — ${latestTreat.date}` : ''}
                </p>
              </div>
              <div><span style={{ fontSize: 10, color: '#64748b', fontWeight: 700, textTransform: 'uppercase' }}>Temp</span><br /><span style={{ fontSize: 15, fontWeight: 700, color: '#1e293b' }}>{latestTemp || '—'}{latestTemp ? '°C' : ''}</span></div>
              <div><span style={{ fontSize: 10, color: '#64748b', fontWeight: 700, textTransform: 'uppercase' }}>Heart Rate</span><br /><span style={{ fontSize: 15, fontWeight: 700, color: '#1e293b' }}>{latestHR || '—'}{latestHR ? ' bpm' : ''}</span></div>
              <div><span style={{ fontSize: 10, color: '#64748b', fontWeight: 700, textTransform: 'uppercase' }}>Weight</span><br /><span style={{ fontSize: 15, fontWeight: 700, color: '#1e293b' }}>{latestWeight || '—'}{latestWeight ? ' kg' : ''}</span></div>
            </div>
          </div>
        )}
...
const PatientRecord = () => {
  ...
  const [editVaxForm, setEditVaxForm] = useState({ name: "", date_given: "", next_due: "", given_by: "", doses: "", lot_number: "" });
  const [editTreatForm, setEditTreatForm] = useState({ date: "", diagnosis: "", notes: "", vet: "", temp: "", heart_rate: "", weight: "" });
  const [editRxForm, setEditRxForm] = useState({ medicine: "", concentration: "", drug_form: "", dosage: "", frequency: "Once daily", route: "Oral", duration: "", instructions: "", prescribed_by: "", date_prescribed: "" });
  const [pendingVax, setPendingVax] = useState([]);
  const [pendingTreat, setPendingTreat] = useState([]);
  const [addVaxForm, setAddVaxForm] = useState({ name: "", date_given: new Date().toISOString().slice(0, 10), next_due: "", given_by: "", doses: "", lot_number: "" });
  const [addTreatForm, setAddTreatForm] = useState({ date: new Date().toISOString().slice(0, 10), diagnosis: "", notes: "", vet: "", temp: "", heart_rate: "", weight: "" });
  ...
  const [typeDeleteModal, setTypeDeleteModal] = useState({ show: false, title: "", message: "", recordLabel: "", typed: "", onConfirm: null });
  const requireTypeToDelete = (title, message, recordLabel, onConfirm) => setTypeDeleteModal({ show: true, title, message, recordLabel, typed: "", onConfirm });
  const closeTypeDeleteModal = () => setTypeDeleteModal(m => ({ ...m, show: false, typed: "" }));
  const [form, setForm] = useState({ name: "", species: "", breed: "", gender: "", age: "", temp: "", heart_rate: "", weight: "", owner: "", owner_first: "", owner_last: "", owner_gender: "", contact: "", owner_email: "", condition: "", status: "Outpatient", health: "Good", room: "" });
  const [addPatientErrors, setAddPatientErrors] = useState({});
  const [rxForm, setRxForm] = useState({ medicine: "", concentration: "", drug_form: "", dosage: "", frequency: "Once daily", route: "Oral", duration: "", instructions: "", prescribed_by: "", date_prescribed: new Date().toISOString().slice(0, 10) });
  const [vaxForm, setVaxForm] = useState({ name: "", date_given: new Date().toISOString().slice(0, 10), next_due: "", given_by: "", doses: "", lot_number: "" });
  const [treatForm, setTreatForm] = useState({ date: new Date().toISOString().slice(0, 10), diagnosis: "", notes: "", vet: "", temp: "", heart_rate: "", weight: "" });
  const [toasts, setToasts] = useState([]);
  const toastTimer = useRef(null);
 const [editingPatient, setEditingPatient] = useState(null);
  const [editPatientForm, setEditPatientForm] = useState({ name: '', species: '', breed: '', gender: '', age: '', temp: '', heart_rate: '', weight: '', owner: '', contact: '', owner_email: '', condition: '', status: 'Outpatient', health: 'Good', room: '' });
  ...
  const SortIcon = ({ field }) => {
    ...
    return (
      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={active ? "var(--royal)" : "#cbd5e1"} strokeWidth="3" strokeLinecap="round" style={{ marginLeft: 4, flexShrink: 0, transform: active && sortDir === "desc" ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.15s" }}>
  ...
  const fetchPatients = async () => {
    setLoading(true);
    let q = supabase.from(T_PATIENTS).select("*").is("deleted_at", null).order("created_at", { ascending: false });
    ...
    if (!error) {
      ...
      setSelectedPatient(prev => (prev ? (data || []).find(pt => pt.id === prev.id) || prev : prev));
      setEditingPatient(prev => (prev ? (data || []).find(pt => pt.id === prev.id) || prev : prev));
    }
    setLoading(false);
  };
  ...
  const fetchDeletedPatients = async () => {
    let q = supabase.from(T_PATIENTS).select("*").not("deleted_at", "is", null).order("deleted_at", { ascending: false });
    if (!seeAllBranches && user?.branchId) q = q.eq("branch_id", user.branchId);
    if (seeAllBranches && branchFilter) q = q.eq("branch_id", branchFilter);
    const { data, error } = await q;
    if (error) return;
    const now = Date.now();
    const expired = (data || []).filter(p => now - new Date(p.deleted_at).getTime() > THIRTY_DAYS_MS);
    if (expired.length > 0) await supabase.from(T_PATIENTS).delete().in("id", expired.map(p => p.id));
    setDeletedPatients((data || []).filter(p => now - new Date(p.deleted_at).getTime() <= THIRTY_DAYS_MS));
  };
  ...
  const fetchExistingPatientsForOwner = async (ownerName, ownerUserId) => {
    ...
    let q = supabase.from(T_PATIENTS).select("id, name, species, breed, gender, owner, owner_user_id");
  ...
  useEffect(() => {
    if (user) logActivity(user, 'Viewed patient records', 'Opened patient records list');
  }, []);

  useEffect(() => {
    if (!user) return;
        fetchPatients(); fetchRooms(); fetchDeletedPatients();
    const patientChannel = supabase.channel("patients-realtime").on("postgres_changes", { event: "*", schema: "public", table: T_PATIENTS }, () => { fetchPatients(); fetchDeletedPatients(); }).subscribe();
  ...
  if (sortField) {
    sortedFiltered = [...filtered].sort((a, b) => {
      let av, bv;
      if (sortField === "created_at") { av = new Date(a.created_at || 0).getTime(); bv = new Date(b.created_at || 0).getTime(); }
      else if (sortField === "health") { av = HEALTH_RANK[a.health] ?? 99; bv = HEALTH_RANK[b.health] ?? 99; }
      else if (sortField === "status") { av = STATUS_RANK[a.status] ?? 99; bv = STATUS_RANK[b.status] ?? 99; }
      else if (sortField === "room") { av = a.room || ""; bv = b.room || ""; }
  ...
  useEffect(() => { setCurrentPage(1); }, [search, statusFilter, speciesFilter, sortField, sortDir]);
  ...
  useEffect(() => {
    ...
    if (!ownerSearchQuery.trim()) { run(); return; }
  ...
  useEffect(() => {
    ...
    setForm(f => (f.owner_email === generated ? f : { ...f, owner_email: generated }));
  }, [form.owner_first, form.owner_last, ownerStep]);

  if (userLoading) {
    return (
      <Layout>
        <div className="topbar" style={{ position: "sticky", top: 0, zIndex: 90, background: "#fff" }}>
  ...
  const openAdd = () => {
    setForm({ name: "", species: "", breed: "", gender: "", owner: "", owner_first: "", owner_last: "", owner_gender: "", contact: "", owner_email: "", condition: "", status: "Outpatient", health: "Good", room: "" });
    setAddPatientErrors({});
    ...
    setExistingPatients([]); setPetMode("new");
  ...
  const resolveOwnerBranchId = () => (seeAllBranches && branchFilter) ? branchFilter : (user?.branchId ?? null);
  ...
  const ensureProfile = async (userId, email, fullName, sex = null) => {
    ...
    if (ensureProfileError) console.error("ensureProfile upsert failed:", ensureProfileError.message);
    // Log so the account shows up in Admin & Security → Logs, tied to the staff member who created it
    if (user) {
      await supabase.from("activity_logs").insert([{
        user_id: user.id,
        user_name: user.fullName || `${user.firstName || ""} ${user.lastName || ""}`.trim() || user.email,
        user_role: user.role,
  ...
  const executeSavePatient = async ({ ownerUserId, ownerPassword, resolvedEmail, existingAccountFound = false, loginRateLimited = false }) => {
    if (form.room) {
    ...
    const patientPayload = withBranchId(user, {
      ...
      age: form.age || null, temp: form.temp || null, heart_rate: form.heart_rate || null, weight: form.weight || null,
    ...
    const parallelTasks = [];
    if (form.room) parallelTasks.push(occupyRoom(form.room, form.name, form.condition));
    if (pendingVax.length > 0) parallelTasks.push(supabase.from(T_VACCINATIONS).insert(pendingVax.map(({ _key, ...v }) => ({ ...v, patient_id: patientId }))));
    if (pendingTreat.length > 0) parallelTasks.push(supabase.from(T_TREATMENTS).insert(pendingTreat.map(({ _key, ...t }) => ({ ...t, patient_id: patientId }))));
    ...
    if (ownerUserId) {
      ...
      if (staffUser?.id) {
        ...
        supabase.from(T_MESSAGES).insert([withBranchId(user, {
        ...
        })]).then(({ error: msgError }) => { if (msgError) console.error('Welcome message failed:', msgError.message); });
      }
    }
    setSavingPatient(false);
    fetchPatients(); fetchRooms(); closeModal();
    logActivity(user, 'Created patient record', `Added new patient: ${form.name}`);
  ...
  const validateAddPatient = () => {
    ...
    else if (form.name.trim().length < 2) errs.name = "Patient name must be at least 2 characters";
    else if (form.name.trim().length > 50) errs.name = "Patient name must not exceed 50 characters";
    if (!form.species) errs.species = "Please select a species";
    if (ownerStep === OWNER_STEPS.FORM) {
      if (!form.owner_first.trim() && !form.owner_last.trim()) errs.owner_first = "Enter at least a first or last name";
      else if (form.owner_first.trim() && form.owner_first.trim().length < 2) errs.owner_first = "First name must be at least 2 characters";
    }
    if (ownerStep === OWNER_STEPS.SEARCH && !selectedOwnerProfile) errs.owner_search = "Please select a customer account";
    if (form.contact && form.contact.length !== 11) errs.contact = "Contact number must be 11 digits";
    return errs;
  };

  const isAddPatientFormValid = () => Object.keys(validateAddPatient()).length === 0;

  const savePatient = async () => {
    const errs = validateAddPatient();
    if (Object.keys(errs).length) {
      setAddPatientErrors(errs);
      showAlert("Missing Fields", "Please fill in all required fields before filing this record.");
      return;
    }
    ...
    if (selectedOwnerProfile) {
      ...
      await ensureProfile(ownerUserId, selectedOwnerProfile.email, selectedOwnerProfile.full_name || form.owner, form.owner_gender);
    ...
    if (ownerEmail) {
      ...
      const { data: { session: staffSession } } = await supabase.auth.getSession();
      ...
      if (signUpError) {
        if (signUpError.message?.toLowerCase().includes("already registered")) {
          const { data: existing } = await supabase.from(T_PROFILES).select("id, first_name, last_name, email").eq("email", ownerEmail).single();
          ownerUserId = existing?.id || null;
          const existingFullName = existing ? [existing.first_name, existing.last_name].filter(Boolean).join(" ") : "";
          if (ownerUserId) await ensureProfile(ownerUserId, ownerEmail, existingFullName || fullOwnerName || ownerEmail, form.owner_gender);
          setSavingPatient(false);
          setExistingAccModal({
            show: true, email: ownerEmail, existingName: existingFullName || "",
            onContinue: async () => {
          ...
          const { data: rateLimited } = await supabase.from(T_PROFILES).select("id").eq("email", ownerEmail).single();
          const rateLimitedUserId = rateLimited?.id || null;
          if (rateLimitedUserId) await ensureProfile(rateLimitedUserId, ownerEmail, fullOwnerName || ownerEmail, form.owner_gender);
          await executeSavePatient({ ownerUserId: rateLimitedUserId, ownerPassword, resolvedEmail: ownerEmail, loginRateLimited: true });
          return;
        } else {
        ...
        if (ownerUserId) await ensureProfile(ownerUserId, ownerEmail, fullOwnerName || ownerEmail, form.owner_gender);
      }
    }
  ...
  const doDelete = async (id) => {
    ...
    const { error } = await supabase.from(T_PATIENTS).update({ deleted_at: new Date().toISOString() }).eq("id", id);
    if (error) { showAlert("Error", error.message); return; }
    logActivity(user, 'Deleted patient record', `Moved to Recently Deleted: ${patient?.name || id}`);
    if (patient?.room) await freeRoom(patient.room);
    showToast(`${patient?.name || 'Patient'} moved to Recently Deleted — permanently removed in 30 days`, 'info');
    fetchPatients(); fetchRooms(); fetchDeletedPatients(); closeModal();
  };

  const restorePatient = async (id, name) => {
    const { error } = await supabase.from(T_PATIENTS).update({ deleted_at: null }).eq("id", id);
    if (error) { showAlert("Error", error.message); return; }
    logActivity(user, 'Restored patient record', `Restored patient: ${name || id}`);
    showToast(`${name || 'Patient'} restored`);
    fetchPatients(); fetchDeletedPatients();
  };

  const permanentlyDeletePatient = (id, name) => showConfirm(
    "Delete Permanently",
    `Permanently delete ${name || 'this patient'}? This cannot be undone.`,
    async () => {
      const { error } = await supabase.from(T_PATIENTS).delete().eq("id", id);
      if (error) { showAlert("Error", error.message); return; }
      logActivity(user, 'Permanently deleted patient record', `Removed patient: ${name || id}`);
  ...
  const openEditPatient = (p) => {
    const initial = { name: p.name || '', species: p.species || '', breed: p.breed || '', gender: p.gender || '', age: p.age || '', temp: p.temp || '', heart_rate: p.heart_rate || '', weight: p.weight || '', owner: p.owner || '', contact: p.contact || '', owner_email: p.owner_email || '', condition: p.condition || '', status: p.status || 'Outpatient', health: p.health || 'Good', room: p.room || '' };
    setEditPatientForm(initial);
    setEditPatientOriginal(initial);
    setEditingPatient(p); fetchRooms();
  };

  const closeEditPatient = () => { setEditingPatient(null); setEditPatientOriginal(null); };

  const hasUnsavedPatientEdits = () => editPatientOriginal && JSON.stringify(editPatientForm) !== JSON.stringify(editPatientOriginal);
  ...
  const isEditPatientFormValid = () => {
    ...
    if (editPatientForm.contact && editPatientForm.contact.length !== 11) return false;
    return true;
  };
  ...
  const saveEditPatient = async () => {
    ...
    const { error } = await supabase.from(T_PATIENTS).update({ name: editPatientForm.name, species: editPatientForm.species, breed: editPatientForm.breed, gender: editPatientForm.gender, age: editPatientForm.age || null, temp: editPatientForm.temp || null, heart_rate: editPatientForm.heart_rate || null, weight: editPatientForm.weight || null, owner: editPatientForm.owner, contact: editPatientForm.contact, owner_email: editPatientForm.owner_email, condition: editPatientForm.condition, status: editPatientForm.status, health: editPatientForm.health, room: editPatientForm.room || null }).eq('id', editingPatient.id);
    setEditPatientSaving(false);
    if (error) { showAlert('Error', error.message); return; }
    logActivity(user, 'Updated patient record', `Edited patient: ${editPatientForm.name}`);
  ...
  const saveVax = async () => {
    ...
    setVaxSaving(false);
    if (error) { showAlert("Error", error.message); return; }
    setVaxForm({ name: "", date_given: new Date().toISOString().slice(0, 10), next_due: "", given_by: "", doses: "", lot_number: "" });
    setShowVaxForm(false); showToast('✓ Vaccination record saved'); await fetchMedical(selectedPatient.id);
  };
  const startEditVax = (v) => { setEditingVaxId(v.id); setEditVaxForm({ name: v.name, date_given: v.date_given, next_due: v.next_due || "", given_by: v.given_by || "", doses: v.doses || "", lot_number: v.lot_number || "" }); setShowVaxForm(false); };
  ...
  const deleteVax = (vaxId) => { const v = vaccinations.find(x => x.id === vaxId); requireTypeToDelete("Delete Vaccination", `This will permanently delete the "${v?.name || 'vaccination'}" record. This cannot be undone.`, "DELETE", async () => { await supabase.from(T_VACCINATIONS).delete().eq("id", vaxId); showToast('Vaccination record deleted', 'info'); await fetchMedical(selectedPatient.id); }); };

  const printVaccination = (v) => {
    const patient = selectedPatient;
    const win = window.open('', '_blank', 'width=650,height=800');
    if (!win) { showAlert('Popup Blocked', 'Please allow popups for this site to print the record.'); return; }
    const isExpired = v.next_due && new Date(v.next_due) < new Date();
    win.document.write(`
      <html>
        <head>
  ...
  const startEditTreat = (t) => { setEditingTreatId(t.id); setEditTreatForm({ date: t.date, diagnosis: t.diagnosis, notes: t.notes || "", vet: t.vet || "", temp: t.temp || "", heart_rate: t.heart_rate || "", weight: t.weight || "" }); setShowTreatForm(false); };
  ...
  const deleteTreat = (treatId) => { const t = treatments.find(x => x.id === treatId); requireTypeToDelete("Delete Treatment", `This will permanently delete the treatment record${t?.diagnosis ? ` for "${t.diagnosis}"` : ''}${t?.date ? ` (${t.date})` : ''}. This cannot be undone.`, "DELETE", async () => { await supabase.from(T_TREATMENTS).delete().eq("id", treatId); showToast('Treatment record deleted', 'info'); await fetchMedical(selectedPatient.id); }); };
  ...
  const saveRx = async () => {
    ...
    setRxSaving(false);
    if (error) { showAlert("Error", error.message); return; }
    setRxForm({ medicine: "", concentration: "", drug_form: "", dosage: "", frequency: "Once daily", route: "Oral", duration: "", instructions: "", prescribed_by: "", date_prescribed: new Date().toISOString().slice(0, 10) });
  ...
  const startEditRx = (rx) => {
    ...
    setEditRxForm({ medicine: rx.medicine, concentration: rx.concentration || "", drug_form: rx.drug_form || "", dosage: rx.dosage, frequency: rx.frequency || "Once daily", route: rx.route || "Oral", duration: rx.duration || "", instructions: rx.instructions || "", prescribed_by: rx.prescribed_by || "", date_prescribed: rx.date_prescribed || new Date().toISOString().slice(0, 10) });
    setShowRxForm(false);
  };
  ...
  const deleteRx = (rxId) => { const rx = prescriptions.find(x => x.id === rxId); requireTypeToDelete("Delete Prescription", `This will permanently delete the "${rx?.medicine || 'prescription'}" record. This cannot be undone.`, "DELETE", async () => { await supabase.from(T_PRESCRIPTIONS).delete().eq("id", rxId); showToast('✓ Prescription deleted', 'info'); await fetchMedical(selectedPatient.id); }); };
...
const S = {
  ...
  const SectionHeader = ({ color, label, onAdd, showForm, onCancelForm }) => (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
      ...
      {!showForm
        ...
        : <button className="btn btn-ghost btn-sm pr-btn-auto" onClick={onCancelForm}>✕ Cancel</button>
      }
    </div>
  );
          ...
          {selectedOwnerProfile ? (
            ...
            <>
              <div style={{ position: "relative", marginBottom: 10 }}>
                <input autoFocus type="text" value={ownerSearchQuery} onChange={e => { setOwnerSearchQuery(e.target.value); setAddPatientErrors(er => ({ ...er, owner_search: "" })); }} placeholder="Search by customer name or email…"
                  style={{ width: "100%", padding: "10px 12px", border: `1.5px solid ${addPatientErrors.owner_search ? "#ef4444" : "var(--border)"}`, borderRadius: 8, fontSize: 13, color: "var(--text)", boxSizing: "border-box", background: "var(--bg)", outline: "none", fontFamily: "inherit" }} />
              </div>
              {addPatientErrors.owner_search && <p style={{ fontSize: 11, color: "#dc2626", margin: "0 0 10px" }}>{addPatientErrors.owner_search}</p>}
              {ownerSearchLoading && (
              ...
              {ownerSearchRes.length > 0 && (
                <div style={{ border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden", marginBottom: 4 }}>
                  {ownerSearchRes.map((p, i) => (
                    <div key={p.id} onClick={() => { setSelectedOwnerProfile(p); setForm(prev => ({ ...prev, owner: p.full_name || prev.owner, owner_email: p.email })); setPetMode("new"); fetchExistingPatientsForOwner(p.full_name, p.id); }}
                      style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderBottom: i < ownerSearchRes.length - 1 ? "1px solid var(--border)" : "none", cursor: "pointer", background: "var(--card)", transition: "background 0.12s" }}
  ...
  const VaxEditRow = ({ v }) => (
    <div style={{ background: "#f0fdf4", border: "1.5px solid #86efac", borderRadius: 10, padding: 20, marginBottom: 8 }}>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 8 }}>
        <button className="btn btn-ghost pr-btn-auto" onClick={() => setEditingVaxId(null)}>Cancel</button>
        <button className="btn btn-primary pr-btn-auto" onClick={saveEditVax} disabled={vaxSaving}>{vaxSaving ? "Saving..." : "Save Changes"}</button>
      </div>
    </div>
  );
  ...
  const TreatEditRow = ({ t }) => (
    <div style={{ background: "#fffbeb", border: "1.5px solid #fde68a", borderRadius: 10, padding: 20, marginBottom: 8 }}>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 8 }}>
        <button className="btn btn-ghost pr-btn-auto" onClick={() => setEditingTreatId(null)}>Cancel</button>
        <button className="btn btn-primary pr-btn-auto" onClick={saveEditTreat} disabled={treatSaving}>{treatSaving ? "Saving..." : "Save Changes"}</button>
      </div>
    </div>
  );
    ...
    <Layout>
      ...
      {typeDeleteModal.show && (
        <div className="pr-overlay" style={{ zIndex: 1200 }}>
          <div style={{ background: "var(--card)", borderRadius: 14, boxShadow: "0 24px 64px rgba(0,0,0,0.30)", width: "100%", maxWidth: 420, overflow: "hidden" }}>
            <div style={{ padding: "20px 24px 4px" }}>
              <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: "#dc2626" }}>{typeDeleteModal.title}</h3>
              <p style={{ margin: "8px 0 0", fontSize: 13, color: "var(--muted)", lineHeight: 1.5 }}>{typeDeleteModal.message}</p>
            </div>
            <div style={{ padding: "16px 24px" }}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 6 }}>
                Type <span style={{ color: "#dc2626" }}>{typeDeleteModal.recordLabel}</span> to confirm
              </label>
              <input
                type="text"
                autoFocus
                value={typeDeleteModal.typed}
                onChange={e => setTypeDeleteModal(m => ({ ...m, typed: e.target.value }))}
                onKeyDown={e => { if (e.key === "Enter" && typeDeleteModal.typed === typeDeleteModal.recordLabel) { const fn = typeDeleteModal.onConfirm; closeTypeDeleteModal(); fn && fn(); } }}
                placeholder={typeDeleteModal.recordLabel}
                style={{ width: "100%", border: "1.5px solid var(--border)", borderRadius: 8, padding: "9px 12px", fontSize: 13, fontWeight: 600, color: "var(--text)", outline: "none", fontFamily: "inherit", boxSizing: "border-box" }}
              />
            </div>
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, padding: "14px 24px", borderTop: "1px solid var(--border)" }}>
              <button className="btn btn-ghost pr-btn-auto" onClick={closeTypeDeleteModal}>Cancel</button>
              <button
                className="btn pr-btn-auto"
                disabled={typeDeleteModal.typed !== typeDeleteModal.recordLabel}
                onClick={() => { const fn = typeDeleteModal.onConfirm; closeTypeDeleteModal(); fn && fn(); }}
                style={{
                  background: typeDeleteModal.typed === typeDeleteModal.recordLabel ? "#dc2626" : "#fca5a5",
                  color: "#fff", border: "none", borderRadius: 8, padding: "8px 16px", fontSize: 13, fontWeight: 700,
                  cursor: typeDeleteModal.typed === typeDeleteModal.recordLabel ? "pointer" : "not-allowed",
                  fontFamily: "inherit",
                }}
              >
                Delete Record
      ...
      {existingAccModal.show && (
        <div className="pr-overlay" style={{ zIndex: 1100 }}>
          <div style={{ background: "var(--card)", borderRadius: 14, boxShadow: "0 24px 64px rgba(0,0,0,0.30)", width: "100%", maxWidth: 440, overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, padding: "14px 24px", borderTop: "1px solid var(--border)" }}>
              <button className="btn btn-ghost pr-btn-auto" onClick={() => setExistingAccModal(m => ({ ...m, show: false }))}>Cancel</button>              <button className="btn btn-primary pr-btn-auto" style={{ background: "#f59e0b", borderColor: "#f59e0b" }} onClick={existingAccModal.onContinue}>Got it — Continue Saving</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* ── TOPBAR ── */}
      <div className="topbar patient-record-topbar" style={{ position: "fixed", top: 68, left: "var(--current-sidebar-w, 62px)", right: 0, zIndex: 40, background: "#fff" }}>
        <div className="topbar-title">
        ...
        <div className="topbar-actions">
          ...
          >
            ...
            Recently Deleted {deletedPatients.length > 0 ? `(${deletedPatients.length})` : ""}
          </button>
          <div className="fab-wrap" style={{ position: "fixed", bottom: 28, right: 28, zIndex: 999, display: "flex", alignItems: "center", gap: 10 }}
            ...
            <span className="fab-tooltip" style={{
              ...
              <span style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.2 }}>
                <span style={{ fontSize: 12, fontWeight: 800, color: '#fff' }}>Add Patient</span>
                <span style={{ fontSize: 10, fontWeight: 500, color: 'rgba(255,255,255,0.55)' }}>Register new record</span>
              </span>
              {/* Arrow pointer */}
              <span style={{
      ...
      <div className="content" style={{ paddingTop: 70, marginTop: 0 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 14, marginBottom: 24, marginTop: 12, overflowX: 'hidden', width: '100%', boxSizing: 'border-box' }}>
          {loading ? (
            [1, 2, 3, 4].map(i => <StatCardSkeleton key={i} />)
          ) : (
            [
              { label: "Total Patients", value: patients.length, icon: "/icon/attended.png", color: "blue", sub: "All registered patients", filterValue: "all" },
              { label: "Admitted", value: patients.filter(p => p.status === "Admitted").length, icon: "/icon/admitted.png", color: "green", sub: "Currently admitted", filterValue: "Admitted" },
              { label: "Outpatient", value: patients.filter(p => p.status === "Outpatient").length, icon: "/icon/outpatient.png", color: "yellow", sub: "Outpatient visits", filterValue: "Outpatient" },
              { label: "Critical", value: patients.filter(p => p.health === "Critical").length, icon: "/icon/critical.png", color: "red", sub: patients.filter(p => p.health === "Critical").length > 0 ? "Needs attention" : "All stable", filterValue: "Critical" },
            ].map((sc, i) => (
              <div
                key={i}
                ...
                onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setStatusFilter(sc.filterValue); } }}
              >
        ...
        <div className="pr-card">
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", borderBottom: "1px solid var(--border)", flexWrap: "wrap", gap: 10 }}>
            ...
            <div className="fade-in" style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
              {[
                { label: "All", value: "all" },
              ...
              {[
                ...
                <button key={f.value} onClick={() => setSpeciesFilter(f.value)}
                  style={{
                    padding: "5px 14px", borderRadius: 20, fontSize: 12, fontWeight: 600,
                    cursor: "pointer", fontFamily: "inherit", border: "1.5px solid",
                    background: speciesFilter === f.value ? "var(--royal)" : "transparent",
                    color: speciesFilter === f.value ? "#fff" : "var(--muted)",
                    borderColor: speciesFilter === f.value ? "var(--royal)" : "var(--border)",
                    transition: "all 0.15s",
                  }}>
                  {f.label}
                </button>
              ...
              <span style={{ color: "var(--muted)", fontSize: 12, marginLeft: 6 }}>{sortedFiltered.length} record{sortedFiltered.length !== 1 ? "s" : ""}</span>
            </div>
          </div>
          <div style={{ overflowX: "auto" }}>
            {loading ? (
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 780 }}>
                <thead>
                  <tr>{["Patient", "Owner", "Condition", "Status", "Health", "Room", "Registered", "Actions"].map(h => <th key={h} className="pr-th">{h}</th>)}</tr>
                </thead>
                <tbody>
                  {paginated.length === 0 ? (
                    <tr>
                      <td colSpan={8} style={{ textAlign: "center", padding: "48px 20px", color: "var(--muted)" }}>
                    ...
                    return (
                      <tr
                        key={p.id}
                        className="fade-in"
                        style={{ cursor: "pointer", background: "var(--card)", animationDelay: `${idx * 0.06}s` }}
                        onClick={() => openView(p)}
                      >
                        {/* Patient */}
                        <td className="pr-td">
                        ...
                        {/* Registered date */}
                        <td className="pr-td" style={{ whiteSpace: "nowrap" }}>
                          {p.created_at
                        ...
                        {/* Actions */}
                        <td className="pr-td" style={{ textAlign: "left", padding: "8px 14px" }} onClick={e => e.stopPropagation()}>
                          <div style={{ display: "flex", gap: 6, justifyContent: "flex-start", alignItems: "center" }}>
                            {/* View */}
                            <button
                              title="View"
                              className="btn btn-sm"
                              style={{ ...S.btn, height: 28, padding: "0 10px", gap: 5, display: "inline-flex", alignItems: "center", justifyContent: "center", background: "#eff6ff", border: "1.5px solid #bfdbfe", color: "#1d4ed8", borderRadius: 20, fontSize: 11, fontWeight: 600 }}
                              onClick={() => openView(p)}
                            >
                            ...
                            <button
                              title="Edit"
                              className="btn btn-sm"
                              style={{ ...S.btn, height: 28, padding: "0 10px", gap: 5, display: "inline-flex", alignItems: "center", justifyContent: "center", background: "#f8fafc", border: "1.5px solid #e2e8f0", color: "#475569", borderRadius: 20, fontSize: 11, fontWeight: 600 }}
                            ...
                            <button
                              title="Delete"
                              className="btn btn-sm"
                              style={{ ...S.btn, height: 28, padding: "0 10px", gap: 5, display: "inline-flex", alignItems: "center", justifyContent: "center", background: "#fef2f2", border: "1.5px solid #fca5a5", color: "#dc2626", borderRadius: 20, fontSize: 11, fontWeight: 600 }}
                              onClick={() => showConfirm("Delete Patient", `Delete ${p.name}? This cannot be undone.`, () => doDelete(p.id))}
                            >
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
      ...
      {/* ── ADD PATIENT MODAL ── */}
      {activeModal === "add" && ReactDOM.createPortal(
        <div className="pr-overlay">
          <div className="pr-modal-wrap">
            <div style={{ flexShrink: 0 }}>
              {/* Clipboard top bar */}
              <div style={{ background: "linear-gradient(135deg, #0f172a, #1e3a8a)", padding: "10px 24px", display: "flex", alignItems: "center", justifyContent: "flex-end" }}>
              ...
              {activeTab === "info" && ownerIsConfirmed && (
                <div style={{ paddingTop: 4 }}>
                  ...
                  <div style={{ borderBottom: "1.5px solid #e2e8f0", marginBottom: 0 }}>
                    <div style={{ background: "var(--bg)", borderBottom: "1px solid var(--border)", padding: "6px 16px" }}>
                      <span style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: "1px", color: "var(--muted)" }}>Patient Information</span>
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", borderBottom: "1px solid #e2e8f0" }}>
                      <div style={{ padding: "10px 14px", borderRight: "1px solid #e2e8f0" }}>
                       <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 6 }}>Patient Name <span style={{ color: "#ef4444" }}>*</span></div>
                        <input type="text" value={form.name} maxLength={50} onChange={e => { setForm({ ...form, name: sanitizeName(e.target.value).slice(0, 50) }); setAddPatientErrors(er => ({ ...er, name: "" })); }} placeholder="e.g. Buddy"
                          style={{ width: "100%", border: "none", borderBottom: `1.5px solid ${addPatientErrors.name ? "#ef4444" : "#cbd5e1"}`, background: "transparent", fontSize: 13, fontWeight: 600, color: "var(--text)", outline: "none", padding: "2px 0", fontFamily: "inherit", boxSizing: "border-box" }} />
                        {addPatientErrors.name && <p style={{ fontSize: 10, color: "#dc2626", margin: "4px 0 0" }}>{addPatientErrors.name}</p>}
                      </div>
                      <div style={{ padding: "10px 14px", borderRight: "1px solid #e2e8f0" }}>
                      ...
                      <div style={{ padding: "10px 14px" }}>
                        ...
                        <CustomSelect
                          value={form.species}
                          onChange={val => { setForm({ ...form, species: val }); setAddPatientErrors(er => ({ ...er, species: "" })); }}
                          placeholder="—"
                          options={["Dog", "Cat"]}
                        />
                        {addPatientErrors.species && <p style={{ fontSize: 10, color: "#dc2626", margin: "4px 0 0" }}>{addPatientErrors.species}</p>}
                      </div>
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", borderBottom: "1px solid #e2e8f0" }}>
                      <div style={{ padding: "10px 14px", borderRight: "1px solid #e2e8f0" }}>
                        ...
                        <CustomSelect
                          ...
                          options={form.species === "Cat" ? CAT_BREEDS : form.species === "Dog" ? DOG_BREEDS : []}
                        />
                      </div>
                      <div style={{ padding: "10px 14px", borderRight: "1px solid #e2e8f0" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr" }}>
                      <div style={{ padding: "10px 14px", borderRight: "1px solid #e2e8f0" }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 6 }}>Age <span style={{ fontWeight: 400, textTransform: "none" }}>(optional)</span></div>
                        <CustomSelect
                          value={form.age}
                      ...
                      <div style={{ padding: "10px 14px", borderRight: "1px solid #e2e8f0" }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 6 }}>Temp (°C) <span style={{ fontWeight: 400, textTransform: "none" }}>(optional)</span></div>
                        <input type="text" inputMode="decimal" value={form.temp} onChange={e => setForm({ ...form, temp: e.target.value.replace(/[^0-9.]/g, '') })} placeholder="e.g. 38.5"
                      ...
                      <div style={{ padding: "10px 14px", borderRight: "1px solid #e2e8f0" }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 6 }}>Heart Rate (bpm) <span style={{ fontWeight: 400, textTransform: "none" }}>(optional)</span></div>
                        <input type="text" inputMode="numeric" value={form.heart_rate} onChange={e => setForm({ ...form, heart_rate: e.target.value.replace(/\D/g, '') })} placeholder="e.g. 120"
                          style={{ width: "100%", border: "none", borderBottom: "1.5px solid #cbd5e1", background: "transparent", fontSize: 13, fontWeight: 600, color: "var(--text)", outline: "none", padding: "2px 0", fontFamily: "inherit", boxSizing: "border-box" }} />
                      </div>
                      <div style={{ padding: "10px 14px" }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 6 }}>Weight (kg) <span style={{ fontWeight: 400, textTransform: "none" }}>(optional)</span></div>
                        <input type="text" inputMode="decimal" value={form.weight} onChange={e => setForm({ ...form, weight: e.target.value.replace(/[^0-9.]/g, '') })} placeholder="e.g. 8.2"
                          style={{ width: "100%", border: "none", borderBottom: "1.5px solid #cbd5e1", background: "transparent", fontSize: 13, fontWeight: 600, color: "var(--text)", outline: "none", padding: "2px 0", fontFamily: "inherit", boxSizing: "border-box" }} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ borderBottom: "1.5px solid #e2e8f0" }}>
                    <div style={{ background: "var(--bg)", borderBottom: "1px solid var(--border)", padding: "6px 16px" }}>
                      <span style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: "1px", color: "var(--muted)" }}>Owner / Admittance</span>
                    </div>
                    {ownerStep === OWNER_STEPS.FORM ? (
                      <>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderBottom: "1px solid #e2e8f0" }}>
                          <div style={{ padding: "10px 14px", borderRight: "1px solid #e2e8f0" }}>
                            ...
                            <input type="text" value={form.owner_first} onChange={e => { setForm({ ...form, owner_first: sanitizeName(e.target.value) }); setAddPatientErrors(er => ({ ...er, owner_first: "" })); }} placeholder="e.g. Juan"
                              style={{ width: "100%", border: "none", borderBottom: `1.5px solid ${addPatientErrors.owner_first ? "#ef4444" : "#cbd5e1"}`, background: "transparent", fontSize: 13, fontWeight: 600, color: "var(--text)", outline: "none", padding: "2px 0", fontFamily: "inherit", boxSizing: "border-box" }} />
                            {addPatientErrors.owner_first && <p style={{ fontSize: 10, color: "#dc2626", margin: "4px 0 0" }}>{addPatientErrors.owner_first}</p>}
                          </div>
                          <div style={{ padding: "10px 14px" }}>
                           <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 6 }}>Owner Last Name <span style={{ color: "#ef4444" }}>*</span></div>
                            <input type="text" value={form.owner_last} onChange={e => { setForm({ ...form, owner_last: sanitizeName(e.target.value) }); setAddPatientErrors(er => ({ ...er, owner_first: "" })); }} placeholder="e.g. dela Cruz"
                              style={{ width: "100%", border: "none", borderBottom: "1.5px solid #cbd5e1", background: "transparent", fontSize: 13, fontWeight: 600, color: "var(--text)", outline: "none", padding: "2px 0", fontFamily: "inherit", boxSizing: "border-box" }} />                          </div>
                        </div>
                        <div style={{ borderBottom: "1px solid #e2e8f0" }}>
                          <div style={{ padding: "10px 14px" }}>
                            <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 6 }}>Owner Sex / Gender</div>
                            <CustomSelect
                              value={form.owner_gender}
                              onChange={val => setForm({ ...form, owner_gender: val })}
                              placeholder="—"
                              options={["Male", "Female"]}
                            />
                          </div>
                      ...
                      <div style={{ padding: "10px 14px", borderBottom: "1px solid #e2e8f0" }}>
                        ...
                        <input type="text" value={form.owner} onChange={e => setForm({ ...form, owner: sanitizeName(e.target.value) })} readOnly={!!selectedOwnerProfile}
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderBottom: "1px solid #e2e8f0" }}>
                      <div style={{ padding: "10px 14px", borderRight: "1px solid #e2e8f0" }}>
                        ...
                        <input type="text" value={form.contact} onChange={e => setForm({ ...form, contact: sanitizeContact(e.target.value) })} placeholder="e.g. 09170000000"
                          inputMode="numeric" maxLength={11}
                          style={{ width: "100%", border: "none", borderBottom: "1.5px solid #cbd5e1", background: "transparent", fontSize: 13, fontWeight: 600, color: "var(--text)", outline: "none", padding: "2px 0", fontFamily: "inherit", boxSizing: "border-box" }} />
                        {form.contact && form.contact.length !== 11 && (
                          <p style={{ fontSize: 10, color: "#dc2626", margin: "4px 0 0" }}>Must be 11 digits.</p>
                        )}
                        ...
                        <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 4 }}>
                          ...
                          {selectedOwnerProfile
                            ...
                            : ownerStep === OWNER_STEPS.FORM
                              ? <span style={{ marginLeft: 6, color: "#94a3b8", fontWeight: 400, textTransform: "none", letterSpacing: 0, fontSize: 10 }}>(auto-generated)</span>
                              : <span style={{ marginLeft: 6, color: "#94a3b8", fontWeight: 400, textTransform: "none", letterSpacing: 0, fontSize: 10 }}>(creates login)</span>}
                        </div>
                        <input type="email" value={form.owner_email} onChange={e => setForm({ ...form, owner_email: e.target.value })} placeholder="Enter owner first/last name above" readOnly={!!selectedOwnerProfile || ownerStep === OWNER_STEPS.FORM}
                          style={{ width: "100%", border: "none", borderBottom: "1.5px solid #cbd5e1", background: "transparent", fontSize: 13, fontWeight: 600, color: (selectedOwnerProfile || ownerStep === OWNER_STEPS.FORM) ? "#94a3b8" : "var(--text)", outline: "none", padding: "2px 0", fontFamily: "inherit", boxSizing: "border-box" }} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ borderBottom: "1.5px solid #e2e8f0" }}>
                    <div style={{ background: "var(--bg)", borderBottom: "1px solid var(--border)", padding: "6px 16px" }}>
                      <span style={{ fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: "1px", color: "var(--muted)" }}>Initial Condition / Diagnosis</span>
                    </div>
                ...
                <div style={{ paddingTop: 4 }}>
                  ...
                  {showAddVaxForm && (
                    <div className="pr-form-box">
                      ...
                      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 8 }}>
                        <button className="btn btn-ghost pr-btn-auto" onClick={() => setShowAddVaxForm(false)}>Cancel</button>
                        <button className="btn btn-primary pr-btn-auto" onClick={addPendingVax}>Add to List</button>
                      </div>
                    </div>
                  )}
                ...
                <div style={{ paddingTop: 4 }}>
                  ...
                  {showAddTreatForm && (
                    <div className="pr-form-box">
                      ...
                      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 8 }}>
                        <button className="btn btn-ghost pr-btn-auto" onClick={() => setShowAddTreatForm(false)}>Cancel</button>
                        <button className="btn btn-primary pr-btn-auto" onClick={addPendingTreat}>Add to List</button>
                      </div>
                    </div>
                  )}
            ...
            <div className="pr-modal-footer" style={{ background: "var(--bg)", borderTop: "2px solid var(--border)", gap: 8 }}>
              {ownerIsConfirmed && activeTab !== "info" && (
                <button className="btn btn-ghost pr-btn-auto" onClick={() => setActiveTab("info")}>← Back to Info</button>
              )}
              <div style={{ flex: 1 }} />
              <button className="btn btn-ghost pr-btn-auto" onClick={closeModal}>Cancel</button>
              {ownerIsConfirmed && (
                <button
                  ...
                  style={{
                    ...
                    opacity: (!isAddPatientFormValid() || savingPatient) ? 0.5 : 1,
                    cursor: (!isAddPatientFormValid() || savingPatient) ? "not-allowed" : "pointer",
                  }}
                  onClick={savePatient}
      ...
      {activeModal === "view" && selectedPatient && (
        <div className="pr-overlay">
          <div className="pr-modal-wrap">
           <div className="pr-modal-header" style={{ background: 'var(--bg)', borderBottom: '1px solid var(--border)' }}>              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{
                  width: 40, height: 40, borderRadius: 12,
                  background: selectedPatient.species === "Cat" ? "#f0fdf4" : "#eff6ff",
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  flexShrink: 0,
                }}>
            ...
            <div className="pr-modal-body">
              ...
              {/* ── INFO TAB ── */}
              {activeTab === "info" && <PatientInfoTab patient={selectedPatient} treatments={treatments} />}
              ...
              {activeTab === "vaccination" && (
                <div style={{ paddingTop: 4 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
                    ...
                    {!showVaxForm
                      ? <button className="btn btn-primary pr-btn-auto" style={{ background: '#16a34a', borderColor: '#16a34a', display: 'inline-flex', alignItems: 'center', gap: 5 }} onClick={() => { setShowVaxForm(true); setEditingVaxId(null); }}>
                        <Ic src={plusIcon} size={12} style={{ mixBlendMode: "normal", filter: "brightness(0) invert(1)" }} /> Add Vaccine
                      </button>
                      : <button className="btn btn-ghost btn-sm pr-btn-auto" onClick={() => setShowVaxForm(false)}>✕ Cancel</button>
                    }
                  </div>
                  ...
                  {showVaxForm && (
                    <div style={{ ...S.formBox, background: '#f0fdf4', border: '1.5px solid #86efac', marginBottom: 20 }}>
                      ...
                      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 8 }}>
                        <button className="btn btn-ghost pr-btn-auto" onClick={() => setShowVaxForm(false)}>Cancel</button>
                      ...
                      {vaccinations.map(v => (
                        <React.Fragment key={v.id}>
                          {editingVaxId === v.id ? (
                            ...
                            <VaxCard v={v} onEdit={startEditVax} onDelete={deleteVax} onPrint={printVaccination} isEditing={editingVaxId === v.id} />
                          )}                        </React.Fragment>
                      ))}
                    </div>
                  )}
              ...
              {activeTab === "treatment" && (
                <div style={{ paddingTop: 4 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
                    ...
                    {!showTreatForm
                      ? <button className="btn btn-primary pr-btn-auto" style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }} onClick={() => { setShowTreatForm(true); setEditingTreatId(null); }}>
                        <Ic src={plusIcon} size={12} style={{ mixBlendMode: "normal", filter: "brightness(0) invert(1)" }} /> Add Treatment
                      </button>
                      : <button className="btn btn-ghost btn-sm pr-btn-auto" onClick={() => setShowTreatForm(false)}>✕ Cancel</button>
                    }
                  </div>
                  ...
                  {showTreatForm && (
                    <div style={{ ...S.formBox, marginBottom: 20 }}>
                      ...
                      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 8 }}>
                        <button className="btn btn-ghost pr-btn-auto" onClick={() => setShowTreatForm(false)}>Cancel</button>
                        <button className="btn btn-primary pr-btn-auto" onClick={saveTreat} disabled={treatSaving}>{treatSaving ? "Saving..." : "Save Treatment"}</button>
                      </div>
                    </div>
                  )}
              ...
              {activeTab === "prescription" && (
                <div style={{ paddingTop: 4 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
                    ...
                    {!showRxForm
                      ? <button className="btn btn-primary pr-btn-auto" style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }} onClick={() => { setShowRxForm(true); setEditingRxId(null); }}>
                        <Ic src={plusIcon} size={12} style={{ mixBlendMode: "normal", filter: "brightness(0) invert(1)" }} /> Add Prescription
                      </button>
                      : <button className="btn btn-ghost btn-sm pr-btn-auto" onClick={() => setShowRxForm(false)}>✕ Cancel</button>
                    }
                  </div>
                  ...
                  {showRxForm && (
                    <div style={{ ...S.formBox, marginBottom: 20 }}>
                      <div className="form-grid">
                        <div className="form-group"><label>Drug Name *</label><input type="text" value={rxForm.medicine} onChange={e => setRxForm({ ...rxForm, medicine: e.target.value })} placeholder="e.g. Amoxicillin" /></div>
                        <div className="form-group"><label>Concentration</label><input type="text" value={rxForm.concentration} onChange={e => setRxForm({ ...rxForm, concentration: e.target.value })} placeholder="e.g. 50mg/ml" /></div>
                        <div className="form-group"><label>Form</label><select value={rxForm.drug_form} onChange={e => setRxForm({ ...rxForm, drug_form: e.target.value })}><option value="">— Select —</option>{DRUG_FORM_OPTIONS.map(f => <option key={f}>{f}</option>)}</select></div>
                        ...
                        <div className="form-group"><label>Prescribed By</label><CustomSelect value={rxForm.prescribed_by} onChange={val => setRxForm({ ...rxForm, prescribed_by: val })} placeholder="— Select Vet —" options={VET_OPTIONS} /></div>
                        <div className="form-group form-full"><label>Notes</label><textarea value={rxForm.instructions} onChange={e => setRxForm({ ...rxForm, instructions: e.target.value })} placeholder="Directions and special instructions..." style={{ minHeight: 72 }} /></div>
                      </div>
                      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 4 }}>
                        <button className="btn btn-ghost pr-btn-auto" onClick={() => setShowRxForm(false)}>Cancel</button>
                        <button className="btn btn-primary pr-btn-auto" onClick={saveRx} disabled={rxSaving}>{rxSaving ? "Saving..." : "Save Prescription"}</button>
                      </div>
                    </div>
                  )}
                    ...
                    <div key={rx.id}>
                      {editingRxId === rx.id ? (
                        <div style={{ background: "#eff6ff", border: "1.5px solid #bfdbfe", borderRadius: 10, padding: 20, marginBottom: 12 }}>
                          ...
                          <div className="form-grid">
                            <div className="form-group"><label>Drug Name *</label><input type="text" value={editRxForm.medicine} onChange={e => setEditRxForm({ ...editRxForm, medicine: e.target.value })} /></div>
                            <div className="form-group"><label>Concentration</label><input type="text" value={editRxForm.concentration} onChange={e => setEditRxForm({ ...editRxForm, concentration: e.target.value })} placeholder="e.g. 50mg/ml" /></div>
                            <div className="form-group"><label>Form</label><select value={editRxForm.drug_form} onChange={e => setEditRxForm({ ...editRxForm, drug_form: e.target.value })}><option value="">— Select —</option>{DRUG_FORM_OPTIONS.map(f => <option key={f}>{f}</option>)}</select></div>
                            ...
                            <div className="form-group"><label>Prescribed By</label><CustomSelect value={editRxForm.prescribed_by} onChange={val => setEditRxForm({ ...editRxForm, prescribed_by: val })} placeholder="— Select Vet —" options={VET_OPTIONS} /></div>
                            <div className="form-group form-full"><label>Notes</label><textarea value={editRxForm.instructions} onChange={e => setEditRxForm({ ...editRxForm, instructions: e.target.value })} style={{ minHeight: 72 }} /></div>
                          </div>
                          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 8 }}>
                            <button className="btn btn-ghost pr-btn-auto" onClick={() => setEditingRxId(null)}>Cancel</button>
                            <button className="btn btn-primary pr-btn-auto" onClick={saveEditRx} disabled={rxSaving}>{rxSaving ? "Saving..." : "Save Changes"}</button>
                          </div>
                        </div>
                      ) : (
            ...
            <div className="pr-modal-footer">
              <button className="btn btn-ghost pr-btn-auto" onClick={closeModal}>Close</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="pr-overlay">
          <div className="pr-modal-wrap" style={{ maxWidth: 640 }}>
            <div className="pr-modal-header">
              <div>
                <h3 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: "var(--text)" }}>Recently Deleted</h3>
                <p style={{ margin: 0, fontSize: 12, color: "var(--muted)", marginTop: 2 }}>Patients are permanently removed 30 days after deletion.</p>
              </div>
              <button onClick={() => setShowDeletedModal(false)} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "var(--muted)", lineHeight: 1, padding: "2px 6px" }}>✕</button>
            </div>
            <div className="pr-modal-body">
              {deletedPatients.length === 0 ? (
                <div style={{ textAlign: "center", padding: "40px 0", color: "var(--muted)" }}>
                  <p style={{ fontSize: 13, margin: 0 }}>No recently deleted patients.</p>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {deletedPatients.map(p => {
                    const daysLeft = Math.max(0, 30 - Math.floor((Date.now() - new Date(p.deleted_at).getTime()) / (24 * 60 * 60 * 1000)));
                    return (
                      <div key={p.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, border: "1px solid var(--border)", borderRadius: 10, padding: "12px 14px", flexWrap: "wrap" }}>
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 13, color: "var(--text)" }}>{p.name}</div>
                          <div style={{ fontSize: 11, color: "var(--muted)" }}>{p.species}{p.breed ? ` · ${p.breed}` : ""}{p.owner ? ` · Owner: ${p.owner}` : ""}</div>
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                          <button onClick={() => restorePatient(p.id, p.name)} style={{ background: "#f0fdf4", border: "1.5px solid #86efac", color: "#166534", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Restore</button>
                          <button onClick={() => permanentlyDeletePatient(p.id, p.name)} style={{ background: "#fef2f2", border: "1.5px solid #fca5a5", color: "#dc2626", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Delete Now</button>
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="pr-modal-footer">
              <button className="btn btn-ghost pr-btn-auto" onClick={() => setShowDeletedModal(false)}>Close</button>
            </div>
          </div>
        </div>
      )}

      {/* ── EDIT PATIENT MODAL ── */}      {editingPatient && (
          ...
          <div className="pr-modal-wrap" style={{ maxWidth: 680 }}>
            <div className="pr-modal-header">
              <div>
               ...
               <button onClick={attemptCloseEditPatient} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: 'var(--muted)', lineHeight: 1, padding: '2px 6px' }}>✕</button>
            </div>
            <div className="pr-modal-body">
              <div className="form-grid">
                <div className="form-group"><label>Patient Name *</label><input type="text" value={editPatientForm.name} maxLength={50} onChange={e => setEditPatientForm({ ...editPatientForm, name: sanitizeName(e.target.value).slice(0, 50) })} placeholder="e.g. Buddy" /></div>
                <div className="form-group"><label>Species *</label><CustomSelect value={editPatientForm.species} onChange={val => setEditPatientForm({ ...editPatientForm, species: val })} placeholder="Select" options={["Dog", "Cat"]} /></div>
                <div className="form-group">
                  <label>Breed</label>
                  <CustomSelect
                    value={editPatientForm.breed}
                    onChange={val => setEditPatientForm({ ...editPatientForm, breed: val })}
                    placeholder={editPatientForm.species ? "Select breed" : "Select species first"}
                    options={editPatientForm.species === "Cat" ? CAT_BREEDS : editPatientForm.species === "Dog" ? DOG_BREEDS : []}
                  />
                </div>
                <div className="form-group"><label>Sex / Gender</label><CustomSelect value={editPatientForm.gender} onChange={val => setEditPatientForm({ ...editPatientForm, gender: val })} placeholder="Select" options={["Male", "Female", "Unknown"]} /></div>
                <div className="form-group"><label>Age <span style={{ fontWeight: 400 }}>(optional)</span></label><CustomSelect
                  value={editPatientForm.age}
                  onChange={val => setEditPatientForm({ ...editPatientForm, age: val })}
                  placeholder="Select age"
                  options={AGE_OPTIONS}
                /></div>
                <div className="form-group"><label>Temp (°C) <span style={{ fontWeight: 400 }}>(optional)</span></label><input type="text" inputMode="decimal" value={editPatientForm.temp} onChange={e => setEditPatientForm({ ...editPatientForm, temp: e.target.value.replace(/[^0-9.]/g, '') })} placeholder="e.g. 38.5" /></div>
                <div className="form-group"><label>Heart Rate (bpm) <span style={{ fontWeight: 400 }}>(optional)</span></label><input type="text" inputMode="numeric" value={editPatientForm.heart_rate} onChange={e => setEditPatientForm({ ...editPatientForm, heart_rate: e.target.value.replace(/\D/g, '') })} placeholder="e.g. 120" /></div>
                <div className="form-group"><label>Weight (kg) <span style={{ fontWeight: 400 }}>(optional)</span></label><input type="text" inputMode="decimal" value={editPatientForm.weight} onChange={e => setEditPatientForm({ ...editPatientForm, weight: e.target.value.replace(/[^0-9.]/g, '') })} placeholder="e.g. 8.2" /></div>
                ...
                <div className="form-group">
                  ...
                  <input type="text" value={editPatientForm.contact} onChange={e => setEditPatientForm({ ...editPatientForm, contact: sanitizeContact(e.target.value) })} inputMode="numeric" maxLength={11} placeholder="e.g. 09170000000" />
                  {editPatientForm.contact && editPatientForm.contact.length !== 11 && (
                    <p style={{ fontSize: 11, color: "#dc2626", margin: "4px 0 0" }}>Must be 11 digits.</p>
                  )}
                </div>
                ...
                <div className="form-full" style={{ borderTop: '1px solid var(--border)', margin: '4px 0' }} />
                <div className="form-group"><label>Status</label><CustomSelect value={editPatientForm.status} onChange={val => setEditPatientForm({ ...editPatientForm, status: val })} placeholder="Select" options={["Outpatient", "Admitted"]} /></div>
                <div className="form-group"><label>Health</label><CustomSelect value={editPatientForm.health} onChange={val => setEditPatientForm({ ...editPatientForm, health: val })} placeholder="Select" options={["Good", "Fair", "Critical"]} /></div>
                <div className="form-group">
                  <label>Room</label>
            ...
            <div className="pr-modal-footer">
              <button className="btn btn-ghost pr-btn-auto" onClick={attemptCloseEditPatient}>Cancel</button>
              <button
                className="btn btn-primary pr-btn-auto"
                style={{
                  display: 'inline-flex', alignItems: 'center', gap: 6,
                  opacity: (!isEditPatientFormValid() || editPatientSaving) ? 0.5 : 1,
                  cursor: (!isEditPatientFormValid() || editPatientSaving) ? 'not-allowed' : 'pointer',
                }}
