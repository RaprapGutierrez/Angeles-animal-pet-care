...
// ─── Main Component ───────────────────────────────────────────────────────────
const PointOfSale = () => {
  const { user, isAdmin, isManager, isSuperAdmin, loading: userLoading } = useCurrentUser();
  const canVoid = isAdmin || isManager || isSuperAdmin;
  const { applyFilter } = useBranchFilter();
  const logActivityRef = React.useRef(null);
  useEffect(() => {
    if (typeof window !== 'undefined') {
      import('../../js/Utils/logActivity').then(m => { logActivityRef.current = m.logActivity; });
    }
  }, []);
  ...
  const fetchClients = useCallback(async () => {
    let query = supabase.from("profiles").select("id, first_name, last_name, email, phone, role")
      .eq("status", "Active").ilike("role", "customer").order("first_name");
  ...
  const processPayment = async () => {
    ...
    if (clientType === 'registered' && !selectedClient) { showToast("Please select a client", "error"); return; }
    if (clientType === 'walkin' && !walkinName.trim()) { showToast("Please enter the walk-in client name", "error"); return; }
    ...
    if (!data?.length) { showToast("Transaction failed — check RLS policies", "error"); return; }
    ...
    logActivity(user, 'Completed sale', `Sale total: ₱${total.toFixed(2)} · Client: ${clientName}`);
  ...
  const voidTransaction = (txId) => {
    if (!canVoid) { showToast("You don't have permission to void transactions", "error"); return; }
  ...
  const confirmVoidTransaction = async () => {
    ...
    const { error } = await supabase.from("transactions").update({ status: "Voided", voided_at: new Date().toISOString(), voided_by: user?.id || null }).eq("id", txId);
    if (error) { showToast("Error voiding transaction: " + error.message, "error"); return; }
    ...
    <Layout>
      ...
      <div style={S.page}>
        ...
        <div className="topbar pos-topbar" style={{ position: "fixed", top: 68, left: "var(--current-sidebar-w, 62px)", right: 0, zIndex: 40, background: "var(--card)" }}>
          ...
          <button onClick={() => { setShowTx(true); fetchTransactions(); }} className="tx-history-btn" style={{ ...S.btn, display: "flex", alignItems: "center", gap: 6, background: "var(--royal)", color: "#fff", border: "none", borderRadius: 8, padding: "9px 16px", fontSize: 13, fontWeight: 700, fontFamily: "inherit", cursor: "pointer" }}>
        ...
        <div style={{ ...S.cont, display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 390px", gap: 20, minHeight: "calc(100vh - 160px)" }}>
          ...
          <div>
            ...
            {loading ? (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(150px,1fr))", gap: 12 }}>
                {filtered.map((p, idx) => (
                  <div key={p.id} className="product-card fade-in" onClick={() => addToCart(p)}
                    style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 10, padding: 14, cursor: "pointer", textAlign: "center", animationDelay: `${idx * 0.07}s` }}>
          ...
          {/* ══ Cart Panel ══ */}
          <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", display: "flex", flexDirection: "column", overflow: "visible", boxShadow: "var(--shadow)", height: "fit-content", position: isMobile ? "static" : "sticky", top: 140 }}>
            ...
            <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--border)", position: "relative" }} ref={clientRef}>
              ...
              {clientType === "registered" && (
                selectedClient ? (
                  ...
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "9px 12px", border: `1.5px solid ${showDropdown ? "var(--royal)" : "var(--border)"}`, borderRadius: 8, boxSizing: "border-box", background: "var(--card)", cursor: "text", transition: "border-color 0.15s" }}
                    ...
                    {showDropdown && (
                      <div style={{ position: "absolute", top: "100%", left: 18, right: 18, background: "var(--card)", border: "1.5px solid var(--border)", borderRadius: 10, boxShadow: "0 8px 24px rgba(0,0,0,0.12)", zIndex: 9999, maxHeight: 240, overflowY: "auto", marginTop: 4 }}>
                  ...
                  <div>
                   ...
                   <div>
                    ...
                    <input
                      ...
                      inputMode="numeric"
                      maxLength={11}
                      onChange={e => setWalkinContact(e.target.value.replace(/\D/g, "").slice(0, 11))}
                      style={{ width: "100%", padding: "9px 12px", border: "1.5px solid var(--border)", borderRadius: 8, fontSize: 13, fontFamily: "inherit", outline: "none", boxSizing: "border-box" }}
                    />
                    ...
                    <div style={{ display: "flex", alignItems: "center", background: "var(--bg)", border: "1.5px solid var(--border)", borderRadius: 8, overflow: "hidden" }}>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setCart((prev) => prev.map((i) => i.id === item.id ? { ...i, qty: val === "" ? "" : Math.max(1, parseInt(val, 10) || 1) } : i));
                        }}
                        onBlur={(e) => {
                        ...
                        style={{ width: 40, height: 28, border: "none", borderLeft: "1.5px solid var(--border)", borderRight: "1.5px solid var(--border)", background: "transparent", fontSize: 13, fontWeight: 700, textAlign: "center", fontFamily: "inherit", outline: "none", color: "var(--text)", MozAppearance: "textfield" }}
                      />
                      <button className="qty-btn" onClick={() => updateQty(item.id, 1)} style={{ width: 28, height: 28, border: "none", background: "transparent", cursor: "pointer", fontSize: 15, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
                    </div>
                  </div>
            ...
            <div style={{ padding: "16px 18px", borderTop: "1px solid var(--border)" }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                {["Cash", "Card", "GCash"].map((m) => (
                  <button key={m} onClick={() => setPayMethod(m)} className="pay-method"
                    style={{ flex: 1, padding: "9px 0", border: `2px solid ${payMethod === m ? "var(--royal)" : "var(--border)"}`, borderRadius: 8, background: payMethod === m ? "var(--royal)" : "var(--card)", color: payMethod === m ? "#fff" : "var(--muted)", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", transition: "all 0.18s", width: "auto", letterSpacing: "0.3px" }}>
                    {m}
                  </button>
                ))}
              </div>
              ...
              <div style={{ marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                  ...
                  {amountGiven !== "" && (
                    <span style={{ fontSize: 11, fontWeight: 700, color: Number(amountGiven) < total ? "#dc2626" : "#16a34a" }}>
                      {Number(amountGiven) < total ? "Insufficient" : "Sufficient"}
                    </span>
                  )}
                </div>
                <div style={{ position: "relative" }}>
                  ...
                  <input
                    ...
                    style={{ width: "100%", padding: "10px 12px 10px 26px", border: `1.5px solid ${amountGiven !== "" && Number(amountGiven) < total ? "#fca5a5" : "var(--border)"}`, borderRadius: 8, fontSize: 14, fontWeight: 700, fontFamily: "inherit", outline: "none", boxSizing: "border-box", color: "var(--text)" }}
                  />
                </div>
                {amountGiven !== "" && Number(amountGiven) >= total && total > 0 && (
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8, padding: "8px 12px", background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 8 }}>
                    <span style={{ fontSize: 12, color: "#166534", fontWeight: 600 }}>Change</span>
                    <AnimatedPrice value={Number(amountGiven) - total} style={{ fontSize: 15, fontWeight: 800, color: "#16a34a" }} />
                  </div>
                )}
              </div>

              {(() => {
                const contactValid = !walkinContact || walkinContact.length === 11;
                const clientReady = clientType === "registered" ? !!selectedClient : (!!walkinName.trim() && contactValid);
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 99999, padding: 16 }}>
          <div style={{ background: 'var(--card)', borderRadius: 14, width: '100%', maxWidth: 380, boxShadow: '0 20px 60px rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '18px 22px 14px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: 'var(--text)' }}>Void Transaction?</h3>
            </div>
            <div style={{ padding: '16px 22px' }}>
              <p style={{ margin: 0, fontSize: 13, color: 'var(--muted)', lineHeight: 1.6 }}>
                This transaction will be marked as voided but kept on record. This action can be reviewed later in transaction history.
              </p>
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, padding: '12px 22px', borderTop: '1px solid var(--border)', background: 'var(--bg)' }}>
              <button className="btn btn-ghost" style={{ width: 'auto' }} onClick={() => setConfirmVoid({ show: false, txId: null })}>Cancel</button>
              <button className="btn" style={{ width: 'auto', background: '#dc2626', color: '#fff', border: 'none' }} onClick={confirmVoidTransaction}>Void Transaction</button>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="modal-body">
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead>
                    <tr>
                      {[...["Client", "Email", "Items", "Subtotal", "Discount", "Total", "Payment", "Date", "Status"], ...(canVoid ? ["Actions"] : [])].map((h) => (
                  ...
                  <tbody>
                    {transactions.length === 0 ? (
                      <tr><td colSpan={canVoid ? 10 : 9} style={{ textAlign: "center", padding: 40, color: "var(--muted)" }}>No transactions yet</td></tr>
                    ) : transactions.map((t) => {
                      const isVoided = t.status === "Voided";
                      return (
                      <tr key={t.id} style={{ borderBottom: "1px solid var(--border)", opacity: isVoided ? 0.55 : 1, background: isVoided ? "#fef2f2" : "transparent" }}>
                        <td style={{ padding: "10px 12px", fontWeight: 600, textDecoration: isVoided ? "line-through" : "none" }}>{t.client}</td>
                        ...
                        <td style={{ padding: "10px 12px" }}>
                          <span style={{ fontSize: 10, fontWeight: 700, borderRadius: 99, padding: "3px 9px", background: isVoided ? "#fee2e2" : "#dcfce7", color: isVoided ? "#991b1b" : "#166534" }}>
                          ...
                          <td style={{ padding: "10px 12px" }}>
                            {!isVoided && (
                              <button
                                ...
                                style={{ background: "#fef2f2", border: "1.5px solid #fca5a5", color: "#dc2626", borderRadius: 8, padding: "5px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", display: "inline-flex", alignItems: "center", gap: 4 }}
                              >
