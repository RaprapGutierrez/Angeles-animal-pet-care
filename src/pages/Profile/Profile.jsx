...
// ── Inline SVG Icons ─────────────────────────────────────────
const ICONS = {
  branch: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" /><polyline points="9 22 9 12 15 12 15 22" /></svg>,
  ...
  phone: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.38 2 2 0 0 1 3.6 1.17h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.76a16 16 0 0 0 5.33 5.33l.96-.96a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z" /></svg>,
  address: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z" /><circle cx="12" cy="10" r="3" /></svg>,
  ...
  customer: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>,
...
const Img = ({ icon, size = 16, style, color }) => (
  <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: color || "currentColor", width: size, height: size, ...style }}>
    {icon ? React.cloneElement(icon, { width: size, height: size }) : null}
  </span>
);
...
const InfoField = ({ label, value, icon }) => (
  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      {icon && (
        <span style={{ width: 24, height: 24, display: "inline-flex", alignItems: "center", justifyContent: "center", background: "var(--card, #fff)", borderRadius: 6, flexShrink: 0, border: `1px solid ${T.border}`, color: T.muted }}>
...
const Card = ({ title, icon, children, style }) => (
  <div style={{ background: "var(--card, #fff)", border: `1px solid ${T.border}`, borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,.05)", ...style }}>
    {title && (
      <div style={{ padding: "16px 22px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", gap: 9 }}>
        {icon && (
          <span style={{ width: 26, height: 26, display: "inline-flex", alignItems: "center", justifyContent: "center", background: "var(--card, #fff)", border: `1px solid ${T.border}`, borderRadius: 7, flexShrink: 0, color: T.muted }}>
            {React.cloneElement(icon, { width: 15, height: 15 })}
          </span>
        )}
...
const Modal = ({ modal, onClose }) => {
  ...
  return (
    ...
    >
      <div style={{ background: "var(--card, #fff)", borderRadius: 16, padding: "32px 30px 28px", width: "100%", maxWidth: 420, boxShadow: "0 24px 64px rgba(0,0,0,.2)", position: "relative" }}>
        ...
        <div style={{ width: 56, height: 56, borderRadius: "50%", background: v.bg, border: `1.5px solid ${v.border}`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 18, color: v.btnBg }}>
          {React.cloneElement(modal.type === "error" || modal.type === "confirm" ? ICONS.address : modal.type === "success" ? ICONS.calendar : ICONS.email, { width: 24, height: 24 })}
        </div>
...
const ImageCropModal = ({ imageSrc, onCancel, onSave }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 10000, background: "rgba(15,23,42,0.75)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "var(--card, #fff)", borderRadius: 20, padding: "32px 28px 28px", width: "100%", maxWidth: 380, boxShadow: "0 32px 80px rgba(0,0,0,.3)", position: "relative" }}>
...
const Profile = () => {
  ...
  const setPhoneDigits = (v) => setEditForm((f) => ({ ...f, phone: v.replace(/\D/g, "").slice(0, 11) }));
  ...
  useEffect(() => {
    // Wait until useCurrentUser has resolved
    if (userLoading) return;

    // If still no user after loading is done, bail out
    if (!authUser || !userId) {
      setNoUser(true);
      setLoading(false);
      setApptLoading(false);
  ...
  }, [userId, userLoading]); // ← removed fetchProfile/fetchAppointments to prevent re-trigger loop
  ...
  const saveProfile = async () => {
    ...
    if (editForm.phone && editForm.phone.length !== 11) {
      showModal("error", "Invalid Phone Number", "Contact number must be exactly 11 digits.");
      return;
    }
    setSaving(true);
  ...
  if ((userLoading && !authUser) || loading) {
    return (
      <Layout>
  ...
  if (noUser || !profile) {
    return (
      <Layout>
        <div style={{ padding: 40, textAlign: "center", color: T.muted, fontSize: 14 }}>
          <span style={{ display: "block", marginBottom: 12, opacity: 0.4, color: T.muted }}>{React.cloneElement(ICONS.customer, { width: 40, height: 40 })}</span>
  ...
  const branchLabel = authUser?.branchId
    ? (profile.branch_name || BRANCH_NAMES[authUser.branchId] || `Branch ${authUser.branchId}`)
    : null;
  ...
  return (
    <Layout>
      ...
      <div className="profile-page" style={{ padding: "24px 28px", paddingTop: "calc(64px + 24px)", maxWidth: 960, margin: "0 auto" }}>
        ...
        {/* ── Hero Banner ── */}
        <div className="profile-hero-banner" style={{ background: "linear-gradient(135deg, #1e293b 0%, #0f172a 100%)", borderRadius: 18, padding: "32px 32px 0", marginBottom: 24, position: "relative", overflow: "hidden" }}>
          ...
          <div className="profile-hero-row" style={{ position: "relative" }}>
            <div className="avatar-wrapper" style={{ paddingBottom: 28, paddingRight: 0, position: "relative" }}>
              <Avatar
            ...
            <div style={{ paddingBottom: 30, flex: "0 1 auto", minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6, flexWrap: "wrap", justifyContent: "center" }}>
                ...
                {branchLabel && (
                  <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 12px", borderRadius: 20, background: "rgba(255,255,255,0.15)", color: "#e0e7ff", display: "flex", alignItems: "center", gap: 5 }}>
                    {React.cloneElement(ICONS.branch, { width: 12, height: 12, stroke: "#e0e7ff" })}
                    {branchLabel}
                  </span>
                )}
              </div>
              <div style={{ fontSize: 13, color: "rgba(255,255,255,0.55)", display: "flex", alignItems: "center", gap: 6 }}>
                {React.cloneElement(ICONS.email, { width: 12, height: 12, stroke: "rgba(255,255,255,0.55)" })}
                {profile.email}
              </div>
              <div
                onClick={() => !uploadingPhoto && fileInputRef.current?.click()}
            ...
            <div style={{ paddingBottom: 28 }}>
              {!editing ? (
                <button onClick={() => setEditing(true)} style={{ padding: "9px 20px", background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 10, color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", gap: 7 }}>
                  {React.cloneElement(ICONS.edit, { width: 13, height: 13, stroke: "#fff" })}
                  Edit Profile
                </button>
              ) : (
          ...
          <div className="profile-tabs-row profile-hero-stats" style={{ borderTop: "1px solid rgba(255,255,255,0.08)", margin: "0 -32px", padding: "14px 32px" }}>
            {[
        ...
        <div className="profile-grid-2col" style={{ marginBottom: 20 }}>
          ...
          <Card title="Personal Information" icon={ICONS.customer}>
            {!editing ? (
              <div className="profile-info-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
                ...
                <InfoField label="Email" value={profile.email} icon={ICONS.email} />
                <InfoField label="Phone" value={profile.phone} icon={ICONS.phone} />
                <InfoField label="Role" value={roleNormalized} />
                <InfoField label="Status" value={profile.status || "Active"} />
              </div>
            ) : (
              <div className="profile-info-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                {[
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  <label style={{ fontSize: 11, fontWeight: 700, color: T.muted, textTransform: "uppercase", letterSpacing: "0.5px" }}>Phone</label>
                  <input
                    className="edit-inp"
                    type="text"
                    inputMode="numeric"
                  ...
                  {editForm.phone && editForm.phone.length !== 11 && (
                    <p style={{ fontSize: 11, color: T.danger, margin: "2px 0 0" }}>Contact number must be 11 digits.</p>
                  )}
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
          ...
          {/* Account Details */}
          <Card title="Account Details" icon={ICONS.branch}>
            {!editing ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                <InfoField label="User ID" value={profile.id} />
                <InfoField label="Member Since" value={profile.created_at ? fmtDate(profile.created_at) : "—"} icon={ICONS.calendar} />
                {profile.updated_at && <InfoField label="Last Updated" value={fmtDateTime(profile.updated_at)} icon={ICONS.calendar} />}
                <InfoField label="Address" value={profile.address} icon={ICONS.address} />
                <InfoField label="Branch" value={branchLabel || "—"} icon={ICONS.branch} />
              </div>
            ) : (
        ...
        {/* My Schedule */}
        <Card title="My Schedule" icon={ICONS.calendar}>
          <div style={{ display: "flex", gap: 0, marginBottom: 20, background: T.bg, borderRadius: 10, padding: 4, width: "fit-content" }}>
            {[
              ...
              <button key={t.key} onClick={() => setTab(t.key)} style={{ padding: "8px 20px", borderRadius: 8, border: "none", cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 700, transition: "all .15s", background: tab === t.key ? "var(--card, #fff)" : "transparent", color: tab === t.key ? T.royal : T.muted, boxShadow: tab === t.key ? "0 1px 4px rgba(0,0,0,.08)" : "none" }}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          {apptLoading ? (
            ...
            <div style={{ padding: 40, textAlign: "center" }}>
              <span style={{ display: "block", marginBottom: 10, opacity: 0.3, color: T.muted }}>{React.cloneElement(ICONS.calendar, { width: 36, height: 36 })}</span>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                ...
                <tbody>
                  {displayList.map((a, i) => {
                    ...
                    return (
                      <tr key={a.id} className="appt-row" style={{ background: isToday ? "#eff6ff" : i % 2 === 0 ? "var(--card, #fff)" : T.bg }}>
                        <td style={{ padding: "12px 14px", borderBottom: `1px solid ${T.border}`, verticalAlign: "middle" }}>
                          ...
                          {isToday && <div style={{ fontSize: 10, fontWeight: 700, color: T.royal, marginTop: 2, display: "flex", alignItems: "center", gap: 4 }}>{React.cloneElement(ICONS.calendar, { width: 10, height: 10, stroke: T.royal })}TODAY</div>}
                        </td>
