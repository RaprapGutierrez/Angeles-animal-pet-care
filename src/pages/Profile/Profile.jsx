...
const ImageCropModal = ({ imageSrc, onCancel, onSave }) => {
  ...
  const onTouchStart = (e) => {
    const t = e.touches[0];
    setDragging(true);
    dragStart.current = { x: t.clientX - offsetX, y: t.clientY - offsetY };
  };
  const onTouchMove = (e) => {
    if (!dragging) return;
    const t = e.touches[0];
    setOffsetX(t.clientX - dragStart.current.x);
    setOffsetY(t.clientY - dragStart.current.y);
  };
      ...
      >
        ...
        >
          <canvas
            ...
            style={{
              ...
              boxShadow:
                "0 0 0 4px rgba(37,99,235,0.15), 0 8px 24px rgba(0,0,0,.15)",
              touchAction: "none",
              userSelect: "none",
            }}
            ...
            onWheel={(e) => {
              e.preventDefault();
              setScale((s) => Math.min(4, Math.max(0.3, s - e.deltaY * 0.002)));
            }}
          />
        </div>
        <div style={{ marginBottom: 24 }}>
          ...
          >
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: T.royal }}>
          ...
          <input
            ...
            style={{ width: "100%", accentColor: T.royal }}
          />
        </div>
...
const Profile = () => {
  ...
  const showModal = useCallback(
    (type, title, message, opts = {}) =>
      setModal({ type, title, message, ...opts }),
    [],
  );
  const closeModal = useCallback(() => setModal(null), []);

  const setPhoneDigits = (v) =>
    setEditForm((f) => ({ ...f, phone: v.replace(/\D/g, "").slice(0, 11) }));
  ...
  const fetchProfile = useCallback(async () => {
    ...
    if (dbRow) {
      ...
      if (needsSync) {
        await supabase
          .from("profiles")
          .update({ first_name: authFirstName, last_name: authLastName })
          .eq("id", userId);
      }
      const merged = {
        ...dbRow,
        first_name: authFirstName || dbRow.first_name,
        last_name: authLastName || dbRow.last_name,
      };
      setProfile(merged);
      setEditForm({
  ...
  const fetchAppointments = useCallback(async () => {
    ...
    try {
      ...
      const liveMeta = liveUser?.user_metadata || {};
      const name = [
        liveMeta.first_name || authUser?.firstName,
        liveMeta.last_name || authUser?.lastName,
      ]
        .filter(Boolean)
        .join(" ");
  ...
  useEffect(() => {
    ...
    const apptChannel = supabase
      .channel("profile-appts-realtime")
      .on(
        "postgres_changes",
  ...
  const handleCropSave = async (blob) => {
    ...
    try {
      ...
      await supabase
        .from("profiles")
        .upsert(withBranchId(authUser, { id: userId, avatar_url: publicUrl }), {
          onConflict: "id",
        });
  ...
  const saveProfile = async () => {
    ...
    if (authError) {
      ...
      showModal(
        "error",
        "Save Failed",
        authError.message || "Could not update profile.",
      );
      return;
    }
    ...
    if (dbError) {
      showModal(
        "error",
        "Save Failed",
        dbError.message || "Could not save profile.",
      );
      return;
    }
    ...
    return (
      <Layout>
        ...
        >
          ...
          >
            {React.cloneElement(ICONS.customer, { width: 40, height: 40 })}
          </span>
  ...
  const roleStyle = ROLE_BADGE_STYLE[roleNormalized] || {
    bg: "#f3f4f6",
    color: "#4b5563",
  };
  ...
  const branchLabel = authUser?.branchId
    ? profile.branch_name ||
      BRANCH_NAMES[authUser.branchId] ||
        ...
        >
          ...
          <div className="profile-hero-row" style={{ position: "relative" }}>
            ...
            <div style={{ paddingBottom: 30, flex: "0 1 auto", minWidth: 0 }}>
              ...
              >
                ...
                <span
                  style={{
                    ...
                    borderRadius: 20,
                    background:
                      profile.status === "Active" ? "#dcfce7" : "#fef2f2",
                    color: profile.status === "Active" ? "#15803d" : "#dc2626",
                  }}
                >
                ...
                {branchLabel && (
                  ...
                  >
                    {React.cloneElement(ICONS.branch, {
                      width: 12,
                      height: 12,
                      stroke: "#e0e7ff",
                    })}
              ...
              >
                {React.cloneElement(ICONS.email, {
                  width: 12,
                  height: 12,
          ...
          <div
            className="profile-tabs-row profile-hero-stats"
            style={{
                ...
                {[
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditForm((v) => ({ ...v, [f.key]: e.target.value }))
                      }
                    />
                  </div>
                ))}
              ...
              >
                ...
                >
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setEditForm((v) => ({ ...v, address: e.target.value }))
                    }
        ...
        <Card title="My Schedule" icon={ICONS.calendar}>
          ...
          {apptLoading ? (
            ...
            <div style={{ padding: 40, textAlign: "center" }}>
              ...
              >
                {React.cloneElement(ICONS.calendar, { width: 36, height: 36 })}
              </span>
                      ...
                      >
                        ...
                        >
                          ...
                          {isToday && (
                            ...
                            >
                              {React.cloneElement(ICONS.calendar, {
                                width: 10,
                                height: 10,
                                stroke: T.royal,
                              })}
