...
const CustomerMessages = () => {
  ...
  useEffect(() => {
    ...
    fetchConversations();
    const ch = supabase.channel('customer-convo-ids-' + myId)
      .on('postgres_changes',
  ...
  const startConversation = (staffMember) => {
    ...
    setLastMessageTimes((prev) => ({ ...prev, [staffMember.id]: new Date().toISOString() }));
    setSelected(staffMember);    setShowNewConvo(false);
    setNewConvoSearch('');
    setMobileView('chat');
  ...
  const sendMessage = async () => {
   ...
   // ── PATCH: insert now includes branch_id ─────────────────────────────
    ...
    if (error) {
      ...
      setMessages((prev) => prev.filter((m) => m.id !== optimistic.id));
      setLastMessageTimes((prev) => ({ ...prev, [selected.id]: new Date().toISOString() }));
      fetchMessages(selected.id);
    }
  };
    ...
    return (
      <Layout>
        <div style={{
          ...
          height: 'calc(100vh - 68px)', marginTop: -24, marginLeft: -24, marginRight: -24, marginBottom: -24,
        }}>
  ...
  const conversationStaff = staff
    ...
    .sort((a, b) => new Date(lastMessageTimes[b.id] || 0) - new Date(lastMessageTimes[a.id] || 0));
  const filteredStaff = conversationStaff.filter((d) =>
    !search || (d.full_name || d.email || '').toLowerCase().includes(search.toLowerCase())
  );
  const availableForNewConvo = staff.filter((d) => !conversationIds.has(d.id) &&
    (!newConvoSearch || (d.full_name || d.email || '').toLowerCase().includes(newConvoSearch.toLowerCase())));
  ...
  return (
    <Layout>
      ...
      <div className="msg-page" style={{ position: 'fixed', top: 68, left: 0, right: 0, bottom: 0, display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '300px 1fr', background: '#f4f6fb', overflow: 'hidden' }}>

        {/* ══ LEFT SIDEBAR ══ */}
        <div style={{ display: isMobile && mobileView === 'chat' ? 'none' : 'flex', flexDirection: 'column', background: '#fff', borderRight: '1.5px solid #eef0f6', boxShadow: '3px 0 12px rgba(0,0,0,0.04)', minHeight: 0, overflow: 'hidden' }}>
          <div className="sidebar-header" style={{ padding: '18px 18px 12px', borderBottom: '1px solid #f0f2f8' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
          ...
          <div style={{ padding: '10px 18px 6px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 5, background: '#eff0fe', border: '1px solid #e0e2fb', color: '#6366f1', borderRadius: 20, padding: '4px 10px', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
            >
            ...
            {filteredStaff.length === 0 ? (
              <div style={{ padding: 28, textAlign: 'center' }}>
                <div style={{ marginBottom: 10, display: 'flex', justifyContent: 'center' }}>
                  <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="1.5" strokeLinecap="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /></svg>
                </div>
                <div style={{ fontSize: 13, color: '#9ca3af', marginBottom: conversationStaff.length === 0 ? 12 : 0 }}>
                  {search ? 'No matches found.' : conversationStaff.length === 0 ? "You haven't messaged anyone yet." : 'No staff available'}
                </div>
                {conversationStaff.length === 0 && !search && (
                  <button
                    onClick={() => setShowNewConvo(true)}
                    style={{ background: '#6366f1', color: '#fff', border: 'none', borderRadius: 10, padding: '8px 16px', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
                  >
              ...
              return (
                <div key={c.id} className={`client-row fade-in ${isActive ? 'active' : ''}`}
                  onClick={() => { setSelected(c); setMobileView('chat'); setTimeout(() => inputRef.current?.focus(), 100); }}
                  style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 18px', borderLeft: `3px solid ${isActive ? '#6366f1' : 'transparent'}`, background: isActive ? '#eff0fe' : 'transparent', animationDelay: `${i * 0.05}s` }}
                >
        ...
        <div style={{ display: isMobile && mobileView === 'list' ? 'none' : 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
          {!selected ? (
            ...
            <>
              <div className="chat-header-row" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', background: '#fff', borderBottom: '1.5px solid #eef0f6', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', flexShrink: 0 }}>
                {isMobile && (
                ...
                {messages.length === 0 && !deleting && (
                  <div style={{ margin: 'auto', textAlign: 'center' }}>
                    <div style={{ marginBottom: 8, display: 'flex', justifyContent: 'center' }}>
                      <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 11V6a2 2 0 0 0-4 0v5" /><path d="M14 10V4a2 2 0 0 0-4 0v6" /><path d="M10 10.5V6a2 2 0 0 0-4 0v8" /><path d="M6 14a4 4 0 0 0 4 4h4a4 4 0 0 0 4-4v-2.5" /></svg>
                    </div>
              ...
              <div style={{ background: '#fff', borderTop: '1.5px solid #eef0f6', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: isMobile ? '10px 12px 12px' : '10px 18px 14px' }}>
                  <input
                    ref={inputRef}
                    className="msg-input"
                    value={newMsg}
      ...
      {showNewConvo && (
        <div className="new-convo-overlay" style={{ position: 'fixed', inset: 0, zIndex: 99998, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 18, width: '100%', maxWidth: 420, maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', overflow: 'hidden' }}>
            <div style={{ padding: '18px 20px 14px', borderBottom: '1px solid #f0f2f8', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: '#111827' }}>New Conversation</h3>
              <button onClick={() => { setShowNewConvo(false); setNewConvoSearch(''); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', display: 'flex' }}>
            ...
            <div style={{ padding: '14px 20px 10px' }}>
              <div style={{ position: 'relative' }}>
                <svg style={{ position: 'absolute', left: 11, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }} width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2.5"><circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" /></svg>
                <input value={newConvoSearch} onChange={(e) => setNewConvoSearch(e.target.value)} placeholder="Search staff to message…"
                  style={{ width: '100%', padding: '9px 12px 9px 32px', borderRadius: 10, border: '1.5px solid #eef0f6', background: '#f8f9fc', fontSize: 13, color: '#111827', outline: 'none', fontFamily: 'inherit' }} />
              </div>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '4px 8px 12px' }}>
              {availableForNewConvo.length === 0 ? (
                <div style={{ padding: '28px 16px', textAlign: 'center', fontSize: 13, color: '#9ca3af' }}>
                  {newConvoSearch ? 'No matches found.' : 'You already have conversations with all available staff.'}
                </div>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 13.5, color: '#111827', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.full_name || c.email || 'Unknown'}</div>
                    <div style={{ fontSize: 11.5, color: '#9ca3af', marginTop: 1 }}>{c.role || ''}</div>
                  </div>
                </div>
              ))}
            </div>
