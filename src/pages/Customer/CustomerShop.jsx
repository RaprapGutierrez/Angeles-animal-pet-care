...
const CustomerShop = () => {
  ...
  const getProductDescription = (p) => {
    ...
    const descByName = {
      "amoxicillin 250mg": "A broad-spectrum antibiotic used to treat bacterial infections in dogs and cats, including skin, respiratory, and urinary tract infections.",
      "betadine solution": "An antiseptic solution used to clean and disinfect wounds, cuts, and skin infections before dressing.",
      "flea treatment": "A topical or oral treatment that kills fleas and prevents re-infestation, keeping your pet's coat and skin healthy.",
      "iv fluid (nacl 500ml)": "Sterile saline solution used for rehydration and fluid therapy during treatment, surgery, or recovery.",
      "rabies vaccine": "A core vaccine that protects pets against the rabies virus, required for licensing and boarding in most areas.",
      "distemper vaccine": "A core vaccine that protects dogs against canine distemper, a serious and often fatal viral disease.",
      "surgical gloves": "Disposable, sterile gloves used by veterinary staff during examinations, treatments, and surgical procedures.",
      "syringe 5ml": "A sterile, single-use syringe used for administering injections, vaccines, or oral medications accurately.",
      "dog canned food": "A nutritionally balanced wet food formulated to support your dog's daily dietary needs.",
    };
    if (descByName[name]) return descByName[name];

    const descByCategory = {
      Medicine: "A veterinary medicine used to treat or manage your pet's health condition as prescribed by our vets.",
      Vaccine: "A preventive vaccine that helps protect your pet from common and serious diseases.",
      Supplies: "A clinic supply item used during examinations, treatments, or procedures to keep care safe and sanitary.",
      Food: "A pet food product formulated to support your pet's nutrition and overall health.",
      Equipment: "Veterinary equipment used by our staff to support diagnosis, treatment, or pet care procedures.",
      Other: "A product available at our clinic to support your pet's health and care.",
    };
    return descByCategory[p.category] || "No description available for this product.";
  };
  ...
  const S = {
    ...
    topbar: {
      ...
      zIndex: 40, boxSizing: "border-box", gap: 8, flexWrap: "wrap",
    },
    cont: { padding: "12px 12px", paddingTop: 122, width: "100%", boxSizing: "border-box" },
    btn: { width: "auto" },
  ...
  return (
    <Layout isCustomer>
      <div style={S.page}>
        ...
        <div style={S.topbar} className="branches-topbar">
          <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0, flex: 1 }}>
            <img src="/icon/shopping_cart.png" alt="Shop" width={22} height={22} style={{ flexShrink: 0 }} />
            <div style={{ minWidth: 0 }}>
              <h1 style={{ fontSize: 15, fontWeight: 800, color: "var(--text)", margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Pet Shop</h1>
              <p style={{ fontSize: 11, color: "var(--muted)", margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Browse and order pet products</p>
            </div>
          </div>
        ...
        <div style={S.cont}>
          ...
          {showSuccess && (
            <div style={{ background: "#dcfce7", border: "1px solid #86efac", borderRadius: 10, padding: "14px 20px", marginBottom: 20, display: "flex", alignItems: "center", gap: 12, animation: "shopSlideIn 0.3s ease both" }}>
              <span style={{ color: "#16a34a" }}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ width: 22, height: 22 }}><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg></span>
              <div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
            {categories.map(c => (
              <button key={c} onClick={() => setCatFilter(c)}
                style={{ padding: "7px 16px", border: `1.5px solid ${catFilter === c ? "var(--royal)" : "var(--border)"}`, borderRadius: 99, background: catFilter === c ? "var(--royal)" : "var(--card)", color: catFilter === c ? "#fff" : "var(--muted)", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", width: "auto", transition: "all 0.15s" }}>
                {c}
              </button>
            ))}
          </div>
          ...
          {loading ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 16 }}>
              {filtered.map((p, i) => (
                <div key={p.id} className="shop-product-card fade-in"
                  onClick={() => setSelectedProduct(p)}
                  style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden", animationDelay: `${Math.min(i, 12) * 0.04}s`, cursor: "pointer" }}>
      ...
      {showReceipts && (
        <div style={S.overlay}>
          <div style={{ background: "var(--card)", borderRadius: 16, width: "100%", maxWidth: 620, maxHeight: "90vh", overflowY: "auto", display: "flex", flexDirection: "column", boxShadow: "0 20px 60px rgba(0,0,0,0.25)", animation: "shopPop 0.22s ease both" }}>
                ...
                <>
                  {receipts.length > 0 && (
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, padding: "16px 20px", borderBottom: "1px solid var(--border)" }}>
                      {[
                        { label: "Total Visits", value: receipts.length, icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ width: 18, height: 18 }}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>, bg: "#eff6ff", text: "#1e40af" },
                        { label: "Total Spent", value: `₱${totalSpent.toFixed(2)}`, icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ width: 18, height: 18 }}><rect x="1" y="4" width="22" height="16" rx="2" /><line x1="1" y1="10" x2="23" y2="10" /></svg>, bg: "#f0fdf4", text: "#166534" },
                        { label: "This Month", value: `${thisMonth} txn${thisMonth !== 1 ? "s" : ""}`, icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ width: 18, height: 18 }}><rect x="3" y="4" width="18" height="18" rx="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>, bg: "#faf5ff", text: "#6b21a8" },
                      ].map((c, i) => (
                        <div key={i} style={{ background: c.bg, borderRadius: 10, padding: "12px 14px" }}>
                          <div style={{ marginBottom: 4, color: c.text }}>{c.icon}</div>
                  ...
                  {receipts.length > 0 && (
                    <div style={{ padding: "12px 20px", borderBottom: "1px solid var(--border)" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--bg)", border: "1.5px solid var(--border)", borderRadius: 8, padding: "8px 12px" }}>
                        ...
                        {receiptSearch && (
                          <button onClick={() => setReceiptSearch("")} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--muted)", padding: 0, width: "auto", display: "flex", alignItems: "center" }}>
                  ...
                  {receipts.length === 0 ? (
                    <div style={{ padding: "48px 20px", textAlign: "center", color: "var(--muted)" }}>
                      <div style={{ marginBottom: 12, display: "flex", justifyContent: "center", color: "var(--muted)" }}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ width: 48, height: 48 }}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /></svg></div>
        ...
        Thank you!`;
                    return (
                      <div key={tx.id} style={{ borderBottom: "1px solid var(--border)" }}>
                        <div
                          ...
                          style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "13px 20px", cursor: "pointer", background: isOpen ? "var(--light-blue)" : "var(--card)" }}
                        >
                          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            <div style={{ width: 40, height: 40, borderRadius: 10, background: isOpen ? "var(--royal)" : "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0, transition: "all 0.15s" }}>
                              {isOpen
                                ? <svg viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ width: 18, height: 18 }}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>
                                : <svg viewBox="0 0 24 24" fill="none" stroke="#64748b" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ width: 18, height: 18 }}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /></svg>
                              }
                            </div>
                            <div>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-outline" style={{ ...S.btn, display: "flex", alignItems: "center", gap: 6 }}
                ...
                {receiptsLoading ? "Refreshing…" : <><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ width: 13, height: 13, marginRight: 5 }}><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8" /><path d="M3 3v5h5" /></svg>Refresh</>}
              </button>
            </div>
          </div>
        </div>
      ...
      {selectedProduct && (
        <div style={S.overlay} onClick={() => setSelectedProduct(null)}>
          <div
            ...
            style={{ background: "var(--card)", borderRadius: isMobile ? 12 : 16, width: "100%", maxWidth: 420, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.25)", animation: "shopPop 0.22s ease both" }}
          >
            <div style={{ height: isMobile ? 110 : 140, background: "var(--light-blue)", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
              {ICONS[selectedProduct.category] || <img src="/icon/inventory_2.png" alt={selectedProduct.category} width={isMobile ? 44 : 56} height={isMobile ? 44 : 56} />}
              <button onClick={() => setSelectedProduct(null)}
                style={{ position: "absolute", top: 12, right: 12, width: 30, height: 30, borderRadius: "50%", background: "rgba(255,255,255,0.85)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <svg viewBox="0 0 24 24" fill="none" stroke="#334155" strokeWidth="2.5" strokeLinecap="round" style={{ width: 14, height: 14 }}><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
              </button>
            </div>

            <div style={{ padding: isMobile ? 16 : 20 }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, marginBottom: 6, flexWrap: "wrap" }}>
                <h3 style={{ margin: 0, fontSize: isMobile ? 15 : 17, fontWeight: 800, color: "var(--text)" }}>{selectedProduct.name}</h3>
                <span style={{ fontSize: 11, color: selectedProduct.qty <= 5 ? '#dc2626' : '#16a34a', fontWeight: 700, background: selectedProduct.qty <= 5 ? '#fee2e2' : '#dcfce7', borderRadius: 20, padding: '3px 10px', flexShrink: 0 }}>
                  {selectedProduct.qty <= 5 ? 'Low Stock' : 'In Stock'}
                </span>
              </div>
              <p style={{ margin: "0 0 14px", fontSize: 12, color: "var(--muted)" }}>{selectedProduct.category}</p>

              <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 10, marginBottom: 16 }}>
                <div style={{ flex: 1, background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 10, padding: "10px 14px" }}>
                  <p style={{ margin: 0, fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 0.5 }}>Price</p>
                  <p style={{ margin: "3px 0 0", fontSize: 16, fontWeight: 800, color: "var(--royal)" }}>₱{Number(selectedProduct.price).toFixed(2)}</p>
                </div>
                <div style={{ flex: 1, background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 10, padding: "10px 14px" }}>
                  <p style={{ margin: 0, fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 0.5 }}>Stock</p>
                  <p style={{ margin: "3px 0 0", fontSize: 16, fontWeight: 800, color: selectedProduct.qty <= 5 ? "#dc2626" : "var(--text)" }}>
              ...
              <div>
                <p style={{ margin: "0 0 6px", fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 0.5 }}>Description</p>
            ...
            <div style={{ padding: isMobile ? "12px 16px" : "14px 20px", borderTop: "1px solid var(--border)", display: "flex", justifyContent: isMobile ? "stretch" : "flex-end" }}>
              <button className="btn btn-ghost" style={isMobile ? { width: "100%" } : S.btn} onClick={() => setSelectedProduct(null)}>Close</button>
            </div>
          </div>
        </div>
      )}
