...
const HistoryCard = ({ a }) => {
  ...
  return (
    <div style={{ background: 'var(--bg)', border: `1px solid ${col.border}`, borderRadius: 10, padding: '12px 16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4, gap: 8, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 6, minWidth: 0, maxWidth: '55%' }}>
          <img
            src="/icon/warning.png" alt=""
            style={{ width: 14, height: 14, flexShrink: 0, marginTop: 2, filter: 'brightness(0) saturate(100%) invert(20%) sepia(80%) saturate(2000%) hue-rotate(350deg)' }}
          />
          <strong style={{ fontSize: 13, color: '#dc2626', lineHeight: 1.4, wordBreak: 'break-word', minWidth: 0 }}>{a.type}</strong>
        </div>
    ...
    ? ReactDOM.createPortal(
      <div ref={ref} style={{ position: 'absolute', top: dropPos.top, left: dropPos.left, width: dropPos.width, background: 'var(--card)', borderRadius: 12, zIndex: 99999, boxShadow: '0 16px 40px rgba(0,0,0,0.13), 0 4px 12px rgba(0,0,0,0.07), 0 0 0 1px rgba(0,0,0,0.06)', border: '1.5px solid #e8edf4', maxHeight: 260, overflowY: 'auto', padding: '5px' }}>
        {options.map((opt, i) => {
          ...
          return (
            <div key={i} onClick={() => { if (!opt.disabled) { onChange(optVal); setOpen(false); } }}
              style={{ padding: '8px 10px', fontSize: 13, fontWeight: isSelected ? 700 : 500, color: opt.disabled ? '#cbd5e1' : isSelected ? accent : 'var(--text)', cursor: opt.disabled ? 'not-allowed' : 'pointer', transition: 'background 0.12s, color 0.12s', background: isSelected ? `${accent}12` : 'transparent', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, opacity: opt.disabled ? 0.45 : 1, marginBottom: 1 }}
              onMouseEnter={e => { if (!isSelected && !opt.disabled) e.currentTarget.style.background = '#f4f6fa'; }}
  ...
  return (
    <div style={{ position: 'relative', width: '100%' }}>
      <div ref={triggerRef} onClick={handleOpen}
        style={{ width: '100%', padding: '8px 34px 8px 12px', border: '1.5px solid', borderRadius: 9, background: disabled ? '#f8fafc' : open ? 'linear-gradient(135deg, #ffffff 0%, #fef2f2 100%)' : 'linear-gradient(to bottom, #ffffff 0%, #f8fafc 100%)', fontSize: 13, fontWeight: 600, color: disabled ? '#cbd5e1' : value ? 'var(--text)' : '#b0bac9', cursor: disabled ? 'not-allowed' : 'pointer', userSelect: 'none', boxSizing: 'border-box', boxShadow: open ? `0 0 0 3px ${accent}22, 0 2px 8px rgba(0,0,0,0.08)` : '0 1px 3px rgba(0,0,0,0.06), inset 0 1px 0 rgba(255,255,255,0.9)', borderColor: open ? accent : '#dde3ec', transition: 'border-color 0.18s, box-shadow 0.18s, background 0.18s', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, position: 'relative', minHeight: 36 }}
        onMouseEnter={e => { if (!open) { e.currentTarget.style.borderColor = '#fca5a5'; e.currentTarget.style.boxShadow = '0 2px 8px rgba(220,38,38,0.10), inset 0 1px 0 rgba(255,255,255,0.9)'; } }}
...
const ReportForm = memo(({ sending, onSend, defaultBranch }) => {
  ...
  const handleSend = useCallback(async () => {
    ...
    if (!form.branch) {
      ...
      hasErr = true;
    } else if (form.province && form.city && isLocationTooFar(form.branch, form.province, form.city)) {
      const d = Math.round(distanceToBranchKm(form.branch, form.province, form.city));
      setBranchErr(`${form.city} is about ${d}km from ${form.branch}, which is too far for this branch to respond. Please pick a closer branch.`);
      hasErr = true;
    } else {
      setBranchErr('');
    }
    ...
    const finalLocation = [form.street.trim(), form.barangay, form.city, form.province].filter(Boolean).join(', ');
    const finalType = form.type === OTHER_TYPE ? form.customType.trim() : form.type;
    const result = await onSend({ ...form, type: finalType, description: finalLocation });
    if (result?.success) {
      setForm({ type: '', customType: '', province: '', city: '', barangay: '', street: '', branch: defaultBranch || '' });
    }
  }, [form, onSend, defaultBranch]);

  const canSend = !!(
    form.type && (form.type !== OTHER_TYPE || form.customType.trim()) &&
    form.province && form.city && form.street.trim() &&
    form.branch &&
  ...
  return (
    <div style={{
      ...
      <div className="form-group" style={{ marginBottom: 14 }}>
        ...
        <CustomSelect
          value={form.type}
          onChange={val => { setForm(p => ({ ...p, type: val })); setTypeErr(''); }}
          options={EMERGENCY_TYPES.map(t => t === OTHER_TYPE ? { value: t, label: 'Other (describe emergency)' } : t)}
        ...
        {form.type === OTHER_TYPE && (
          <textarea
            ...
            onChange={e => { setForm(p => ({ ...p, customType: e.target.value })); setTypeErr(''); }}
            placeholder="Briefly describe the emergency"
            style={{ marginTop: 8, minHeight: 60, resize: 'vertical', border: `1.5px solid ${typeErr ? '#f87171' : 'var(--border)'}`, width: '100%', boxSizing: 'border-box', borderRadius: 8, fontSize: 13, fontFamily: 'inherit', padding: '8px 12px', background: 'var(--card)', color: 'var(--text)', outline: 'none' }}
          />
        )}
        {typeErr && <p style={{ fontSize: 11, color: '#dc2626', marginTop: 3 }}>{typeErr}</p>}
      </div>
      ...
      <div className="form-group" style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
          <div>
            ...
            <CustomSelect
              value={form.province}
              onChange={val => { setForm(p => ({ ...p, province: val, city: CITY_TO_PROVINCE[p.city] === val ? p.city : '', barangay: CITY_TO_PROVINCE[p.city] === val ? p.barangay : '' })); setDescErr(''); }}
          ...
          <div>
            ...
            <CustomSelect
              ...
              onChange={val => {
                setForm(p => ({
                  ...p, city: val, province: CITY_TO_PROVINCE[val] || '',
                  barangay: BARANGAYS_BY_CITY[val]?.includes(p.barangay) ? p.barangay : '',
                }));
                setDescErr('');
              }}
              options={(form.province ? (CITIES_BY_PROVINCE[form.province] || []) : ALL_CITIES).map(c => ({ value: c, label: c }))}
              placeholder="— City —"
              accent="#dc2626"
        ...
        <div style={{ marginBottom: 8 }}>
          ...
          <CustomSelect
            value={form.city ? form.barangay : (form.barangay ? `${form.barangay}||${form.city}` : '')}
            onChange={val => {
              if (form.city) {
                setForm(p => ({ ...p, barangay: val }));
              } else {
                const [brgy, city] = val.split('||');
                setForm(p => ({ ...p, barangay: brgy, city, province: CITY_TO_PROVINCE[city] || '' }));
              }
              setDescErr('');
            }}
            options={form.city
              ? (BARANGAYS_BY_CITY[form.city] || []).map(b => ({ value: b, label: b }))
        ...
        <div>
          ...
          <input
            type="text"
            value={form.street}
            onChange={e => { setForm(p => ({ ...p, street: e.target.value })); setDescErr(''); }}
            ...
            style={{
              ...
              background: !form.barangay ? '#f8fafc' : 'var(--card)', color: 'var(--text)', outline: 'none',
            }}
          />
        </div>
      ...
      <div className="form-group" style={{ marginBottom: 20 }}>
        <label>Nearest Branch <span style={{ color: '#dc2626' }}>*</span></label>
        <CustomSelect
          value={form.branch}
          onChange={val => { setForm(p => ({ ...p, branch: val })); setBranchErr(''); }}
          options={BRANCHES.map(b => ({ value: b, label: b }))}
          placeholder="— Select Branch —"
          accent="#dc2626"
          disabled={!form.barangay}
        />
        {branchErr && <p style={{ fontSize: 11, color: '#dc2626', marginTop: 3 }}>{branchErr}</p>}
      </div>
      ...
      >
        <svg viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" style={{ width: 16, height: 16 }}><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z" /><path d="M12 9v4" /><path d="M12 17h.01" /></svg>
         {sending ? 'Sending...' : 'Send Emergency Alert to Staff'}
      </button>

      {form.branch && form.province && form.city && isLocationTooFar(form.branch, form.province, form.city) && (
        <div style={{
          ...
          <svg viewBox="0 0 24 24" fill="none" stroke="#991b1b" strokeWidth="2" strokeLinecap="round" style={{ width: 14, height: 14, flexShrink: 0, marginTop: 1 }}><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z" /><path d="M12 9v4" /><path d="M12 17h.01" /></svg>
          <span>Cannot send: {form.city} is about {Math.round(distanceToBranchKm(form.branch, form.province, form.city))}km from {form.branch}, which is too far for this branch to respond. Please pick a closer branch.</span>
        </div>
      )}
      ...
      <div style={{
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="#92400e" strokeWidth="2" strokeLinecap="round" style={{ width: 14, height: 14, flexShrink: 0, marginTop: 1 }}><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z" /><path d="M12 9v4" /><path d="M12 17h.01" /></svg>
        <span>
...
const BranchCards = ({ alerts, branchAvailability }) => (
  <div style={{
    ...
    {BRANCHES.map(b => {
      ...
      return (
        <div key={b} className="fade-in" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{
              ...
              background: !isAvailable ? '#94a3b8' : hasActive ? '#dc2626' : '#16a34a',
              boxShadow: !isAvailable ? '0 0 0 2px #f1f5f9' : hasActive ? '0 0 0 2px #fee2e2' : '0 0 0 2px #dcfce7',
            }} />
            <span style={{ fontSize: 11, color: !isAvailable ? '#94a3b8' : hasActive ? '#dc2626' : '#16a34a', fontWeight: 600 }}>
              {!isAvailable
                ? 'Branch Unavailable'
                : hasActive
...
const SuccessModal = ({ onClose }) => (
  <div style={{
    ...
    <div style={{
      background: 'var(--card)', borderRadius: 16, padding: '36px 32px',
      boxShadow: '0 8px 40px rgba(0,0,0,0.18)',
      maxWidth: 380, width: '90%', textAlign: 'center',
    }}>
      <div style={{ marginBottom: 12, display: 'flex', justifyContent: 'center', color: '#dc2626' }}>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" style={{ width: 52, height: 52 }}><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z" /><path d="M12 9v4" /><path d="M12 17h.01" /></svg>
      </div>
  ...
  useEffect(() => {
    ...
    const loadAvailability = async () => {
      ...
      if (data && data.length > 0) {
        ...
        BRANCHES.forEach(b => { if (map[b] === undefined) map[b] = true; });
        setBranchAvailability(map);
      } else {
        // No data at all — default everything to true
        const map = {};
        BRANCHES.forEach(b => { map[b] = true; });
        setBranchAvailability(map);
      }
    };
    loadAvailability();
    const availCh = supabase.channel('branch-avail-customer-' + Date.now())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'branch_availability' },
       (payload) => {
          if (payload.new?.branch) {
            setBranchAvailability(prev => ({ ...prev, [payload.new.branch]: payload.new.available }));
          }
        })
       .subscribe();
    ...
    return () => { supabase.removeChannel(channel); supabase.removeChannel(availCh); };
        ...
        <div style={S.topbar} className="branches-topbar">
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0, overflow: 'hidden' }}>
            <img
              ...
              style={{ width: 18, height: 18, flexShrink: 0, filter: 'brightness(0) saturate(100%) invert(17%) sepia(82%) saturate(1200%) hue-rotate(210deg)' }}
            />
            <div style={{ minWidth: 0 }}>
              <h1 style={{ fontSize: 14, fontWeight: 800, color: 'var(--text)', margin: 0, lineHeight: 1.2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Emergency Alert</h1>
              <p style={{ fontSize: 10, color: 'var(--muted)', margin: 0, lineHeight: 1.3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Report your pet's emergency directly to our staff</p>
            </div>
          </div>
          <button
            onClick={fetchAlerts}
            style={{ fontSize: 11, fontWeight: 700, padding: '6px 10px', borderRadius: 8, border: '1.5px solid var(--border)', background: '#fff', cursor: 'pointer', fontFamily: 'inherit', color: 'var(--text)', width: 'auto', flexShrink: 0, whiteSpace: 'nowrap' }}
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ width: 13, height: 13, marginRight: 5 }}><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8" /><path d="M3 3v5h5" /></svg>
            Refresh
          </button>
        </div>
        ...
        <div style={S.cont}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
            ...
            <div className="fade-in" style={{ animationDelay: '0.05s' }}>
              <ReportForm sending={sending} onSend={sendAlert} defaultBranch="" />
            </div>
            ...
            <div className="fade-in" style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                <svg viewBox="0 0 24 24" fill="none" stroke="var(--royal)" strokeWidth="2" strokeLinecap="round" style={{ width: 16, height: 16 }}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /></svg>
              ...
              {loading ? (
                ...
                <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                  <div style={{ marginBottom: 12, display: 'flex', justifyContent: 'center', color: '#16a34a' }}>
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" style={{ width: 40, height: 40 }}><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg>
                  </div>
          ...
          <BranchCards alerts={alerts} branchAvailability={branchAvailability} />
        </div>
      </div>
    </Layout>
  );
