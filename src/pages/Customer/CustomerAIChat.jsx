...
const CustomSelect = ({ value, onChange, options, placeholder = "—", accent = "#1e3a8a" }) => {
  ...
  const portal = open && typeof document !== "undefined"
    ? ReactDOM.createPortal(
      <div ref={ref} style={{ position: "absolute", top: dropPos.top, left: dropPos.left, width: dropPos.width, background: "var(--card)", borderRadius: 12, zIndex: 99999, boxShadow: "0 16px 40px rgba(0,0,0,0.13), 0 4px 12px rgba(0,0,0,0.07), 0 0 0 1px rgba(0,0,0,0.06)", border: "1.5px solid var(--border)", maxHeight: 260, overflowY: "auto", padding: "5px" }}>
        {[{ value: "", label: placeholder }, ...options].map((opt, i) => {
          ...
          return (
            <div key={i} onClick={() => { if (!opt.disabled) { onChange(optVal); setOpen(false); } }}
              style={{ padding: "8px 10px", fontSize: 13, fontWeight: isSelected ? 700 : 500, color: opt.disabled ? "#cbd5e1" : isEmpty ? "#b0bac9" : isSelected ? accent : "#1e293b", cursor: opt.disabled ? "not-allowed" : "pointer", transition: "background 0.12s, color 0.12s", background: isSelected ? `${accent}12` : "transparent", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, opacity: opt.disabled ? 0.45 : 1, marginBottom: 1 }}
  ...
  return (
    <div style={{ position: "relative", width: "100%" }}>
      <div ref={triggerRef} onClick={handleOpen}
        style={{ width: "100%", padding: "10px 34px 10px 12px", border: "1.5px solid", borderRadius: 10, background: "var(--card)", fontSize: 14, fontWeight: 600, color: value ? "var(--text)" : "var(--muted)", cursor: "pointer", userSelect: "none", boxSizing: "border-box", boxShadow: open ? `0 0 0 3px ${accent}22, 0 2px 8px rgba(0,0,0,0.08)` : "0 1px 3px rgba(0,0,0,0.06)", borderColor: open ? accent : "var(--border)", transition: "border-color 0.18s, box-shadow 0.18s, background 0.18s", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, position: "relative", minHeight: 42 }}
...
const StepDotMobile = ({ n, active, done, label }) => (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
    <div style={{
      ...
      color: done || active ? "#fff" : "#94a3b8",
      display: "flex", alignItems: "center", justifyContent: "center",
      fontSize: done ? 11 : 11, fontWeight: 800, transition: "all 0.2s",
      border: `2px solid ${done ? "#16a34a" : active ? "#1e3a8a" : "#e2e8f0"}`,
      flexShrink: 0,
    }}>
      {done ? "✓" : n}
    ...
    <span style={{
      ...
      whiteSpace: "nowrap", maxWidth: 36, overflow: "hidden",
      textOverflow: "ellipsis", textAlign: "center", lineHeight: 1.2,
    }}>
      {label}
    </span>
  </div>
...
Owner's question: ${userText}`;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 0, background: "var(--card)", border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden", boxShadow: "0 2px 12px rgba(30,58,138,0.06)", height: "100%" }}>
      {/* Chat header */}
      ...
      {/* Messages */}
      <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "16px", display: "flex", flexDirection: "column", gap: 12, background: "var(--bg)" }}>
        {messages.map((msg, i) => (
          <div key={i} style={{ display: "flex", flexDirection: msg.role === "user" ? "row-reverse" : "row", gap: 8, alignItems: "flex-start" }}>
            {msg.role === "assistant" && (
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: "linear-gradient(135deg,#1a1a6e,#1e3a8a)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2 }}>
            ...
            <div style={{
              ...
              background: msg.role === "user" ? "#1e3a8a" : msg.isError ? "#fef2f2" : "var(--card)",
              color: msg.role === "user" ? "#fff" : msg.isError ? "#dc2626" : "var(--text)",
              fontSize: 13,
              lineHeight: 1.6,
        ...
        {chatLoading && (
          <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
            <div style={{ width: 28, height: 28, borderRadius: "50%", background: "linear-gradient(135deg,#1a1a6e,#1e3a8a)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <AIIcon size={15} color="#fff" />
            </div>
            <div style={{ padding: "10px 14px", borderRadius: "4px 16px 16px 16px", background: "var(--card)", border: "1px solid var(--border)" }}>
              <TypingDots />
            </div>
          </div>
        )}
      ...
      {/* Suggested questions */}
      {messages.length <= 1 && (
        <div style={{ padding: "10px 16px 0", background: "var(--bg)", borderTop: "1px solid var(--border)" }}>
          <p style={{ fontSize: 11, color: "var(--muted)", fontWeight: 700, margin: "0 0 8px", textTransform: "uppercase", letterSpacing: 0.5 }}>Suggested questions</p>
      ...
      {/* Input */}
      <div style={{ padding: "12px 16px", background: "var(--card)", borderTop: "1px solid var(--border)", display: "flex", gap: 8 }}>
        <textarea
          ref={inputRef}
          value={input}
          ...
          style={{ flex: 1, padding: "9px 12px", borderRadius: 10, border: "1.5px solid var(--border)", fontSize: 13, fontFamily: "inherit", outline: "none", resize: "none", lineHeight: 1.5, transition: "border-color 0.15s", background: "var(--bg)", color: "var(--text)" }}
      ...
      {/* Proceed CTA */}
      <div style={{ padding: "12px 16px 16px", background: "var(--card)", borderTop: "1px solid var(--border)" }}>
        <button
...
- IMPORTANT: Write every text value above (urgencyReason, summary, warningSigns, homeCareTips, appointment_notes, and condition names) in the SAME language the owner used in SYMPTOMS/ADDITIONAL NOTES — English, Tagalog/Filipino, Taglish, Bisaya, or any other language. Detect it naturally from their wording and match it. Keep the JSON keys themselves exactly as shown above in English — only the values should switch language.`;
  ...
  const reset = () => {
    setStep(1); setAssessment(null); setSaved(null); setError(""); setShowChat(false); setPetMode("new");
  ...
  const S = {
    ...
    card: { background: "var(--card)", borderRadius: 16, border: "1px solid var(--border)", boxShadow: "0 2px 12px rgba(30,58,138,0.06)", padding: 24, marginBottom: 16 },
    inp: { width: "100%", padding: "11px 14px", borderRadius: 10, border: "1.5px solid var(--border)", fontSize: 14, fontFamily: "inherit", outline: "none", boxSizing: "border-box", transition: "border-color 0.15s", background: "var(--card)", color: "var(--text)" },
    label: { fontSize: 12, fontWeight: 700, color: "var(--muted)", display: "block", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.4px" },
  };
  ...
  return (
    <Layout>
      <div style={S.page}>
        ...
        {/* ── Page Header ── */}
        <div style={{ background: "linear-gradient(135deg,#1a1a6e,#1e3a8a)", borderRadius: 16, padding: "16px 18px", marginBottom: 16, display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, flex: 1, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <h1 style={{ fontSize: 15, fontWeight: 800, color: "#fff", margin: "0 0 2px", lineHeight: 1.3 }}>AI Symptom Pre-Assessment</h1>
              <p style={{ fontSize: 11, color: "rgba(255,255,255,0.65)", margin: 0, lineHeight: 1.4 }}>Describe symptoms · Get AI triage · Book</p>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
            {step === 6 && (
              <button onClick={() => navigate("/customer/appointments")} style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", color: "#fff", fontFamily: "inherit" }}>
                View Appointments
              </button>
            )}
            {step === 4 && (
              <button onClick={reset} style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", color: "#fff", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 5 }}>
                <Icons.RotateCcw /> New
              </button>
            )}
            {step > 1 && step < 6 && (
              <button onClick={reset} style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", color: "#fff", fontFamily: "inherit" }}>
                ← Start Over
              </button>
            )}
          </div>
        ...
        {/* ── Step indicator ── */}
        <div style={{ background: "var(--card)", borderBottom: "1px solid var(--border)", padding: "10px 12px", display: "flex", alignItems: "center", gap: 0, borderRadius: 12, marginBottom: 16 }}>
          <div style={{ maxWidth: 600, margin: "0 auto", display: "flex", alignItems: "center", gap: 0, width: "100%" }}>
            {STEPS.map((label, i) => (
              <React.Fragment key={label}>
                <StepDotMobile n={i + 1} active={step === i + 1} done={step > i + 1} label={label} />
        ...
        <div style={S.cont}>
          ...
          {step === 1 && (
            <div className="ai-enter" style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              <div style={{ background: "linear-gradient(135deg,#1a1a6e,#1e3a8a)", borderRadius: 16, padding: "20px 24px", display: "flex", gap: 14, alignItems: "center" }}>
                <div style={{ color: "#fff", opacity: 0.9 }}><PetsIcon size={36} color="#fff" /></div>
                <div>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                  {[{ key: "new", label: "New Pet" }, { key: "existing", label: "Existing Pet" }].map(({ key, label }) => (
                    ...
                    }} style={{ padding: "6px 16px", borderRadius: 8, border: `1.5px solid ${petMode === key ? "#1e3a8a" : "#e2e8f0"}`, background: petMode === key ? "#1e3a8a" : "#fff", color: petMode === key ? "#fff" : "#374151", fontFamily: "inherit", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                      {label}
                    </button>
                  ))}
                </div>
                {petMode === "existing" ? (
                  loadingExistingPatients ? (
                    <p style={{ fontSize: 12, color: "#94a3b8", margin: 0 }}>Loading your pets…</p>
                  ) : existingPatients.length === 0 ? (
                    <p style={{ fontSize: 12, color: "#94a3b8", margin: 0 }}>No registered pets found yet. Switch to "New Pet" to enter one.</p>
                  ) : (
                    <CustomSelect
                      value={form.petName}
                      onChange={val => {
                        const sel = existingPatients.find(p => p.name === val);
                        set("petName", sel?.name || "");
                        if (sel?.species && PET_TYPES.includes(sel.species)) set("petType", sel.species);
                      }}
                      placeholder="— Select a pet —"
                      accent="#1e3a8a"
                      options={existingPatients.map(p => ({ value: p.name, label: p.name + (p.species ? ` (${p.species})` : "") }))}
                    />
                  )
                ) : (
              ...
              <div>
                <label style={S.label}>Type of Pet *</label>                <div style={{ display: "flex", gap: 8 }}>
                  {PET_TYPES.map(t => {
                    const active = form.petType === t;
                    return (
                      <button key={t} className="chip-btn" onClick={() => set("petType", t)} style={{ flex: 1, padding: "12px 14px", borderRadius: 12, fontFamily: "inherit", fontSize: 13, fontWeight: 700, cursor: "pointer", border: `1.5px solid ${active ? "#1e3a8a" : "#e2e8f0"}`, background: active ? "#1e3a8a" : "#fff", color: active ? "#fff" : "#374151", transition: "all 0.15s", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                        {t === "Cat"
                          ? <svg width="18" height="18" viewBox="0 0 16 16" fill={active ? "#fff" : "#374151"} xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" clipRule="evenodd" d="M1 7L4.80061 1.43926C5.56059 0.527292 6.68638 0 7.8735 0H8V4L12 5L15 10L14.1875 11.2188C13.4456 12.3316 12.1967 13 10.8593 13H9L7 16H5L1 7ZM10 9C10.5523 9 11 8.55229 11 8C11 7.44772 10.5523 7 10 7C9.44771 7 9 7.44772 9 8C9 8.55229 9.44771 9 10 9Z" /><path d="M10 0.465878V2.43845L12 2.93845V0H11.8735C11.2125 0 10.5704 0.163501 10 0.465878Z" /></svg>
                          : <svg width="18" height="18" viewBox="0 0 16 16" fill={active ? "#fff" : "#374151"} xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" clipRule="evenodd" d="M16 4V7C16 9.20914 14.2091 11 12 11H10V15H0V13L0.931622 10.8706C1.25226 10.9549 1.59036 11 1.94124 11C3.74931 11 5.32536 9.76947 5.76388 8.01538L3.82359 7.53031C3.60766 8.39406 2.83158 9.00001 1.94124 9.00001C1.87789 9.00001 1.81539 8.99702 1.75385 8.99119C1.02587 8.92223 0.432187 8.45551 0.160283 7.83121C0.0791432 7.64491 0.0266588 7.44457 0.00781272 7.23658C-0.0112323 7.02639 0.00407892 6.80838 0.0588889 6.58914L0.698705 4.02986C1.14387 2.24919 2.7438 1 4.57928 1H10L12 4H16ZM9 6C9.55229 6 10 5.55228 10 5C10 4.44772 9.55229 4 9 4C8.44771 4 8 4.44772 8 5C8 5.55228 8.44771 6 9 6Z" /></svg>
                        }
                        {t}
                      </button>
                    );
          ...
          {step === 2 && (
            <div className="ai-enter" style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              <div style={{ background: "linear-gradient(135deg,#1a1a6e,#1e3a8a)", borderRadius: 16, padding: "20px 24px", display: "flex", gap: 14, alignItems: "center" }}>
                <div style={{ color: "#fff", opacity: 0.9 }}><StethoscopeIcon size={36} color="#fff" /></div>
                <div>
              ...
              <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 10, padding: "12px 14px", display: "flex", gap: 10, alignItems: "flex-start" }}>
                <span style={{ color: "#92400e", flexShrink: 0, marginTop: 1 }}><Icons.AlertTriangle /></span>
              ...
              {error && <div style={{ background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: 10, padding: "12px 14px", fontSize: 13, color: "#dc2626", fontWeight: 600, display: "flex", alignItems: "center", gap: 8 }}><Icons.AlertTriangle /> {error}</div>}

              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setStep(1)} style={{ padding: "13px 20px", borderRadius: 12, border: "1.5px solid var(--border)", background: "var(--bg)", fontFamily: "inherit", fontSize: 14, fontWeight: 700, cursor: "pointer", color: "var(--muted)" }}>← Back</button>
                <button onClick={analyzeSymptoms} disabled={!canSubmit} style={{ flex: 1, padding: 13, borderRadius: 12, border: "none", background: canSubmit ? "#1e3a8a" : "#e2e8f0", color: canSubmit ? "#fff" : "#94a3b8", fontFamily: "inherit", fontSize: 14, fontWeight: 800, cursor: canSubmit ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "all 0.15s" }}>
                  <StethoscopeIcon size={16} color={canSubmit ? "#fff" : "#94a3b8"} /> Analyze with Gemini AI
                </button>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="ai-enter" style={{ ...S.card, textAlign: "center", padding: "48px 24px" }}>
              <div style={{ width: 64, height: 64, borderRadius: "50%", background: "linear-gradient(135deg,#ede9fe,#dbeafe)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", animation: "pulse 1.5s infinite" }}>
          ...
          {step === 4 && assessment && urgency && (
            <div className="ai-enter" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {/* Pet row */}
              <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ color: "#1e3a8a" }}><PetsIcon size={24} color="#1e3a8a" /></div>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 800, color: "var(--text)" }}>{form.petName} · {form.petType} · {form.petAge}</div>
                  <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2 }}>Assessment complete · Owner: {form.ownerName}</div>
                </div>
              </div>

              {/* Summary */}
              <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, padding: 16 }}>
                <p style={{ fontSize: 11, fontWeight: 800, color: "var(--muted)", textTransform: "uppercase", letterSpacing: 0.8, margin: "0 0 10px" }}>AI Summary</p>
                <p style={{ fontSize: 14, color: "var(--text)", lineHeight: 1.7, margin: 0 }}>{assessment.summary}</p>
              </div>

              {/* Conditions + Service */}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, padding: "14px 16px" }}>
                  <p style={{ fontSize: 11, fontWeight: 800, color: "var(--muted)", textTransform: "uppercase", letterSpacing: 0.8, margin: "0 0 10px" }}>Possible Conditions</p>
                  {assessment.conditions?.map((c, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 6 }}>
                      ...
                      <span style={{ fontSize: 12, color: "var(--text)", lineHeight: 1.5 }}>{c}</span>
                    </div>
                  ))}
                ...
                <div style={{ background: "#e8edf8", border: "1px solid #c7d4f0", borderRadius: 12, padding: "14px 16px" }}>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 800, color: "#1e3a8a", marginBottom: 6, display: "flex", alignItems: "center", gap: 6 }}>
                    <StethoscopeIcon size={16} color="#1e3a8a" /> {assessment.recommendedService}
                  </div>
              ...
              {assessment.warningSigns?.length > 0 && (
                <div style={{ background: "#fff7ed", border: "1px solid #fdba74", borderRadius: 12, padding: "14px 16px" }}>
                  <p style={{ fontSize: 11, fontWeight: 800, color: "#ea580c", textTransform: "uppercase", letterSpacing: 0.8, margin: "0 0 10px", display: "flex", alignItems: "center", gap: 6 }}><Icons.Zap /> Watch for these warning signs</p>
              ...
              {["Low", "Moderate"].includes(assessment.urgency) && assessment.homeCareTips?.length > 0 && (
                <div style={{ background: "#f0fdf4", border: "1px solid #86efac", borderRadius: 12, padding: "14px 16px" }}>
                  <p style={{ fontSize: 11, fontWeight: 800, color: "#16a34a", textTransform: "uppercase", letterSpacing: 0.8, margin: "0 0 10px", display: "flex", alignItems: "center", gap: 6 }}><Icons.Home /> Home care tips in the meantime</p>
                  {assessment.homeCareTips.map((t, i) => (
                    <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginBottom: 6 }}>
                      <span style={{ color: "#16a34a", flexShrink: 0 }}><Icons.CheckCircle /></span>
              ...
              {!showChat && (
                <div style={{ background: "linear-gradient(135deg,#f0f4ff,#e8edf8)", border: "1.5px solid #c7d4f0", borderRadius: 14, padding: "16px 20px", display: "flex", alignItems: "center", gap: 14 }}>
                  <span style={{ color: "#1e3a8a", flexShrink: 0 }}><Icons.MessageCircle /></span>
                  <div style={{ flex: 1 }}>
              ...
              {showChat && (
                <div style={{ position: "fixed", inset: 0, zIndex: 99999, background: "var(--bg)", display: "flex", flexDirection: "column" }} className="ai-enter">
                  <button
                    onClick={() => setShowChat(false)}
                    aria-label="Close chat"
                    style={{ position: "absolute", top: 14, right: 14, zIndex: 2, width: 36, height: 36, borderRadius: "50%", border: "1px solid var(--border)", background: "var(--card)", color: "var(--text)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}
                  >
                   ...
                   <div style={{ flex: 1, minHeight: 0, width: "100%", boxSizing: "border-box", padding: "16px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    ...
                    <FollowUpChat
                      ...
                      petAge={form.petAge}
                      symptoms={form.symptoms}
                      onProceedToBook={() => { setShowChat(false); setStep(5); }}
                    />
                    </div>
                  </div>
                </div>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                {!showChat && (
                  <button onClick={() => setStep(5)} style={{ flex: 1, padding: 14, borderRadius: 12, border: "none", background: "linear-gradient(135deg,#1a1a6e,#1e3a8a)", color: "#fff", fontFamily: "inherit", fontSize: 14, fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
          ...
          {step === 5 && assessment && (
            <div className="ai-enter" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={S.card}>
                ...
                <div style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 10, padding: 14, marginBottom: 16, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
                  {[["Patient", form.petName], ["Owner", form.ownerName], ["Contact", form.contact || "—"], ["Service", assessment.recommendedService], ["Urgency", assessment.urgency]].map(([l, v]) => (
                    <div key={l} style={{ padding: "6px 0", borderBottom: "1px solid var(--border)" }}>
                      <span style={{ fontSize: 11, color: "var(--muted)", fontWeight: 700 }}>{l}: </span>
          ...
          {step === 6 && saved && (
            <div className="ai-enter" style={{ ...S.card, textAlign: "center", padding: "48px 24px" }}>
              <div style={{ width: 72, height: 72, borderRadius: "50%", background: "linear-gradient(135deg,#f0fdf4,#dcfce7)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", border: "2px solid #bbf7d0" }}>
                <svg viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ width: 36, height: 36 }}>
