...
const CustomerDashboard = () => {
  ...
  const STAT_CARDS = [
    { label: 'My Pets', value: loading ? '—' : pets.length, icon: '/icon/paw.png', color: '#dbeafe', filter: 'invert(37%) sepia(90%) saturate(500%) hue-rotate(195deg) brightness(95%)', accent: '#3b82f6', sub: 'Total registered', subColor: 'var(--muted)' },
    { label: 'Upcoming Appts', value: loading ? '—' : appts.length, icon: '/icon/appointment.png', color: '#dcfce7', filter: 'invert(50%) sepia(60%) saturate(400%) hue-rotate(100deg) brightness(90%)', accent: '#16a34a', sub: appts.length === 0 ? 'None scheduled' : 'Scheduled ahead', subColor: '#16a34a' },
    { label: 'Admitted Pets', value: loading ? '—' : pets.filter(p => p.status === 'Admitted').length, icon: '/icon/hospital-svgrepo-com.svg', color: '#fef9c3', filter: 'invert(70%) sepia(80%) saturate(500%) hue-rotate(5deg) brightness(95%)', accent: '#d97706', sub: 'Currently admitted', subColor: 'var(--muted)' },    { label: 'Pending Appts', value: loading ? '—' : appts.filter(a => a.status === 'Pending').length, icon: '/icon/pending.png', color: '#fee2e2', filter: 'invert(30%) sepia(80%) saturate(500%) hue-rotate(330deg) brightness(95%)', accent: '#dc2626', sub: appts.filter(a => a.status === 'Pending').length === 0 ? 'All cleared' : 'Awaiting confirmation', subColor: appts.filter(a => a.status === 'Pending').length === 0 ? '#16a34a' : '#dc2626' },
  ];
  ...
  return (
    <Layout isCustomer>
      <div style={{ width: '100%', display: 'block', minHeight: '100%', overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
        <div style={{
          padding: isMobile ? '12px' : '24px 28px',
          paddingTop: isMobile ? '68px' : 'calc(var(--topbar-h) + 20px)',
          ...
          <div className="fade-in" style={{
            ...
            <div>
               <h1 style={{ fontSize: isMobile ? 17 : 24, fontWeight: 800, color: '#fff', marginBottom: 4 }}>
            ...
            <div style={{ textAlign: isMobile ? 'left' : 'right', color: 'rgba(255,255,255,0.75)', fontSize: isMobile ? 13 : 15, zIndex: 1 }}>
              <strong style={{ display: 'block', fontSize: isMobile ? 15 : 18, color: '#fff', fontWeight: 700 }}>{dateStr}</strong>
              {timeStr}
            </div>
          </div>
          ...
          <div style={{
            ...
            {STAT_CARDS.map((sc, i) => (
              <div key={i} className="fade-in" style={{
                ...
                onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = '0 8px 24px rgba(30,58,138,0.12)'; e.currentTarget.style.border = '1px solid rgba(30,58,138,0.3)'; e.currentTarget.style.borderTop = `3px solid ${sc.accent || 'var(--royal)'}`; }}
                onMouseLeave={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = 'var(--shadow)'; e.currentTarget.style.border = '1px solid var(--border)'; e.currentTarget.style.borderTop = `3px solid ${sc.accent || 'var(--royal)'}`; }}
              >
                <div style={{
                  ...
                  <img src={sc.icon} alt={sc.label} width={isMobile ? 16 : 20} height={isMobile ? 16 : 20} style={{ filter: sc.filter, display: 'block' }} />
                </div>
                <div style={{ minWidth: 0 }}>
                  <p style={{ fontSize: isMobile ? 9 : 10, color: 'var(--muted)', fontWeight: 700, marginBottom: 3, textTransform: 'uppercase', letterSpacing: '0.6px' }}>{sc.label}</p>
                  <h3 style={{ fontSize: isMobile ? 22 : 28, fontWeight: 800, color: 'var(--text)', margin: '0 0 2px', lineHeight: 1 }}>{sc.value}</h3>
                  <p style={{ fontSize: isMobile ? 9 : 10, color: sc.subColor || 'var(--muted)', margin: 0, fontWeight: 600 }}>{sc.sub}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{
            ...
            {QUICK_LINKS.map((ql, i) => (
              <Link key={ql.to} to={ql.to} className="fade-in" style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 10 : 14 }}>
            ...
            <div className="fade-in" style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', boxShadow: 'var(--shadow)', animationDelay: '0.55s' }}>
            ...
            <div className="fade-in" style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', boxShadow: 'var(--shadow)', animationDelay: '0.62s' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: isMobile ? '10px 12px' : '13px 16px', borderBottom: '1px solid var(--border)' }}>
