...
const DatePicker = ({ value, onChange, placeholder = 'Pick a date', min = '' }) => {
  ...
  const handleOpen = () => {
    if (!open && triggerRef.current) {
      ...
      const spaceBelow = window.innerHeight - rect.bottom;
      setPopPos({
        top: spaceBelow > 320 ? rect.bottom + window.scrollY + 6 : rect.top + window.scrollY - 310 - 6,
        left: rect.left + window.scrollX,
...
const PET_ICON_DOG = (color) => (
  <svg width="14" height="14" viewBox="0 0 16 16" fill={color} xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" clipRule="evenodd" d="M16 4V7C16 9.20914 14.2091 11 12 11H10V15H0V13L0.931622 10.8706C1.25226 10.9549 1.59036 11 1.94124 11C3.74931 11 5.32536 9.76947 5.76388 8.01538L3.82359 7.53031C3.60766 8.39406 2.83158 9.00001 1.94124 9.00001C1.87789 9.00001 1.81539 8.99702 1.75385 8.99119C1.02587 8.92223 0.432187 8.45551 0.160283 7.83121C0.0791432 7.64491 0.0266588 7.44457 0.00781272 7.23658C-0.0112323 7.02639 0.00407892 6.80838 0.0588889 6.58914L0.698705 4.02986C1.14387 2.24919 2.7438 1 4.57928 1H10L12 4H16ZM9 6C9.55229 6 10 5.55228 10 5C10 4.44772 9.55229 4 9 4C8.44771 4 8 4.44772 8 5C8 5.55228 8.44771 6 9 6Z" /></svg>
);
const PET_ICON_CAT = (color) => (
  <svg width="14" height="14" viewBox="0 0 16 16" fill={color} xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" clipRule="evenodd" d="M1 7L4.80061 1.43926C5.56059 0.527292 6.68638 0 7.8735 0H8V4L12 5L15 10L14.1875 11.2188C13.4456 12.3316 12.1967 13 10.8593 13H9L7 16H5L1 7ZM10 9C10.5523 9 11 8.55229 11 8C11 7.44772 10.5523 7 10 7C9.44771 7 9 7.44772 9 8C9 8.55229 9.44771 9 10 9Z" /><path d="M10 0.465878V2.43845L12 2.93845V0H11.8735C11.2125 0 10.5704 0.163501 10 0.465878Z" /></svg>
);
...
const PetPicker = ({ pets, value, onChange }) => {
  ...
  const speciesColor = (sp) => sp === 'Cat' ? { bg: '#f0fdf4', color: '#15803d' } : { bg: '#eff6ff', color: '#1d4ed8' };

  const portal = open && typeof document !== 'undefined' ? ReactDOM.createPortal(
    <div ref={ref} style={{ position: 'absolute', top: dropPos.top, left: dropPos.left, width: dropPos.width, background: 'var(--card)', borderRadius: 12, zIndex: 99999, boxShadow: '0 16px 40px rgba(0,0,0,0.13), 0 4px 12px rgba(0,0,0,0.07), 0 0 0 1px rgba(0,0,0,0.06)', border: '1.5px solid #e8edf4', maxHeight: 280, overflowY: 'auto', padding: '6px' }}>
      {pets.map(mp => {
        const isSelected = mp.id === value;
        const sc = speciesColor(mp.species);
        return (
          <div key={mp.id} onClick={() => { onChange(mp.id); setOpen(false); }}
            style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 9, cursor: 'pointer', background: isSelected ? '#eff6ff' : 'transparent', transition: 'background 0.12s', marginBottom: 2 }}
            ...
            <div style={{ width: 34, height: 34, borderRadius: 9, background: sc.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              {mp.species === 'Cat' ? PET_ICON_CAT(sc.color) : PET_ICON_DOG(sc.color)}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{mp.name}</div>
              {mp.species && <div style={{ fontSize: 11, color: sc.color, fontWeight: 600 }}>{mp.species}</div>}
            </div>
            {isSelected && (
              <div style={{ width: 18, height: 18, borderRadius: 5, background: '#1d4ed8', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12" /></svg>
              </div>
            )}
          </div>
  ...
  return (
    <div style={{ position: 'relative', width: '100%' }}>
      <div ref={triggerRef} onClick={handleOpen}
        style={{ width: '100%', padding: selected ? '6px 34px 6px 8px' : '10px 34px 10px 12px', border: '1.5px solid', borderRadius: 9, background: open ? 'linear-gradient(135deg,#ffffff,#f5f3ff)' : 'linear-gradient(to bottom,#ffffff,#f8fafc)', cursor: 'pointer', userSelect: 'none', boxSizing: 'border-box', boxShadow: open ? '0 0 0 3px rgba(99,102,241,0.12),0 2px 8px rgba(0,0,0,0.08)' : '0 1px 3px rgba(0,0,0,0.06),inset 0 1px 0 rgba(255,255,255,0.9)', borderColor: open ? '#6366f1' : '#dde3ec', transition: 'all 0.18s', display: 'flex', alignItems: 'center', gap: 8, position: 'relative', minHeight: 36 }}
        onMouseEnter={e => { if (!open) e.currentTarget.style.borderColor = '#a5b4fc'; }}
        onMouseLeave={e => { if (!open) e.currentTarget.style.borderColor = '#dde3ec'; }}>
        {selected ? (
          <>
            <div style={{ width: 26, height: 26, borderRadius: 7, background: speciesColor(selected.species).bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              {selected.species === 'Cat' ? PET_ICON_CAT(speciesColor(selected.species).color) : PET_ICON_DOG(speciesColor(selected.species).color)}
            </div>
            <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>
              {selected.name}{selected.species ? ` · ${selected.species}` : ''}
            </span>
          </>
        ) : (
          <span style={{ fontSize: 13, fontWeight: 600, color: '#b0bac9', flex: 1 }}>— Select a pet —</span>
        )}
        <div style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', width: 20, height: 20, borderRadius: 6, background: open ? '#6366f1' : '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.18s', flexShrink: 0 }}>
          <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke={open ? '#fff' : '#94a3b8'} strokeWidth="3" strokeLinecap="round" style={{ transition: 'transform 0.2s, stroke 0.18s', transform: open ? 'rotate(180deg)' : 'rotate(0deg)' }}>
            <polyline points="6 9 12 15 18 9" />
          </svg>
        </div>
...
const Modal = ({ show, title, message, onConfirm, onCancel, confirmText = 'OK', cancelText = null, confirmColor = 'var(--royal)' }) => {
  if (!show) return null;
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100000, padding: 16 }}>
      <div style={{ background: 'var(--card)', borderRadius: 14, width: '100%', maxWidth: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.28)', overflow: 'hidden' }}>
...
// ── Vet availability schedule (which weekdays each vet works) ──
const VET_SCHEDULE = {
...
const isVetAvailableOnDate = (vet, dateStr) => {
  ...
  const sched = VET_SCHEDULE[vet];
  if (!sched) return true;
  const day = new Date(dateStr + 'T00:00:00').getDay();
  return sched.includes(day);
};

// ── Vet time-of-day availability — which slots each vet actually works ──
// Edit these to match each vet's real hours.
const VET_TIME_SCHEDULE = {
  'Dr. Santos': ['11:00 AM', '01:00 PM', '02:00 PM', '04:00 PM'],
...
const isVetAvailableAtTime = (vet, time) => {
  ...
  const sched = VET_TIME_SCHEDULE[vet];
  if (!sched) return true;
...
const CustomerAppointment = () => {
  ...
  const filteredAppts = apptStatFilter === 'upcoming'
  ...
  // ── Multi-pet booking ─────────────────────────────────────────────────────
  const EMPTY_PET = { mode: 'new', existingId: null, patient: '', species: 'Dog', purpose: 'Consultation', imagingType: '', vet: 'Any Available', date: '', time: '', notes: '' };
  ...
  const fetchVetBookedTimes = async (vet, date) => {
    ...
    if (!error) {
      setVetBookedTimes(prev => ({ ...prev, [key]: (data || []).map(r => r.time) }));
    }
  };

  const getVetConflictFor = (pet) => {
    if (!pet.vet || pet.vet === 'Any Available' || !pet.date || !pet.time) return null;
  ...
  const [reviewModal, setReviewModal] = useState({ show: false, appt: null, rating: 0, comment: '' });
...
useEffect(() => {
  ...
  const fetchMyReviews = async () => {
    if (!user?.id) return;
    const { data, error } = await supabase.from('reviews').select('appointment_id, rating, comment').eq('user_id', user.id);
    if (!error && data) {
      const map = {};
  ...
  const openReviewModal = (appt) => setReviewModal({ show: true, appt, rating: 0, comment: '' });
  ...
  const openReviews = () => { fetchAllReviews(); setShowReviews(true); };

  const submitReview = async () => {
    if (!reviewModal.rating) { showAlert('Rating Required', 'Please select a star rating before submitting.'); return; }
    ...
    setMyReviews(prev => ({ ...prev, [reviewModal.appt.id]: { rating: reviewModal.rating, comment: reviewModal.comment.trim() } }));
  ...
  // ── Pet list helpers ───────────────────────────────────────────────────
  const updatePet = (idx, patch) => setPets(prev => prev.map((p, i) => i === idx ? { ...p, ...patch } : p));
  const addPet = () => setPets(prev => [...prev, { ...EMPTY_PET, purpose: prev[0]?.purpose || EMPTY_PET.purpose }]);
  const removePet = (idx) => setPets(prev => prev.filter((_, i) => i !== idx));

  const renderTimeOptions = (pet, idx) => TIMES.map(t => (
  ...
  const saveAppt = async () => {
    for (const pet of pets) {
      if (pet.mode === 'existing' && !pet.existingId) { showAlert('Missing Fields', 'Please select an existing pet for each entry, or switch to "New Pet".'); return; }
      if (pet.mode === 'new' && !pet.patient.trim()) { showAlert('Missing Fields', 'Please enter a pet name for each new pet.'); return; }
      if (!pet.vet || pet.vet === '') { showAlert('Missing Fields', 'Please select a veterinarian for each pet.'); return; }
      if (!pet.date || !pet.time) { showAlert('Missing Fields', 'Please select a date and time for each pet.'); return; }
      if (pet.purpose === 'Imaging' && !pet.imagingType) { showAlert('Missing Fields', 'Please select an imaging type (X-ray, Ultrasound, or CT-scan) for each pet.'); return; }
      const vetConflict = getVetConflictFor(pet);
      if (vetConflict === 'vet-day') { showAlert('Vet Unavailable', `${pet.vet} does not work on the selected date. Please choose a different day or vet.`); return; }
      if (vetConflict === 'vet-time') { showAlert('Vet Unavailable', `${pet.vet} is not available at ${pet.time}. Please choose one of their available time slots.`); return; }
      if (vetConflict === 'vet-slot') { showAlert('Time Slot Unavailable', `${pet.vet} is already booked at ${pet.time} on ${pet.date}. Please choose a different time.`); return; }
    }
    if (saving) return;
    setSaving(true);
    ...
    try {
      for (const pet of pets) {
        const petName = pet.mode === 'existing'
          ? (myPets.find(mp => mp.id === pet.existingId)?.name || '')
          : pet.patient.trim();
        const petSpecies = pet.mode === 'existing'
        ...
        if (existingAppt) {
          showAlert('Already Booked', `${petName} already has an appointment on ${pet.date} at ${pet.time}. Please choose a different date or time.`);
          setSaving(false);
          return;
        }
         ...
         const apptPayload = withBranchId(user, {
          ...
          time: pet.time,
          purpose: pet.purpose,
          price: getServicePrice(pet.purpose, user.branchId, pet.imagingType) || 0,
          status: 'Pending',
          notes: pet.notes,
        });
        ...
        if (pet.mode === 'new') {
          ...
          if (!existingPet) {
            ...
            if (petErr) {
              ...
              showAlert('Pet Record Warning', `Your appointment was booked, but we couldn't save ${petName}'s pet profile automatically (${petErr.message}). Please contact staff to register ${petName} in your account.`);
            }
          }
        }
      }
      ...
      if (isMidnightBookingHour()) {
        ...
        showAlert('Request Submitted!', `Your appointment request${pets.length > 1 ? 's have' : ' has'} been submitted and ${pets.length > 1 ? 'are' : 'is'} now pending approval. You will be notified once a staff member confirms ${pets.length > 1 ? 'them' : 'it'}.`);
      }
      setShowModal(false);
      setPets([{ ...EMPTY_PET }]);
      setContact('');
  ...
  const isPetIncomplete = (pet) => {
    const hasPetIdentity = pet.mode === 'existing' ? !!pet.existingId : !!pet.patient.trim();
    const hasVet = !!pet.vet;
    return !hasPetIdentity || !hasVet || !pet.date || !pet.time;
  };
  const isFormIncomplete = pets.some(isPetIncomplete) || !contact.trim() || contact.length !== 11;
      ...
      <Layout isCustomer>
        <div style={{ padding: '20px 24px', paddingTop: 'calc(var(--topbar-h) + var(--pagetop-h) + 20px)', width: '100%', boxSizing: 'border-box' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 18 }}>
            {[...Array(3)].map((_, i) => (
              <div key={i} style={{ background: 'var(--card)', borderRadius: 11, border: '1px solid var(--border)', padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {/* Table skeleton */}
          <div style={{ background: 'var(--card)', borderRadius: 12, border: '1px solid var(--border)', overflow: 'hidden' }}>            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
  ...
  return (
    <Layout isCustomer>
          ...
          <div style={{ background: 'var(--card)', borderRadius: 14, width: '100%', maxWidth: 560, maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.28)', overflow: 'hidden' }}>
            <div style={{ background: 'linear-gradient(135deg,#78350f,#d97706)', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <p style={{ margin: '4px 0 0', fontSize: 12, color: 'rgba(255,255,255,0.85)' }}>{allReviews.length} review{allReviews.length !== 1 ? 's' : ''}</p>
              </div>
            ...
            <div style={{ overflowY: 'auto', flex: 1, padding: '14px 20px' }}>
              {loadingAllReviews ? (
                ...
                <p style={{ textAlign: 'center', color: 'var(--muted)', padding: 20 }}>No reviews yet. Be the first to share your experience!</p>
              ) : allReviews.map(r => (
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '12px 20px', borderTop: '1px solid var(--border)' }}>
              <button className="btn btn-ghost" style={{ width: 'auto' }} onClick={() => setShowReviews(false)}>Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ width: '100%', minHeight: '100vh', display: 'block' }}>
        {/* ── Topbar ── */}
        <div className="branches-topbar" style={{
          background: 'var(--card)', borderBottom: '1px solid var(--border)',
          padding: '10px 16px',
          display: 'flex', flexDirection: 'row',
          alignItems: 'center', justifyContent: 'space-between',
          position: 'fixed', top: 68, left: 'var(--current-sidebar-w, 62px)', right: 0,
          zIndex: 40, boxSizing: 'border-box', gap: 8,
        }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <button onClick={openReviews} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, border: '1.5px solid #fde68a', background: '#fffbeb', color: '#92400e', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
          ...
          <div style={{ position: "fixed", bottom: 28, right: 28, zIndex: 999, display: "flex", alignItems: "center", gap: 10 }}
            ...
            <span className="fab-tooltip" style={{
              ...
              <span style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.2 }}>
                <span style={{ fontSize: 12, fontWeight: 800, color: '#fff' }}>Book Appointment</span>
                <span style={{ fontSize: 10, fontWeight: 500, color: 'rgba(255,255,255,0.55)' }}>Request a visit</span>
              </span>
            ...
            <button onClick={() => { setPets([{ ...EMPTY_PET }]); setContact(''); setBookStep('service'); fetchMyPets(); setShowModal(true); }} className="fab-btn" style={{ width: 56, height: 56, borderRadius: "50%", background: "linear-gradient(135deg,#1e3a8a,#3b82f6)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 20px rgba(30,58,138,0.4)", transition: "transform 0.2s, box-shadow 0.2s", flexShrink: 0 }}>
        ...
        <div style={{ padding: '12px', paddingTop: 122, width: '100%', boxSizing: 'border-box' }}>
          ...
          {appts.some(a => a.status === 'Pending') && (
            <div style={{
              ...
              <span style={{ color: '#d97706' }}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{ width: 18, height: 18 }}><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg></span>
              <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: 18 }}>
            {[
              { label: 'Total', value: appts.length, icon: '/icon/attended.png', color: '#dbeafe', filter: 'invert(37%) sepia(90%) saturate(500%) hue-rotate(195deg) brightness(95%)', accent: '#3b82f6', sub: 'All appointments', subColor: 'var(--muted)', filterValue: '' },
              { label: 'Upcoming', value: appts.filter(a => a.status === 'Confirmed' || a.status === 'Pending').length, icon: '/icon/admitted.png', color: '#dcfce7', filter: 'invert(50%) sepia(60%) saturate(400%) hue-rotate(100deg) brightness(90%)', accent: '#16a34a', sub: 'Confirmed & pending', subColor: '#16a34a', filterValue: 'upcoming' },
              { label: 'Pending', value: appts.filter(a => a.status === 'Pending').length, icon: '/icon/chat_bubble.png', color: '#fef9c3', filter: 'invert(70%) sepia(80%) saturate(500%) hue-rotate(5deg) brightness(95%)', accent: '#d97706', sub: appts.filter(a => a.status === 'Pending').length === 0 ? 'All cleared' : 'Awaiting approval', subColor: appts.filter(a => a.status === 'Pending').length === 0 ? '#16a34a' : '#d97706', filterValue: 'Pending' },
              { label: 'Cancelled', value: appts.filter(a => a.status === 'Cancelled').length, icon: '/icon/cancel.png', color: '#fee2e2', filter: 'invert(30%) sepia(80%) saturate(500%) hue-rotate(330deg) brightness(95%)', accent: '#dc2626', sub: 'Cancelled requests', subColor: 'var(--muted)', filterValue: 'Cancelled' },
            ].map((sc, i) => (
              <div key={i} className="fade-in"
                role="button" tabIndex={0}
                onClick={() => setApptStatFilter(f => f === sc.filterValue ? '' : sc.filterValue)}
                onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setApptStatFilter(f => f === sc.filterValue ? '' : sc.filterValue); } }}
                style={{
                background: 'var(--card)',
                borderRadius: 11,
                border: apptStatFilter === sc.filterValue ? `1.5px solid ${sc.accent}` : '1px solid var(--border)',
                ...
                gap: 10,
                boxShadow: apptStatFilter === sc.filterValue ? `0 0 0 2px ${sc.accent}22` : 'var(--shadow)',
                minWidth: 0,
                overflow: 'hidden',
                ...
                onMouseLeave={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = apptStatFilter === sc.filterValue ? `0 0 0 2px ${sc.accent}22` : 'var(--shadow)'; e.currentTarget.style.border = apptStatFilter === sc.filterValue ? `1.5px solid ${sc.accent}` : '1px solid var(--border)'; e.currentTarget.style.borderTop = `3px solid ${sc.accent}`; }}
              >
                ...
                <div style={{ minWidth: 0 }}>
                  <p style={{ fontSize: 10, color: 'var(--muted)', fontWeight: 700, marginBottom: 3, textTransform: 'uppercase', letterSpacing: '0.6px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{sc.label}</p>
                  <h3 style={{ fontSize: 24, fontWeight: 800, color: 'var(--text)', margin: '0 0 2px', lineHeight: 1 }}>{sc.value}</h3>
                  <p style={{ fontSize: 10, color: sc.subColor || 'var(--muted)', margin: 0, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{sc.sub}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* ── Appointments table ── */}
          <div style={{ background: 'var(--card)', borderRadius: 12, border: '1px solid var(--border)', boxShadow: 'var(--shadow)', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: isMobile ? '10px 12px' : '14px 18px', borderBottom: '1px solid var(--border)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                {apptStatFilter && (
                  <button onClick={() => setApptStatFilter('')} style={{ fontSize: 11, fontWeight: 700, color: 'var(--royal)', background: 'var(--light-blue)', border: 'none', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontFamily: 'inherit' }}>
                    Clear filter ✕
                  </button>
                )}
                <span style={{ color: 'var(--muted)', fontSize: isMobile ? 11 : 12 }}>{filteredAppts.length} record{filteredAppts.length !== 1 ? 's' : ''}</span>
              </div>
            </div>
            <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 780 }}>
                <thead>
                  <tr>{['Pet', 'Veterinarian', 'Date & Time', 'Room', 'Purpose', 'Price', 'Status', 'Actions'].map(h => (                    <th key={h} style={{ background: 'var(--bg)', padding: '11px 14px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.5px', borderBottom: '1px solid var(--border)', whiteSpace: 'nowrap' }}>{h}</th>
                  ))}</tr>
                </thead>
                <tbody>
                  {loading ? (
                    ...
                    <tr><td colSpan={8} style={{ textAlign: 'center', padding: 40, color: 'var(--muted)' }}>{apptStatFilter ? 'No appointments match this filter' : 'No appointments yet'}</td></tr>                  
                  ) : filteredAppts.map(a => {
                    ...
                    return (
                      <tr key={a.id} className="fade-in" onClick={() => { setSelectedAppt(a); setShowViewModal(true); }} style={{ background: 'var(--card)', animationDelay: `${filteredAppts.indexOf(a) * 0.05}s`, cursor: 'pointer' }}>
                        {/* Pet */}
                        ...
                        <td style={{ padding: '8px 14px', borderBottom: '1px solid var(--border)', verticalAlign: 'middle' }}>
                          <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-start', alignItems: 'center' }}>
                            <button title="View" style={{ height: 28, padding: '0 10px', gap: 5, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: '#eff6ff', border: '1.5px solid #bfdbfe', color: '#1d4ed8', borderRadius: 20, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
                              onClick={(e) => { e.stopPropagation(); setSelectedAppt(a); setShowViewModal(true); }}>
                            ...
                            {(a.status === 'Pending' || a.status === 'Confirmed') && (
                              <button title="Cancel" style={{ height: 28, padding: '0 10px', gap: 5, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: '#fef2f2', border: '1.5px solid #fca5a5', color: '#dc2626', borderRadius: 20, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
                                onClick={(e) => { e.stopPropagation(); cancelAppt(a.id); }}>
                            ...
                            {a.status === 'Completed' && (
                              myReviews[a.id] ? (
                                <span title="Reviewed" style={{ height: 28, padding: '0 10px', gap: 5, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: '#fefce8', border: '1.5px solid #fde68a', color: '#a16207', borderRadius: 20, fontSize: 11, fontWeight: 600 }}>
                                ...
                                <button title="Rate Service" style={{ height: 28, padding: '0 10px', gap: 5, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: '#fefce8', border: '1.5px solid #fde68a', color: '#a16207', borderRadius: 20, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
                                  onClick={(e) => { e.stopPropagation(); openReviewModal(a); }}>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 16, overflowY: 'auto' }}>
          <div style={{ background: 'var(--card)', borderRadius: 14, width: '100%', maxWidth: 620, maxHeight: 'calc(100vh - 32px)', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.28)', overflow: 'hidden', margin: 'auto' }}>
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
                    ...
                    {SERVICE_OPTIONS.map(opt => {
                      ...
                      return (
                        <button
                          key={opt}
                          type="button"
                          onClick={() => { updatePet(0, { purpose: opt, imagingType: '', vet: pets[0].vet }); setBookStep('form'); }}
                          style={{
              ...
              {bookStep === 'form' && pets.map((pet, idx) => {
                return (
                  <div key={idx} style={{ borderBottom: '6px solid var(--bg)' }}>

                    {/* Pet header bar */}
                    <div style={{ background: 'var(--bg)', borderBottom: '1px solid var(--border)', padding: '6px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px', color: 'var(--muted)', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                      ...
                      {pets.length > 1 && (
                        <button onClick={() => removePet(idx)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#dc2626', fontSize: 11, fontWeight: 700, padding: '2px 6px', display: 'inline-flex', alignItems: 'center', gap: 4, fontFamily: 'inherit' }}>
                    ...
                    <div style={{ padding: '10px 16px', borderBottom: '1px solid #e2e8f0' }}>
                      <div style={{ display: 'flex', border: '1.5px solid var(--border)', borderRadius: 8, overflow: 'hidden', width: 'fit-content', marginBottom: pet.mode === 'existing' ? 10 : 0 }}>
                        {[{ key: 'new', label: 'New Pet' }, { key: 'existing', label: 'My Pets' }].map(({ key, label }) => (
                          <button key={key} type="button"
                            onClick={() => updatePet(idx, key === 'existing' ? { mode: 'existing', patient: '', species: 'Dog' } : { mode: 'new', existingId: null })}
                            style={{ padding: '6px 16px', border: 'none', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s', background: pet.mode === key ? 'var(--royal)' : '#fff', color: pet.mode === key ? '#fff' : 'var(--muted)' }}>
                            {label}
                          </button>
                        ))}
                      </div>
                      ...
                      {pet.mode === 'existing' ? (
                        loadingMyPets ? (
                          <div style={{ fontSize: 12, color: 'var(--muted)' }}>Loading your pets…</div>
                        ) : myPets.length === 0 ? (
                          <div style={{ fontSize: 12, color: 'var(--muted)', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px' }}>
                            You don't have any registered pets yet. Switch to "New Pet" to add one.
                          </div>
                        ) : (
                          <PetPicker
                            pets={myPets}
                            ...
                            onChange={(id) => {
                              ...
                              updatePet(idx, { existingId: sel?.id || null, species: sel?.species || 'Dog' });
                            }}
                          />
                        )
                      ) : (
                        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 10 }}>
                          <div>
                            ...
                            <input type="text" value={pet.patient} onChange={e => updatePet(idx, { patient: e.target.value })} placeholder="e.g. Buddy"
                          ...
                          <div>
                            <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 8 }}>Pet Type</div>
                            <div style={{ display: 'flex', gap: 8 }}>
                              {['Dog', 'Cat'].map(type => (
                                <div key={type} onClick={() => updatePet(idx, { species: type })}
                                  style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 6, padding: '6px 10px', borderRadius: 8, cursor: 'pointer', border: `1.5px solid ${pet.species === type ? (type === 'Dog' ? '#1d4ed8' : '#15803d') : '#e2e8f0'}`, background: pet.species === type ? (type === 'Dog' ? '#eff6ff' : '#f0fdf4') : 'transparent', transition: 'all 0.15s' }}>
                                  <span style={{ fontSize: 12, fontWeight: 700, color: pet.species === type ? (type === 'Dog' ? '#1d4ed8' : '#15803d') : '#94a3b8' }}>{type}</span>
                                </div>
                              ))}
                            </div>
                          </div>
                    ...
                    {/* Purpose · Vet */}
                    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', borderBottom: '1px solid #e2e8f0' }}>
                        <div style={{ padding: '10px 16px', borderRight: isMobile ? 'none' : '1px solid #e2e8f0', borderBottom: isMobile ? '1px solid #e2e8f0' : 'none' }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6 }}>Purpose</div>
                        <CustomSelect
                          value={pet.purpose}
                          onChange={val => updatePet(idx, { purpose: val, imagingType: '', vet: pet.vet })}
                        ...
                        {pet.purpose === 'Imaging' && (
                          <div style={{ marginTop: 8 }}>
                            ...
                            <CustomSelect
                              ...
                              onChange={val => updatePet(idx, { imagingType: val })}
                        ...
                        {pet.purpose && (() => {
                          const price = getServicePrice(pet.purpose, user.branchId, pet.imagingType);
                          return (
                            <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 8, padding: '8px 12px' }}>
                              <span style={{ fontSize: 11, fontWeight: 700, color: '#166534' }}>Service Price</span>
                              <span style={{ fontSize: 14, fontWeight: 800, color: '#16a34a' }}>
                                {price != null ? `₱${Number(price).toLocaleString()}` : 'Contact clinic'}
                              </span>
                            </div>
                          );
                        })()}
                        <p style={{ margin: '6px 0 0', fontSize: 10, color: 'var(--muted)', fontStyle: 'italic' }}>
                          Price is fixed by the clinic and cannot be changed here.
                        </p>
                      </div>
                      <div style={{ padding: '10px 16px' }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6 }}>
                          Preferred Vet <span style={{ color: '#ef4444' }}>*</span>
                        </div>
                        <CustomSelect
                          value={pet.vet}
                          onChange={val => {
                          ...
                          options={VETS}
                        />
                        {pet.vet && pet.vet !== 'Any Available' && VET_SCHEDULE[pet.vet] && (
                          <p style={{ margin: '6px 0 0', fontSize: 10, color: 'var(--muted)' }}>
                            Available days: {VET_SCHEDULE[pet.vet].map(d => DAY_NAMES[d]).join(', ')}
                          </p>
                        )}
                        {pet.vet && pet.vet !== 'Any Available' && VET_TIME_SCHEDULE[pet.vet] && (
                          <p style={{ margin: '2px 0 0', fontSize: 10, color: 'var(--muted)' }}>
                            Available times: {VET_TIME_SCHEDULE[pet.vet].join(', ')}
                          </p>
                        )}
                      </div>
                    </div>
                      ...
                      <div style={{ padding: '10px 16px', borderRight: isMobile ? 'none' : '1px solid #e2e8f0', borderBottom: isMobile ? '1px solid #e2e8f0' : 'none' }}>
                        ...
                        <DatePicker
                          ...
                          onChange={val => {
                            ...
                            if (pet.vet && pet.vet !== 'Any Available') fetchVetBookedTimes(pet.vet, val);
                          }}
                          placeholder="Select date"
                        />
                        {pet.vet && pet.vet !== 'Any Available' && pet.date && !isVetAvailableOnDate(pet.vet, pet.date) && (
                          <p style={{ margin: '6px 0 0', fontSize: 11, color: '#dc2626', fontWeight: 600 }}>
                            {pet.vet} isn't available on this day. Try {VET_SCHEDULE[pet.vet]?.map(d => DAY_NAMES[d]).join(', ') || 'another day'}.
                          </p>
                        )}
                      </div>
                      <div style={{ padding: '10px 16px' }}>
                        ...
                        <CustomSelect
                          ...
                          options={TIMES.map(t => {
                            const vetKey = pet.vet && pet.date ? `${pet.vet}|${pet.date}` : null;
                            const vetTaken = vetKey && pet.vet !== 'Any Available' && (vetBookedTimes[vetKey] || []).includes(t);
                            const vetDayBlocked = pet.vet && pet.vet !== 'Any Available' && pet.date && !isVetAvailableOnDate(pet.vet, pet.date);
                            const vetTimeBlocked = pet.vet && pet.vet !== 'Any Available' && !isVetAvailableAtTime(pet.vet, t);
                            const disabled = vetTaken || vetDayBlocked || vetTimeBlocked;
                            let label = t;
                            if (vetDayBlocked) label = `${t} — Vet unavailable`;
                            else if (vetTimeBlocked) label = `${t} — Not this vet's hours`;
                            else if (vetTaken) label = `${t} — Already booked`;
                    ...
                    {getVetConflictFor(pet) === 'vet-day' && (
                      <div style={{ background: '#fee2e2', borderTop: '1px solid #fca5a5', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                        <p style={{ margin: 0, color: '#b91c1c', fontSize: 11 }}>{pet.vet} doesn't work on this day. Please pick another date or vet.</p>
                      </div>
                    )}
                    {getVetConflictFor(pet) === 'vet-time' && (
                      <div style={{ background: '#fee2e2', borderTop: '1px solid #fca5a5', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                        <p style={{ margin: 0, color: '#b91c1c', fontSize: 11 }}>{pet.vet} isn't available at this time. Available times: {VET_TIME_SCHEDULE[pet.vet]?.join(', ') || '—'}.</p>
                      </div>
                    )}
                    {getVetConflictFor(pet) === 'vet-slot' && (
                      <div style={{ background: '#fee2e2', borderTop: '1px solid #fca5a5', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                        <p style={{ margin: 0, color: '#b91c1c', fontSize: 11 }}>{pet.vet} is already booked at that time. Please pick a different slot.</p>
                      </div>
                    )}
                    ...
                    <div style={{ padding: '10px 16px' }}>
                      ...
                      <textarea value={pet.notes} onChange={e => updatePet(idx, { notes: e.target.value })}
              ...
              <div style={{ padding: '12px 16px', borderBottom: '1.5px solid #e2e8f0' }}>
                <button type="button" onClick={addPet}
                  style={{ width: '100%', padding: '10px 14px', borderRadius: 8, border: '1.5px dashed #c7d2fe', background: '#f5f3ff', color: '#6366f1', fontWeight: 700, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              ...
              <div style={{ borderBottom: '1.5px solid #e2e8f0' }}>
                ...
                <div style={{ padding: '10px 16px' }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6 }}>Contact Number</div> <input type="text" value={contact}
                  ...
                  <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 8, padding: '8px 12px', fontSize: 11, color: '#1e40af', marginTop: 10 }}>
                    {pets.length > 1 ? 'All appointments' : 'Your appointment'} will be <strong>Pending</strong> until staff approves {pets.length > 1 ? 'them' : 'it'}.
                  </div>
                </div>
              </div>
              </>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, padding: '14px 24px', borderTop: '2px solid var(--border)', background: 'var(--bg)', flexShrink: 0, flexWrap: 'wrap' }}>
              <div>
                {bookStep === 'form' && (
                  <button className="btn btn-ghost" style={{ width: 'auto' }} onClick={() => setBookStep('service')}>← Change Service</button>
                )}
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button
                  ...
                  style={{ width: 'auto', background: '#0f172a', borderColor: '#0f172a', opacity: (bookStep !== 'form' || hasAnyConflict || isFormIncomplete) ? 0.5 : 1, cursor: (bookStep !== 'form' || hasAnyConflict || isFormIncomplete) ? 'not-allowed' : 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6 }}
                  onClick={saveAppt}
                  disabled={saving || bookStep !== 'form' || hasAnyConflict || isFormIncomplete}
                  title={bookStep !== 'form' ? 'Please select a service first.' : isFormIncomplete ? 'Please fill in all required fields before submitting.' : undefined}>
                  ...
                  {saving ? 'Submitting...' : pets.length > 1 ? `Submit ${pets.length} Requests` : 'Submit Request'}
                </button>
              </div>
            </div>
          </div>
      ...
      {showViewModal && selectedAppt && (() => {
        const statusTheme = {
          Confirmed: { dot: '#16a34a', text: '#166534', bg: '#f0fdf4', border: '#bbf7d0' },
          Pending: { dot: '#d97706', text: '#92400e', bg: '#fffbeb', border: '#fde68a' },
          Cancelled: { dot: '#dc2626', text: '#991b1b', bg: '#fef2f2', border: '#fecaca' },
          Completed: { dot: '#2563eb', text: '#1e40af', bg: '#eff6ff', border: '#bfdbfe' },
        }[selectedAppt.status] || { dot: '#9ca3af', text: '#475569', bg: '#f8fafc', border: '#e2e8f0' };

        const infoRow = (label, value, icon) => (
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
            <div style={{ width: 26, height: 26, borderRadius: 7, background: 'var(--light-blue)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 }}>
              {icon}
            </div>
            <div style={{ minWidth: 0 }}>
              <p style={{ margin: 0, fontSize: 9.5, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{label}</p>
              <p style={{ margin: '2px 0 0', fontSize: 13, fontWeight: 600, color: 'var(--text)', wordBreak: 'break-word' }}>{value}</p>
            </div>
          </div>
        );

        const icons = {
          pet: <svg viewBox="0 0 20 20" fill="#1e3a8a" style={{ width: 13, height: 13 }}><path d="M11.9 8.4c1.3 0 2.1-1.9 2.1-3.1 0-1-.5-2.2-1.5-2.2-1.3 0-2.1 1.9-2.1 3.1 0 1 .5 2.2 1.5 2.2zm-3.8 0c1 0 1.5-1.2 1.5-2.2C9.6 4.9 8.8 3 7.5 3 6.5 3 6 4.2 6 5.2c-.1 1.3.7 3.2 2.1 3.2zm7.4-1c-1.3 0-2.2 1.8-2.2 3.1 0 .9.4 1.8 1.3 1.8 1.3 0 2.2-1.8 2.2-3.1 0-.9-.5-1.8-1.3-1.8zm-8.7 3.1c0-1.3-1-3.1-2.2-3.1-.9 0-1.3.9-1.3 1.8 0 1.3 1 3.1 2.2 3.1.9 0 1.3-.9 1.3-1.8zm3.2-.2c-2 0-4.7 3.2-4.7 5.4 0 1 .7 1.3 1.5 1.3 1.2 0 2.1-.8 3.2-.8 1 0 1.9.8 3 .8.8 0 1.7-.2 1.7-1.3 0-2.2-2.7-5.4-4.7-5.4z" /></svg>,
          purpose: <svg viewBox="0 0 24 24" fill="none" stroke="#1e3a8a" strokeWidth="2" strokeLinecap="round" style={{ width: 13, height: 13 }}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>,
          vet: <svg viewBox="0 0 24 24" fill="none" stroke="#1e3a8a" strokeWidth="2" strokeLinecap="round" style={{ width: 13, height: 13 }}><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>,
          room: <svg viewBox="0 0 24 24" fill="none" stroke="#1e3a8a" strokeWidth="2" strokeLinecap="round" style={{ width: 13, height: 13 }}><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" /><polyline points="9 22 9 12 15 12 15 22" /></svg>,
          date: <svg viewBox="0 0 24 24" fill="none" stroke="#1e3a8a" strokeWidth="2" strokeLinecap="round" style={{ width: 13, height: 13 }}><rect x="3" y="4" width="18" height="18" rx="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>,
          time: <svg viewBox="0 0 24 24" fill="none" stroke="#1e3a8a" strokeWidth="2" strokeLinecap="round" style={{ width: 13, height: 13 }}><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>,
          contact: <svg viewBox="0 0 24 24" fill="none" stroke="#1e3a8a" strokeWidth="2" strokeLinecap="round" style={{ width: 13, height: 13 }}><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.59 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z" /></svg>,
          notes: <svg viewBox="0 0 24 24" fill="none" stroke="#1e3a8a" strokeWidth="2" strokeLinecap="round" style={{ width: 13, height: 13 }}><line x1="8" y1="6" x2="21" y2="6" /><line x1="8" y1="12" x2="21" y2="12" /><line x1="8" y1="18" x2="21" y2="18" /><line x1="3" y1="6" x2="3.01" y2="6" /><line x1="3" y1="12" x2="3.01" y2="12" /><line x1="3" y1="18" x2="3.01" y2="18" /></svg>,
        };
        ...
        return (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 16 }}
            onClick={() => { setShowViewModal(false); setSelectedAppt(null); }}>
            <div style={{ background: 'var(--card)', borderRadius: 16, width: '100%', maxWidth: 460, boxShadow: '0 32px 80px rgba(0,0,0,0.35)', overflow: 'hidden' }}
              onClick={e => e.stopPropagation()}>

              {/* Header */}
              <div style={{ background: 'linear-gradient(135deg,#0f172a 0%,#1e3a8a 100%)', padding: '20px 22px 18px', position: 'relative' }}>
                ...
                <div style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                  <div>
                    <p style={{ margin: 0, fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', letterSpacing: '0.6px' }}>Appointment · #{String(selectedAppt.id).slice(-6).toUpperCase()}</p>
                    <h3 style={{ margin: '4px 0 0', fontSize: 18, fontWeight: 800, color: '#fff' }}>{selectedAppt.patient}</h3>
                    <p style={{ margin: '2px 0 0', fontSize: 12, color: 'rgba(255,255,255,0.65)' }}>{selectedAppt.purpose}{selectedAppt.date ? ` · ${selectedAppt.date}` : ''}</p>
                  </div>
                  <button onClick={() => { setShowViewModal(false); setSelectedAppt(null); }} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: 8, width: 28, height: 28, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', flexShrink: 0 }}>
                ...
                <div style={{ position: 'relative', zIndex: 1, marginTop: 14, display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.18)', borderRadius: 20, padding: '4px 10px 4px 8px' }}>
                  <span style={{ width: 7, height: 7, borderRadius: '50%', background: statusTheme.dot }} />
                  <span style={{ fontSize: 11.5, fontWeight: 700, color: '#fff' }}>{selectedAppt.status}</span>
                  {selectedAppt.status === 'Pending' && (
                    <span style={{ fontSize: 10.5, color: 'rgba(255,255,255,0.6)' }}>· Awaiting approval</span>
                  )}
                </div>
              </div>
              ...
              <div style={{ padding: '18px 22px', background: 'var(--card)' }}>
                ...
                {/* Schedule section */}
                <p style={{ margin: '0 0 10px', fontSize: 10, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.6px' }}>Schedule</p>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px 16px', marginBottom: 18 }}>
                  ...
                  {infoRow('Veterinarian', selectedAppt.vet || '—', icons.vet)}
                  {selectedAppt.room && infoRow('Room', selectedAppt.room, icons.room)}
                </div>

                <div style={{ height: 1, background: 'var(--border)', margin: '0 0 18px' }} />

                {/* Patient & billing section */}
                <p style={{ margin: '0 0 10px', fontSize: 10, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.6px' }}>Patient & Billing</p>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px 16px' }}>
                  {infoRow('Pet Name', selectedAppt.patient, icons.pet)}
                  {infoRow('Purpose', selectedAppt.purpose, icons.purpose)}
                  {infoRow('Contact', selectedAppt.contact || '—', icons.contact)}
                </div>

                {/* Price highlight */}
                <div style={{ marginTop: 16, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--muted)' }}>Estimated Cost</span>
                  <span style={{ fontSize: 17, fontWeight: 800, color: 'var(--royal)' }}>{selectedAppt.price ? `₱${Number(selectedAppt.price).toLocaleString()}` : '—'}</span>
                </div>

                {/* Notes */}
                {selectedAppt.notes && (
                  <div style={{ marginTop: 16 }}>
                    <p style={{ margin: '0 0 6px', fontSize: 10, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.6px' }}>Notes</p>
                    <p style={{ margin: 0, fontSize: 12.5, color: 'var(--text)', lineHeight: 1.5, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 12px' }}>{selectedAppt.notes}</p>
                  </div>
                )}
              </div>
              ...
              <div style={{ padding: '14px 22px', borderTop: '1px solid var(--border)', background: 'var(--bg)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                {(selectedAppt.status === 'Pending' || selectedAppt.status === 'Confirmed') ? (
                  <button onClick={() => { cancelAppt(selectedAppt.id); setShowViewModal(false); setSelectedAppt(null); }}
                    style={{ padding: '9px 16px', borderRadius: 8, border: '1.5px solid #fca5a5', background: '#fef2f2', color: '#dc2626', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <button onClick={() => { setShowViewModal(false); setSelectedAppt(null); }}
                  style={{ padding: '9px 22px', borderRadius: 8, border: '1px solid #e2e8f0', background: 'var(--card)', color: '#64748b', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                  Close
                </button>
              </div>
            </div>
   ...
   {/* ══ REVIEW MODAL ══ */}
      {reviewModal.show && reviewModal.appt && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 16 }}>
          <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 420, boxShadow: '0 24px 64px rgba(0,0,0,0.28)', overflow: 'hidden' }}>
            <div style={{ background: 'linear-gradient(135deg,#78350f,#d97706)', padding: '16px 20px' }}>
              <h3 style={{ margin: 0, fontSize: 15, fontWeight: 800, color: '#fff' }}>Rate Your Visit</h3>
            ...
            <div style={{ padding: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'center', gap: 6, marginBottom: 16 }}>
                {[1, 2, 3, 4, 5].map(star => (
                  <svg key={star} onClick={() => setReviewModal(m => ({ ...m, rating: star }))}
                    width="32" height="32" viewBox="0 0 24 24"
                    fill={star <= reviewModal.rating ? '#f59e0b' : 'none'} stroke={star <= reviewModal.rating ? '#f59e0b' : '#cbd5e1'} strokeWidth="1.5"
                    style={{ cursor: 'pointer' }}>
                    <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                  </svg>
                ))}
              </div>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6 }}>Comments (optional)</div>
              <textarea value={reviewModal.comment} onChange={e => setReviewModal(m => ({ ...m, comment: e.target.value }))}
                placeholder="Tell us about your experience..."
                style={{ width: '100%', border: '1px solid #e2e8f0', borderRadius: 8, background: 'transparent', fontSize: 13, color: '#0f172a', outline: 'none', resize: 'vertical', minHeight: 70, fontFamily: 'inherit', lineHeight: 1.6, boxSizing: 'border-box', padding: '8px 10px' }} />
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, padding: '14px 20px', borderTop: '1px solid #e2e8f0' }}>
              <button className="btn btn-ghost" style={{ width: 'auto' }} onClick={() => setReviewModal({ show: false, appt: null, rating: 0, comment: '' })}>Cancel</button>
              <button className="btn" style={{ width: 'auto', background: '#d97706', color: '#fff', border: 'none' }} disabled={submittingReview} onClick={submitReview}>
